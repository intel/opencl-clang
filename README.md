...
```bash
cd <workspace>
git clone https://github.com/llvm/llvm-project.git . -b release/23.x
git clone https://github.com/KhronosGroup/SPIRV-LLVM-Translator.git -b llvm_release_230
git clone https://github.com/intel/opencl-clang.git -b ocl-open-230
```
...
By default, opencl-clang's cmake script is searching for LLVM 23.1. You can
override target version of LLVM by using the `PREFERRED_LLVM_VERSION` cmake
option:

Example:
```bash
cmake -DPREFERRED_LLVM_VERSION="23.1" ../opencl-clang
```
