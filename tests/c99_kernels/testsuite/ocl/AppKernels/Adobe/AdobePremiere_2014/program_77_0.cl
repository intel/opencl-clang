...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE_1D(float4, texture1_1D) GF_KERNEL ShaderKernel_COMPOP_LUT1D(COMMON_PARAMS, GF_OPENCL_TEXTURE_1D(texture1_1D)) { BEGIN_NORMAL LOAD_PARAMS(2) PARAM (__scale1 , program_local[0]) PARAM (__offset1, program_local[1]) TEMP r0, r1; TEX (&r0, fragment_texcoord0, texture0_RECT); MAD (&r1, r0, __scale1, __offset1); TEXs_1d_x (result_color.x, r1.x, texture1_1D); TEXs_1d_y (result_color.y, r1.y, texture1_1D); TEXs_1d_z (result_color.z, r1.z, texture1_1D); MOVs (result_color.w, r0.w); END }
