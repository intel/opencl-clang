...
#endif
GF_KERNEL FillWithColorKernel( GF_PTR(float4) outImage, int inPitch, DevicePixelFormat inDeviceFormat, int inOffsetX, int inOffsetY, int inWidth, int inHeight, float4 inColor) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { WritePixel(inColor, outImage, inPitch, inDeviceFormat, x + inOffsetX, y + inOffsetY); } }
