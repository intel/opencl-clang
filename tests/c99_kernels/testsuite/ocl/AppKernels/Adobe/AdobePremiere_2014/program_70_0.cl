...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE(float4, texture1_RECT) GF_KERNEL ShaderKernel_fxAntiAliasComposerV(COMMON_PARAMS, GF_OPENCL_TEXTURE(texture1_RECT)) { BEGIN_MULTITEXTURE LOAD_PARAMS(2) PARAM (w1 , program_local[0]) PARAM (invSum, program_local[1]) TEMP r0, r1, r2, r3, r4; ADD (&r2, s(fragment_texcoord1, xxxy), co_xxxy(1, -1, 0, 0)); ADD (&r4, s(fragment_texcoord0, xyxy), co_xyxy(0, -1, 0, 1)); TEX (&r0, fragment_texcoord1, texture1_RECT); TEX (&r1, s(r2, xwww), texture1_RECT); MAD (&r3, r1, w1, r0); TEX (&r1, s(r2, ywww), texture1_RECT); MAD (&r3, r1, w1, r3); MUL (&r3, r3, invSum); TEXs_x (r1.x, fragment_texcoord0, texture0_RECT); TEXs_y (r1.y, r4, texture0_RECT); TEXs_z (r1.z, s(r4, zwzw), texture0_RECT); DP3vvs_SAT (&r1, r1, 1); LRP (&result_color, r1, r3, r0); END }
