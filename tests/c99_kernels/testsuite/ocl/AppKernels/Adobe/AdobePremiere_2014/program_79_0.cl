...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_KERNEL ShaderKernel_ColorSpace(COMMON_PARAMS) { BEGIN_NORMAL LOAD_PARAMS(3) PARAM (RCoeffs, program_local[0]) PARAM (GCoeffs, program_local[1]) PARAM (BCoeffs, program_local[2]) TEMP r0; TEX (&r0, fragment_texcoord0, texture0_RECT); DPHsvv (result_color.x, r0, RCoeffs); DPHsvv (result_color.y, r0, GCoeffs); DPHsvv (result_color.z, r0, BCoeffs); MOVs (result_color.w, r0.w); END }
