...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_KERNEL ShaderKernel_fxEmboss(COMMON_PARAMS) { BEGIN_NORMAL LOAD_PARAMS(1) PARAM (embossLevel, program_local[0]) TEMP r0, r1, r2, r3; ADD (&r3, s(fragment_texcoord0, xyxy), co_xyxy(0, 1, -1, 0)); TEX (&r0, fragment_texcoord0, texture0_RECT); MULvvs (&r1, r0, -3); TEX (&r2, r3, texture0_RECT); MADvvsv (&r1, r2, 1.5, r1); TEX (&r2, s(r3, zwzw), texture0_RECT); MADvvsv (&r1, r2, 1.5, r1); DP3 (&r1, r1, embossLevel); ADDvvs_xyz (&result_color, r1, 0.333333333333); MOVs (result_color.w, r0.w); END }
