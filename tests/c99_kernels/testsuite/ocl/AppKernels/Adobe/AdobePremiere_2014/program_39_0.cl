...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE_3D(float4, texture2_3D) GF_KERNEL ShaderKernel_fxTechnicolor3strip(COMMON_PARAMS, GF_OPENCL_TEXTURE_3D(texture2_3D)) { BEGIN_NORMAL TEMP r0, r1, r2; TEX (&r0, fragment_texcoord0, texture0_RECT); MOV_SAT (&r2, r0); MADvvss (&r1, r2, 0.75, 0.015625); TEX_3d_xyz (&result_color, r1, texture2_3D); MOVs (result_color.w, r0.w); END }
