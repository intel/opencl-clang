...
#endif
GF_KERNEL CopyAlphaKernel( const GF_PTR(float) inAlphaSrc, int inAlphaSrcPitch, int inSrcWidth, int inSrcHeight, GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inDstWidth, int inDstHeight) { int x = KernelX(); int y = KernelY(); if (x < inDstWidth && y < inDstHeight) { PixelRGB pixel = BGRA(0,0,0,0); if (x < inSrcWidth && y < inSrcHeight) { pixel.a = GF_READ2D(inAlphaSrc, inAlphaSrcPitch, x, y); } WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, x, y); } } GF_KERNEL CopyDeepAlphaKernel( const GF_PTR(float4) inAlphaSrc, int inAlphaSrcPitch, int inSrcWidth, int inSrcHeight, GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inDstWidth, int inDstHeight) { int x = KernelX(); int y = KernelY(); if (x < inDstWidth && y < inDstHeight) { PixelRGB pixel = BGRA(0,0,0,0); if (x < inSrcWidth && y < inSrcHeight) { PixelRGB maskPixel = ReadRGBPixel(inAlphaSrc, inAlphaSrcPitch, inDeviceFormat, x, y); pixel.a = maskPixel.a; } WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, x, y); } }
