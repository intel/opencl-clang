...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE_1D(float4, texture1_1D) GF_CUDA_TEXTURE_3D(float4, texture2_3D) GF_KERNEL ShaderKernel_COMPOP_LUT1D3D(COMMON_PARAMS, GF_OPENCL_TEXTURE_1D(texture1_1D), GF_OPENCL_TEXTURE_3D(texture2_3D)) { BEGIN_NORMAL LOAD_PARAMS(2) PARAM (__scale1 , program_local[0]) PARAM (__offset1, program_local[1]) TEMP r0, r1; TEX (&r0, fragment_texcoord0, texture0_RECT); MAD (&r1, r0, __scale1, __offset1); TEXs_1d_x (r1.x, r1.x, texture1_1D); TEXs_1d_y (r1.y, r1.y, texture1_1D); TEXs_1d_z (r1.z, r1.z, texture1_1D); TEX_3d_xyz (&result_color, r1, texture2_3D); MOVs (result_color.w, r0.w); END }
