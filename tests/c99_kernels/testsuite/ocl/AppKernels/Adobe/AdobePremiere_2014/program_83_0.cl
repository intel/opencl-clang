...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_KERNEL ShaderKernel_ASCCombined(COMMON_PARAMS) { BEGIN_NORMAL LOAD_PARAMS(4) PARAM (slope , program_local[0]) PARAM (offset, program_local[1]) PARAM (power , program_local[2]) PARAM (saturation, program_local[3]) TEMP r0, r1, r2; TEX (&r0, fragment_texcoord0, texture0_RECT); MAD (&r1, r0, slope, offset); CMPvvvs (&r2, r1, c4(-1,-1,-1, 1), 1); ABS (&r1, r1); POWs (r0.x, r1.x, power.x); POWs (r0.y, r1.y, power.y); POWs (r0.z, r1.z, power.z); MUL (&r0, r0, r2); DP3_xyz (&r1, r0, c3(0.2126, 0.7152, 0.0722)); MOVs (r1.w, 0); SUB (&r2, r0, r1); MAD (&result_color, r2, saturation, r1); END }
