...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_KERNEL ShaderKernel_HDRLayer(COMMON_PARAMS) { BEGIN_NORMAL LOAD_PARAMS(2) PARAM (newAlpha, program_local[0]) PARAM (gain , program_local[1]) TEMP r0; TEX (&r0, fragment_texcoord0, texture0_RECT); MUL_xyz (&result_color, r0, gain); MOVs (result_color.w, newAlpha.x); END }
