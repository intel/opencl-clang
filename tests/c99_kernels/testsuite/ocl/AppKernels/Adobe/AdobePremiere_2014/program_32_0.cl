...
#define bGamma paiBrVector[0]

#define bPreSat paiBrVector[1]

#define bPostSat paiBrVector[2]
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_KERNEL ShaderKernel_IRIDASSimplePrimary(COMMON_PARAMS, GF_PTR(int) paiBrVector) { BEGIN_NORMAL LOAD_PARAMS(8) PARAM (pregain , program_local[0]) PARAM (preoffset , program_local[1]) PARAM (postgain , program_local[2]) PARAM (postoffset, program_local[3]) PARAM (luminance , program_local[4]) PARAM (presaturation , program_local[5]) PARAM (postsaturation, program_local[6]) PARAM (gamma , program_local[7]) TEMP r0, r1, r2, r3; TEX (&r0, fragment_texcoord0, texture0_RECT); MOVs (r3.w, r0.w); if (bPreSat) { DP3 (&r1, r0, luminance); SUB (&r2, r0, r1); MAD (&r0, r2, presaturation, r1); } MUL (&r0, r0, pregain); ADD (&r0, r0, preoffset); if (bGamma) { CMPvvvs (&r1, r0, c4(-1,-1,-1, 1), 1); ABS (&r2, r0); POWs (r0.x, r2.x, gamma.x); POWs (r0.y, r2.y, gamma.y); POWs (r0.z, r2.z, gamma.z); MUL (&r0, r0, r1); } MUL (&r0, r0, postgain); ADD (&r0, r0, postoffset); if (bPostSat) { DP3 (&r1, r0, luminance); SUB (&r2, r0, r1); MAD (&r0, r2, postsaturation, r1); } MOV_xyz (&result_color, r0); MOVs (result_color.w, r3.w); END }
