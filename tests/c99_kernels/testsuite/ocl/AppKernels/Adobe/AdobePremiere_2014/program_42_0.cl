...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_KERNEL ShaderKernel_fxSepiaTone(COMMON_PARAMS) { BEGIN_NORMAL LOAD_PARAMS(1) PARAM (IQMask, program_local[0]) PARAMC YCoeffs = c3(0.2989, 0.5866, 0.1144); PARAMC ICoeffs = c3(0.5959, -0.2741, -0.3218); PARAMC QCoeffs = c3(0.2113, -0.5227, 0.3113); PARAMC YInvCoeffs = c3(1.000162113398, 0.956268145567, 0.620971871091); PARAMC IInvCoeffs = c3(1.000035166139, -0.271705204519, -0.648373780342); PARAMC QInvCoeffs = c3(1.000270243429, -1.105299613107, 1.702164049643); TEMP r0, r1; TEX (&r0, fragment_texcoord0, texture0_RECT); DP3svv (r1.x, YCoeffs, r0); DP3svv (r1.y, ICoeffs, r0); DP3svv (r1.z, QCoeffs, r0); MOV_yz (&r1, IQMask); DP3svv (result_color.x, YInvCoeffs, r1); DP3svv (result_color.y, IInvCoeffs, r1); DP3svv (result_color.z, QInvCoeffs, r1); MOVs (result_color.w, r0.w); END }
