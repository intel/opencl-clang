...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE_3D(float4, texture1_3D) GF_KERNEL ShaderKernel_COMPOP_LUT3D(COMMON_PARAMS, GF_OPENCL_TEXTURE_3D(texture1_3D)) { BEGIN_NORMAL LOAD_PARAMS(2) PARAM (__scale1 , program_local[0]) PARAM (__offset1, program_local[1]) TEMP r0, r1; TEX (&r0, fragment_texcoord0, texture0_RECT); MAD (&r1, r0, __scale1, __offset1); TEX_3d_xyz (&result_color, r1, texture1_3D); MOVs (result_color.w, r0.w); END }
