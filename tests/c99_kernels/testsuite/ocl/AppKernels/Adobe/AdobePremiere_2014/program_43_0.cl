...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_KERNEL ShaderKernel_fxPaletteCut(COMMON_PARAMS) { BEGIN_NORMAL LOAD_PARAMS(2) PARAM (numOfColors , program_local[0]) PARAM (invNumOfColors, program_local[1]) TEMP r0, r1, r2; TEX (&r0, fragment_texcoord0, texture0_RECT); MOV_SAT (&r1, r0); MADvvvs (&r2, r1, numOfColors, 0.5); FLR (&r2, r2); MUL_xyz (&result_color, r2, invNumOfColors); MOVs (result_color.w, r0.w); END }
