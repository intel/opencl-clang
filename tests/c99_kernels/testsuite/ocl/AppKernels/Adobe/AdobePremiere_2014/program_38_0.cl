...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_KERNEL ShaderKernel_fxTinting(COMMON_PARAMS) { BEGIN_NORMAL LOAD_PARAMS(7) PARAM (color , program_local[0]) PARAM (invColor, program_local[1]) PARAM (cmp0, program_local[2]) PARAM (cmp1, program_local[3]) PARAM (cmp2, program_local[4]) PARAM (cmp3, program_local[5]) PARAM (cmp4, program_local[6]) TEMP r0, r1, r2, r3, r4, r5, r10; TEX (&r0, fragment_texcoord0, texture0_RECT); SUBvsv (&r10, 1, r0); MUL (&r3, r10, invColor); MUL (&r1, r0, color); ADD (&r2, r0, color); SUBvsv (&r3, 1, r3); MAX (&r4, r0, color); MIN (&r5, r0, color); CMP (&r0, cmp0, color, r1); CMP (&r0, cmp1, r0, r2); CMP (&r0, cmp2, r0, r3); CMP (&r0, cmp3, r0, r4); CMP (&result_color, cmp4, r0, r5); END }
