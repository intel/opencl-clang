...
enum { kMaxWaveFormPoints = 16 }; typedef struct { float mPointsX[kMaxWaveFormPoints]; float mPointsY[kMaxWaveFormPoints]; float mPointsY2[kMaxWaveFormPoints]; unsigned int mSize; } WaveformCurveParameters; GF_DEVICE_FUNCTION float InterpolateNaturalCubicSpline2D( GF_CONSTANT_PTR(float) xa, GF_CONSTANT_PTR(float) ya, GF_CONSTANT_PTR(float) y2a, unsigned int n, float x) { float value = x; unsigned int k = 0; unsigned int klo = 0; unsigned int khi = n - 1; float h, b, a; while (khi - klo > 1) { k = (khi + klo) >> 1; if (xa[k] > x) { khi = k; } else { klo = k; } } h = xa[khi] - xa[klo]; if (h != 0.0f) { a = (xa[khi] - x) / h; b = (x - xa[klo]) / h; value = (float)(a * ya[klo] + b * ya[khi] + ((a*a*a-a) * y2a[klo] + (b*b*b-b) * y2a[khi]) * (h*h) / 6.0f); } return value; } GF_DEVICE_FUNCTION float ApplyCurve( GF_CONSTANT_PTR(WaveformCurveParameters) inSpline, float inX) { float value = inX; if (inX < inSpline->mPointsX[0]) { if (inSpline->mPointsY[0] == 0.0f) { value = inX - inSpline->mPointsX[0]; } else if (inSpline->mPointsY[0] == 1.0f) { value = inSpline->mPointsX[0] + 1.0f - inX; } else if (inX >= 0.0f) { value = inSpline->mPointsY[0]; } else { value = inX + inSpline->mPointsY[0]; } } else if (inX > inSpline->mPointsX[inSpline->mSize-1]) { if (inSpline->mPointsY[inSpline->mSize-1] == 0.0f) { value = inSpline->mPointsX[inSpline->mSize-1] - inX; } else if (inSpline->mPointsY[inSpline->mSize-1] == 1.0f) { value = inX + 1.0f - inSpline->mPointsX[inSpline->mSize-1]; } else if (inX <= 1.0f) { value = inSpline->mPointsY[inSpline->mSize-1]; } else { value = inX - (1.0f - inSpline->mPointsY[inSpline->mSize-1]); } } else { value = InterpolateNaturalCubicSpline2D(inSpline->mPointsX, inSpline->mPointsY, inSpline->mPointsY2, inSpline->mSize, inX); } return value; }
#endif
...
#endif
GF_CUDA_CONSTANT(WaveformCurveParameters, inSpline0); GF_CUDA_CONSTANT(WaveformCurveParameters, inSpline1); GF_CUDA_CONSTANT(WaveformCurveParameters, inSpline2); GF_CUDA_CONSTANT(WaveformCurveParameters, inSpline3); GF_DEVICE_FUNCTION PixelRGB CalculateBGRA( GF_CONSTANT_PTR(WaveformCurveParameters) inSpline0, GF_CONSTANT_PTR(WaveformCurveParameters) inSpline1, GF_CONSTANT_PTR(WaveformCurveParameters) inSpline2, GF_CONSTANT_PTR(WaveformCurveParameters) inSpline3, PixelRGB inPixel, float inSecondaryWeight) { PixelRGB result = inPixel; if (inSecondaryWeight > 0.001f) { result.r = ApplyCurve(inSpline1, result.r); result.r = ApplyCurve(inSpline0, result.r); result.r = LERP(inPixel.r, result.r, inSecondaryWeight); result.g = ApplyCurve(inSpline2, result.g); result.g = ApplyCurve(inSpline0, result.g); result.g = LERP(inPixel.g, result.g, inSecondaryWeight); result.b = ApplyCurve(inSpline3, result.b); result.b = ApplyCurve(inSpline0, result.b); result.b = LERP(inPixel.b, result.b, inSecondaryWeight); } return result; } GF_KERNEL RGBCurves_MaskKernel( GF_PTR(float4) inImage, int inPitch, GF_PTR(float) inSecondaryMask, int inSecondaryPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB pixel = ReadRGBPixel(inImage, inPitch, inDeviceFormat, x, y); float secondaryWeight = inSecondaryMask ? GF_READ2D(inSecondaryMask, inSecondaryPitch, x, y) : 1.0f; pixel.r = pixel.g = pixel.b = secondaryWeight; pixel.a = 1.0f; WriteRGBPixel(pixel, inImage, inPitch, inDeviceFormat, x, y); } } GF_KERNEL RGBCurves_LumaKernel( GF_PTR(float4) inImage, int inPitch, GF_PTR(float) inSecondaryMask, int inSecondaryPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, GF_OPENCL_CONSTANT(WaveformCurveParameters, inSpline0), GF_OPENCL_CONSTANT(WaveformCurveParameters, inSpline1), GF_OPENCL_CONSTANT(WaveformCurveParameters, inSpline2), GF_OPENCL_CONSTANT(WaveformCurveParameters, inSpline3)) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB pixelRGB = ReadRGBPixel(inImage, inPitch, inDeviceFormat, x, y); float secondaryWeight = inSecondaryMask ? GF_READ2D(inSecondaryMask, inSecondaryPitch, x, y) : 1.0f; pixelRGB = CalculateBGRA( GF_GET_CONSTANT(inSpline0), GF_GET_CONSTANT(inSpline1), GF_GET_CONSTANT(inSpline2), GF_GET_CONSTANT(inSpline3), pixelRGB, secondaryWeight); PixelYUV pixelYUV = ConvertRGB_To_YUV(pixelRGB); pixelRGB.r = pixelYUV.y; pixelRGB.g = pixelYUV.y; pixelRGB.b = pixelYUV.y; WriteRGBPixel(pixelRGB, inImage, inPitch, inDeviceFormat, x, y); } } GF_KERNEL RGBCurves_CompositeKernel( GF_PTR(float4) inImage, int inPitch, GF_PTR(float) inSecondaryMask, int inSecondaryPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, GF_OPENCL_CONSTANT(WaveformCurveParameters, inSpline0), GF_OPENCL_CONSTANT(WaveformCurveParameters, inSpline1), GF_OPENCL_CONSTANT(WaveformCurveParameters, inSpline2), GF_OPENCL_CONSTANT(WaveformCurveParameters, inSpline3)) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB pixelRGB = ReadRGBPixel(inImage, inPitch, inDeviceFormat, x, y); float secondaryWeight = inSecondaryMask ? GF_READ2D(inSecondaryMask, inSecondaryPitch, x, y) : 1.0f; WriteRGBPixel( CalculateBGRA( GF_GET_CONSTANT(inSpline0), GF_GET_CONSTANT(inSpline1), GF_GET_CONSTANT(inSpline2), GF_GET_CONSTANT(inSpline3), pixelRGB, secondaryWeight), inImage, inPitch, inDeviceFormat, x, y); } }
