...
#endif
GF_KERNEL VerticalRecursiveGaussianGRAYF32_kernel( GF_PTR(float) pOut, GF_PTR(float) pIn, int width, int height, int pitch_f, float n_plus0, float n_plus1, float neg_d_plus1, float neg_d_plus2, float n_minus1, float n_minus2, float neg_d_minus1, float neg_d_minus2, float inHoldEdge) { int X = KernelX(); if( X > width ) return; int gmem_idx = X; float new_in = pIn[gmem_idx] * inHoldEdge; float prev_in = new_in; float prev_in2 = new_in; float prev_out = new_in; float prev_out2 = new_in; float result; int Y = 0;
#pragma unroll 32
for(;Y < height; Y++) { new_in = pIn[gmem_idx]; result = n_plus0*new_in + n_plus1*prev_in + neg_d_plus1*prev_out + neg_d_plus2*prev_out2; prev_in = new_in; prev_out2 = prev_out; prev_out = result; pOut[gmem_idx] = result; gmem_idx += pitch_f; } new_in = new_in * inHoldEdge; prev_in = new_in; prev_in2 = new_in; prev_out = new_in; prev_out2 = new_in;
#pragma unroll 32
for(;Y > 0; Y--) { gmem_idx -= pitch_f; result = n_minus1*prev_in + n_minus2*prev_in2 + neg_d_minus1*prev_out + neg_d_minus2*prev_out2; prev_in2 = prev_in; prev_in = pIn[gmem_idx]; prev_out2 = prev_out; prev_out = result; pOut[gmem_idx] += result; } }
#define GRAY_TILE_W 32
#define GRAY_SMEM_W (GRAY_TILE_W + 1)
#define GRAY_TILE_H 32
GF_KERNEL HorizontalRecursiveGaussianGrayF32_kernel( GF_PTR(float) pOut, GF_PTR(float) pIn, int width, int height, int pitch_f, float n_plus0, float n_plus1, float neg_d_plus1, float neg_d_plus2, float n_minus1, float n_minus2, float neg_d_minus1, float neg_d_minus2, float inHoldEdge) { GF_SHARED(float) smem[GRAY_SMEM_W * GRAY_TILE_H]; int blockX = 0; int Y = GF_FASTMULTIPLY(BlockID().y, GRAY_TILE_H) + ThreadID().y; int gmem_base = Y * pitch_f; float new_in = ThreadID().y == 0 && inHoldEdge > 0.0f ? pIn[gmem_base] * inHoldEdge : 0.0f; float prev_in = new_in; float prev_in2 = new_in; float prev_out = new_in; float prev_out2 = new_in; float result; while(blockX < width) { int gmem_idx = gmem_base + blockX + ThreadID().x; int rw_smem_idx = GF_FASTMULTIPLY(ThreadID().y, GRAY_SMEM_W) + ThreadID().x;
#pragma unroll 4
for(int i=0;i<4;i++) { if(Y + (i<<3) < height) { if(blockX + ThreadID().x < width) smem[rw_smem_idx] = pIn[gmem_idx]; else smem[rw_smem_idx] = 0; } gmem_idx += (pitch_f << 3); rw_smem_idx += (GRAY_SMEM_W * 8); } SyncThreads(); if(ThreadID().y == 0) { int proc_smem_idx = GF_FASTMULTIPLY(ThreadID().x, GRAY_SMEM_W);
#pragma unroll 32
...
#pragma unroll 4
for(int i=0;i<4;i++) { if(Y + (i<<3) < height) { if(blockX + ThreadID().x < width) pOut[gmem_idx] = smem[rw_smem_idx]; } gmem_idx += (pitch_f << 3); rw_smem_idx += (GRAY_SMEM_W * 8); } SyncThreads(); blockX += GRAY_TILE_W; } new_in = ThreadID().y == 0 && inHoldEdge > 0.0f ? pIn[gmem_base + width - 1] * inHoldEdge : 0.0f; prev_in = new_in; prev_in2 = new_in; prev_out = new_in; prev_out2 = new_in; while(blockX > 0) { blockX -= GRAY_TILE_W; int gmem_idx = gmem_base + blockX + ThreadID().x; int rw_smem_idx = GF_FASTMULTIPLY(ThreadID().y, GRAY_SMEM_W) + ThreadID().x;
#pragma unroll 4
for(int i=0;i<4;i++) { if(Y + (i<<3) < height) { if(blockX + ThreadID().x < width) smem[rw_smem_idx] = pIn[gmem_idx]; else smem[rw_smem_idx] = 0; } gmem_idx += (pitch_f << 3); rw_smem_idx += (GRAY_SMEM_W * 8); } SyncThreads(); if(ThreadID().y == 0) { int proc_smem_idx = GF_FASTMULTIPLY(ThreadID().x, GRAY_SMEM_W) + GRAY_TILE_W -1;
#pragma unroll 32
...
#pragma unroll 4
for(int i=0;i<4;i++) { if(Y + (i<<3) < height) { if(blockX + ThreadID().x < width) pOut[gmem_idx] += smem[rw_smem_idx]; } gmem_idx += (pitch_f << 3); rw_smem_idx += (GRAY_SMEM_W * 8); } SyncThreads(); } }
