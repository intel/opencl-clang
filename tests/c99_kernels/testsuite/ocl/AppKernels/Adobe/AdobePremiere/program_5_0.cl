...
#endif
GF_KERNEL InvertAlphaKernel( GF_PTR(float4) ioImage, int inPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB pixel = ReadRGBPixel(ioImage, inPitch, inDeviceFormat, x, y); pixel.a = 1.0f - saturate(pixel.a); WriteRGBPixel(pixel, ioImage, inPitch, inDeviceFormat, x, y); } }
