...
#endif
GF_CUDA_TEXTURE(float4, inSrcTexture) GF_KERNEL RemoveFlickerKernel( GF_OPENCL_TEXTURE(inSrcTexture), GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB result = GF_READTEXTUREPIXEL(inSrcTexture, x+0.5f, y+0.5f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST); if (y != 0 && y != inHeight - 1) { PixelRGB up = GF_READTEXTUREPIXEL(inSrcTexture, x+0.5f, y-0.5f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST); PixelRGB down = GF_READTEXTUREPIXEL(inSrcTexture, x+0.5f, y+1.5f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST); result = AddPixels(MultiplyPixel(result, 0.5f), MultiplyPixel(AddPixels(up, down), 0.25f)); } WriteRGBPixel(result, outDest, inDestPitch, inDeviceFormat, x, y); } }
