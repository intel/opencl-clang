...
#endif
GF_CUDA_TEXTURE(float4, inSrcTexture) GF_KERNEL SharpenKernel( GF_OPENCL_TEXTURE(inSrcTexture), GF_PTR(float4) outDest, int inPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, float inCenterCoeff, float inSurroundCoeff) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB up = GF_READTEXTUREPIXEL(inSrcTexture, x+0.5f, y-0.5f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST); PixelRGB left = GF_READTEXTUREPIXEL(inSrcTexture, x-0.5f, y+0.5f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST); PixelRGB center = GF_READTEXTUREPIXEL(inSrcTexture, x+0.5f, y+0.5f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST); PixelRGB right = GF_READTEXTUREPIXEL(inSrcTexture, x+1.5f, y+0.5f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST); PixelRGB down = GF_READTEXTUREPIXEL(inSrcTexture, x+0.5f, y+1.5f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST); PixelRGB surround = AddPixels(AddPixels(AddPixels(up, left), right), down); surround = MultiplyPixel(surround, inSurroundCoeff); center = MultiplyPixel(center, inCenterCoeff); PixelRGB result = AddPixels(center, surround); result = DividePixel(result, inCenterCoeff + 4*inSurroundCoeff); WriteRGBPixel(result, outDest, inPitch, inDeviceFormat, x, y); } }
