...
#endif
GF_DEVICE_FUNCTION int ClampInt(int x, int y, int z) { return min(max(x, y), z); } GF_KERNEL InterlevedToPlanar( GF_PTR(float) dest, const GF_PTR(float4) src, int pitch_in_pixels, int width, int height) { int X = KernelX(); int Y = KernelY(); if (Y < height && X < width) { int gmem_idx = Y * pitch_in_pixels + X; float4 pix = src[gmem_idx]; pix.w = saturate(pix.w); dest[gmem_idx] = ToLinearColor(pix.x)*pix.w; int plane_size = pitch_in_pixels * height; gmem_idx += plane_size; dest[gmem_idx] = ToLinearColor(pix.y)*pix.w; gmem_idx += plane_size; dest[gmem_idx] = ToLinearColor(pix.z)*pix.w; gmem_idx += plane_size; dest[gmem_idx] = pix.w; } } GF_KERNEL HorizConvKernel_planar_out( GF_PTR(float) dest, const GF_PTR(float4) src, int pitch_in_pixels, int width, int height, float multiplier, int radius, GF_SHARED(float*) smem, GF_CONSTANT_PTR(float) LPFCoefficients) { int smem_width = BlockSize().x + (2*radius); int smem_width2 = smem_width<<1; int smem_width3 = smem_width2 + smem_width; int X = KernelX(); int gmem_Y_idx = BlockID().y * pitch_in_pixels ; float4 pix = src[gmem_Y_idx + ClampInt(X-radius,0,width-1)]; pix.w = saturate(pix.w); int smem_idx = ThreadID().x; smem[smem_idx] = ToLinearColor(pix.x)*pix.w; smem[smem_idx + smem_width] = ToLinearColor(pix.y)*pix.w; smem[smem_idx + smem_width2] = ToLinearColor(pix.z)*pix.w; smem[smem_idx + smem_width3] = pix.w; if(ThreadID().x < (radius<<1)) { pix = src[gmem_Y_idx + min(X-radius+BlockSize().x,width-1)]; pix.w = saturate(pix.w); smem_idx += BlockSize().x; smem[smem_idx] = ToLinearColor(pix.x)*pix.w; smem[smem_idx + smem_width] = ToLinearColor(pix.y)*pix.w; smem[smem_idx + smem_width2] = ToLinearColor(pix.z)*pix.w; smem[smem_idx + smem_width3] = pix.w; } SyncThreads(); if(X < width) { pix = make_float4(0,0,0,0); smem_idx = ThreadID().x; for (int i = 0; i < radius*2+1; ++i) { pix.x += smem[smem_idx] * LPFCoefficients[i]; pix.y += smem[smem_idx + smem_width] * LPFCoefficients[i]; pix.z += smem[smem_idx + smem_width2] * LPFCoefficients[i]; pix.w += smem[smem_idx + smem_width3] * LPFCoefficients[i]; smem_idx++; } pix.x *= multiplier; pix.y *= multiplier; pix.z *= multiplier; pix.w *= multiplier; int gmem_idx = BlockID().y * pitch_in_pixels + X; dest[gmem_idx] = pix.x; int plane_size = pitch_in_pixels * height; gmem_idx += plane_size; dest[gmem_idx] = pix.y; gmem_idx += plane_size; dest[gmem_idx] = pix.z; gmem_idx += plane_size; dest[gmem_idx] = pix.w; } } GF_KERNEL HorizConvKernel( GF_PTR(float4) dest, const GF_PTR(float4) src, int pitch_in_pixels, int width, int height, float multiplier, int radius, GF_SHARED(float*) smem, GF_CONSTANT_PTR(float) LPFCoefficients) { int smem_width = BlockSize().x + (2*radius); int smem_width2 = smem_width<<1; int smem_width3 = smem_width2 + smem_width; int X = KernelX(); int gmem_Y_idx = BlockID().y * pitch_in_pixels ; float4 pix = src[gmem_Y_idx + ClampInt(X-radius,0,width-1)]; pix.w = saturate(pix.w); int smem_idx = ThreadID().x; smem[smem_idx] = ToLinearColor(pix.x)*pix.w; smem[smem_idx + smem_width] = ToLinearColor(pix.y)*pix.w; smem[smem_idx + smem_width2] = ToLinearColor(pix.z)*pix.w; smem[smem_idx + smem_width3] = pix.w; if(ThreadID().x < (radius<<1)) { pix = src[gmem_Y_idx + min(X-radius+BlockSize().x,width-1)]; pix.w = saturate(pix.w); smem_idx += BlockSize().x; smem[smem_idx] = ToLinearColor(pix.x)*pix.w; smem[smem_idx + smem_width] = ToLinearColor(pix.y)*pix.w; smem[smem_idx + smem_width2] = ToLinearColor(pix.z)*pix.w; smem[smem_idx + smem_width3] = pix.w; } SyncThreads(); if(X < width) { pix = make_float4(0,0,0,0); smem_idx = ThreadID().x; for (int i = 0; i < radius*2+1; ++i) { pix.x += smem[smem_idx] * LPFCoefficients[i]; pix.y += smem[smem_idx + smem_width] * LPFCoefficients[i]; pix.z += smem[smem_idx + smem_width2] * LPFCoefficients[i]; pix.w += smem[smem_idx + smem_width3] * LPFCoefficients[i]; ++smem_idx; } pix.x *= multiplier; pix.y *= multiplier; pix.z *= multiplier; pix.w *= multiplier; dest[gmem_Y_idx + X] = pix; } } GF_KERNEL VertConvKernel_planar_in( GF_PTR(float4) dest, const GF_PTR(float) src, int pitch_in_pixels, int width, int height, float Multiplier, int radius, GF_SHARED(float*) smem, GF_CONSTANT_PTR(float) LPFCoefficients) { int X = KernelX(); int Y = BlockID().y*64 + ThreadID().y; int gmem_idx; float4 pix1, pix2, pix3, pix4; int planeOffset = 0; int row = ThreadID().y; if (X < width) { while(row < (64 + (radius<<1))) { gmem_idx = ClampInt((int)BlockID().y*64 + row - radius, 0, height-1) * pitch_in_pixels + X; smem[row*16 + ThreadID().x] = src[gmem_idx]; row+=16; }} SyncThreads(); if(X < width && Y < height) { float val = 0; int smemIdx = (ThreadID().y)*16 + ThreadID().x; int coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix1.x = val * Multiplier; if(Y+16 < height) { val = 0; smemIdx = (ThreadID().y+16)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix2.x = val * Multiplier; if(Y+32 < height) { val = 0; smemIdx = (ThreadID().y+32)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix3.x = val * Multiplier; if(Y+48 < height) { val = 0; smemIdx = (ThreadID().y+48)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix4.x = val * Multiplier; }}}} SyncThreads(); planeOffset = height*pitch_in_pixels; row = ThreadID().y; if (X < width) { while(row < (64 + (radius<<1))) { gmem_idx = planeOffset + ClampInt((int)BlockID().y*64 + row - radius, 0, height-1) * pitch_in_pixels + X; smem[row*16 + ThreadID().x] = src[gmem_idx]; row+=16; }} SyncThreads(); if(X < width && Y < height) { float val = 0; int smemIdx = (ThreadID().y)*16 + ThreadID().x; int coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix1.y = val * Multiplier; if(Y+16 < height) { val = 0; smemIdx = (ThreadID().y+16)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix2.y = val * Multiplier; if(Y+32 < height) { val = 0; smemIdx = (ThreadID().y+32)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix3.y = val * Multiplier; if(Y+48 < height) { val = 0; smemIdx = (ThreadID().y+48)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix4.y = val * Multiplier; }}}} SyncThreads(); planeOffset *= 2; row = ThreadID().y; if (X < width) { while(row < (64 + (radius<<1))) { gmem_idx = planeOffset + ClampInt((int)BlockID().y*64 + row - radius, 0, height-1) * pitch_in_pixels + X; smem[row*16 + ThreadID().x] = src[gmem_idx]; row+=16; }} SyncThreads(); if(X < width && Y < height) { float val = 0; int smemIdx = (ThreadID().y)*16 + ThreadID().x; int coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix1.z = val * Multiplier; if(Y+16 < height) { val = 0; smemIdx = (ThreadID().y+16)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix2.z = val * Multiplier; if(Y+32 < height) { val = 0; smemIdx = (ThreadID().y+32)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix3.z = val * Multiplier; if(Y+48 < height) { val = 0; smemIdx = (ThreadID().y+48)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix4.z = val * Multiplier; }}}} SyncThreads(); planeOffset += height * pitch_in_pixels; row = ThreadID().y; if (X < width) { while(row < (64 + (radius<<1))) { gmem_idx = planeOffset + ClampInt((int)BlockID().y*64 + row - radius, 0, height-1) * pitch_in_pixels + X; smem[row*16 + ThreadID().x] = src[gmem_idx]; row+=16; }} SyncThreads(); if(X < width && Y < height) { float val = 0; int smemIdx = (ThreadID().y)*16 + ThreadID().x; int coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix1.w = val * Multiplier; if(Y+16 < height) { val = 0; smemIdx = (ThreadID().y+16)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix2.w = val * Multiplier; if(Y+32 < height) { val = 0; smemIdx = (ThreadID().y+32)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix3.w = val * Multiplier; if(Y+48 < height) { val = 0; smemIdx = (ThreadID().y+48)*16 + ThreadID().x; coeff = 128; for (int i = 0; i < radius*2+1; ++i) { val += smem[smemIdx] * LPFCoefficients[coeff++]; smemIdx += 16; } pix4.w = val * Multiplier; }}}} SyncThreads(); if(Y < height && X < width) { gmem_idx = Y*pitch_in_pixels + X; dest[gmem_idx] = pix1; if(Y+16 < height) { gmem_idx += pitch_in_pixels*16; dest[gmem_idx] = pix2; if(Y+32 < height) { gmem_idx += pitch_in_pixels*16; dest[gmem_idx] = pix3; if(Y+48 < height) { gmem_idx += pitch_in_pixels*16; dest[gmem_idx] = pix4; } } } } }
