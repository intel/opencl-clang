...
#ifndef NOISE_H
#define NOISE_H
GF_DEVICE_FUNCTION void Noise( PixelRGB* ioPixel, int* ioX, int* ioY, int inWidth, int inHeight, GF_CONSTANT_PTR(NoiseParameters) inParameters) { float randR; float randG; float randB; if (inParameters->mUseColorNoise == 1.0f) { randR = ((float)Rand2DSeed(*ioX*3+0, *ioY, inParameters->mRandSeed) / kRandMax) * inParameters->mAmount - inParameters->mAmount*0.5f; randG = ((float)Rand2DSeed(*ioX*3+1, *ioY, inParameters->mRandSeed) / kRandMax) * inParameters->mAmount - inParameters->mAmount*0.5f; randB = ((float)Rand2DSeed(*ioX*3+2, *ioY, inParameters->mRandSeed) / kRandMax) * inParameters->mAmount - inParameters->mAmount*0.5f; } else { randR = randG = randB = ((float)Rand2DSeed(*ioX, *ioY, inParameters->mRandSeed) / kRandMax) * inParameters->mAmount - inParameters->mAmount*0.5f; } ioPixel->r += randR; ioPixel->g += randG; ioPixel->b += randB; if (inParameters->mClipResult == 1.0f) { *ioPixel = ClampPixel(*ioPixel, 0.0f, 1.0f); } }
#endif
