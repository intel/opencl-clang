...
#endif
GF_CUDA_TEXTURE(float4, inSrcTexture) GF_KERNEL ShearHorizontalKernel( GF_OPENCL_TEXTURE(inSrcTexture), DevicePixelFormat inDeviceFormat, GF_PTR(float4) inOutput, int inOutputWidth, int inOutputHeight, int inOutputPitch, int inInputWidth, int inInputHeight, float inShearAmount, int inOffset) { int x_out = KernelX(); int y_out = KernelY(); float x_in = x_out - y_out * inShearAmount - inOffset; float y_in = y_out; if (x_out < inOutputWidth && y_out < inOutputHeight) { PixelRGB pixel; if (x_in >= 0.0f && x_in <= inInputWidth-1) { pixel = GF_READTEXTUREPIXEL(inSrcTexture, x_in + 0.f, y_in + 0.f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_LINEAR); } else { pixel = BGRA(0,0,0,0); } WriteRGBPixel( pixel, inOutput, inOutputPitch, inDeviceFormat, x_out, y_out); } } GF_KERNEL ShearVerticalKernel( GF_OPENCL_TEXTURE(inSrcTexture), DevicePixelFormat inDeviceFormat, GF_PTR(float4) inOutput, int inOutputWidth, int inOutputHeight, int inOutputPitch, int inInputWidth, int inInputHeight, float inShearAmount, int inOffset) { int x_out = KernelX(); int y_out = KernelY(); float y_in = y_out - x_out * inShearAmount - inOffset; float x_in = x_out; if (x_out < inOutputWidth && y_out < inOutputHeight) { PixelRGB pixel; if (y_in >= 0.0f && y_in <= inInputHeight-1) { pixel = GF_READTEXTUREPIXEL(inSrcTexture, x_in + 0.5f, y_in + 0.5f, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_LINEAR); } else { pixel = BGRA(0,0,0,0); } WriteRGBPixel( pixel, inOutput, inOutputPitch, inDeviceFormat, x_out, y_out); } }
