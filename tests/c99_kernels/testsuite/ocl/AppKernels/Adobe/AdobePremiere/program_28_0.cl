...
#endif
GF_KERNEL AdditiveDissolveKernel( const GF_PTR(float4) inSrc0, int inSrcPitch0, const GF_PTR(float4) inSrc1, int inSrcPitch1, GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, float inDissolve) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB pixel0 = ReadRGBPixel(inSrc0, inSrcPitch0, inDeviceFormat, x, y); PixelRGB pixel1 = ReadRGBPixel(inSrc1, inSrcPitch1, inDeviceFormat, x, y); PixelRGB result; if (inDissolve <= 1.0f) { result.b = saturate(pixel0.b + pixel1.b*inDissolve); result.g = saturate(pixel0.g + pixel1.g*inDissolve); result.r = saturate(pixel0.r + pixel1.r*inDissolve); result.a = saturate(pixel0.a*(1.0f-inDissolve) + (pixel0.a*pixel1.a*inDissolve)); } else { result.b = saturate(pixel0.b*(2.0f-inDissolve) + pixel1.b); result.g = saturate(pixel0.g*(2.0f-inDissolve) + pixel1.g); result.r = saturate(pixel0.r*(2.0f-inDissolve) + pixel1.r); result.a = saturate(pixel0.a*pixel1.a*(2.0f-inDissolve) + pixel1.a*(inDissolve-1.0f)); } WriteRGBPixel(result, outDest, inDestPitch, inDeviceFormat, x, y); } }
