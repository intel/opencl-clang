...
#endif
GF_KERNEL cuda_kernel_timecode(const GF_PTR(uchar4) srcFrame, int srcX, int srcY, GF_PTR(float4) dstFrame, int width, int height, int destX, int destY, int srcPitch, int dstPitch, float t, DevicePixelFormat inDeviceFormat) { float sc, sa, da, ra, f; int x = KernelX(); int y = KernelY(); if (x >= width || y >= height) return; uchar4 timecodePixel = GF_READ2D(srcFrame, srcPitch, x + srcX, y + srcY); PixelRGB dstPixel = ReadRGBPixel(dstFrame, dstPitch, inDeviceFormat, x+destX, y+destY); sc = (float)timecodePixel.x/255.0f; sa = sc + t*(1.0f-sc); da = saturate(dstPixel.a) * (1.0f-sa); ra = sa + da; f = ra != 0.0f ? 1.0f / ra : 0.0f; dstPixel.b = (sc + dstPixel.b*da)*f; dstPixel.g = (sc + dstPixel.g*da)*f; dstPixel.r = (sc + dstPixel.r*da)*f; dstPixel.a = ra; WriteRGBPixel(dstPixel, dstFrame, dstPitch, inDeviceFormat, x+destX, y+destY); }
