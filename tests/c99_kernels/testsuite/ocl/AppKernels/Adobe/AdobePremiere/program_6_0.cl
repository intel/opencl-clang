...
#endif
GF_KERNEL AdjustmentLayerKernel( const GF_PTR(float4) inAdjustment, int inAdjustmentPitch, const GF_PTR(float4) inAlpha, int inAlphaPitch, GF_PTR(float4) ioDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, float inAlphaGain, BlendMode inBlendMode) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB composite = ReadRGBPixel(ioDest, inDestPitch, inDeviceFormat, x, y); PixelRGB adjustment = ReadRGBPixel(inAdjustment, inAdjustmentPitch, inDeviceFormat, x, y); float alpha = inAlpha ? ReadRGBPixel(inAlpha, inAlphaPitch, inDeviceFormat, x, y).a : 1.0f; PixelRGB dest; if (inBlendMode != kBlendMode_Normal) { dest = CompositePixels(adjustment, composite, inWidth, inHeight, inAlphaGain, inBlendMode, 0); } else { alpha *= inAlphaGain; dest = adjustment; } if (fIsLessThanOrEqual(adjustment.a, 0.0f)) { dest.b = composite.b; dest.g = composite.g; dest.r = composite.r; dest.a = composite.a * (1.0f - alpha); } else if (fIsLessThanOrEqual(composite.a, 0.0f)) { dest.a = adjustment.a * alpha; } else { float alpha1 = adjustment.a * alpha; float alpha2 = composite.a * (1.0f - alpha); float alphaSum = alpha1 + alpha2; if (fIsLessThanOrEqual(alphaSum, 0.0f)) { dest.b = 0.0f; dest.g = 0.0f; dest.r = 0.0f; dest.a = 0.0f; } else { dest.b = (dest.b * alpha1 + composite.b * alpha2) / alphaSum; dest.g = (dest.g * alpha1 + composite.g * alpha2) / alphaSum; dest.r = (dest.r * alpha1 + composite.r * alpha2) / alphaSum; dest.a = alphaSum; } } WriteRGBPixel(dest, ioDest, inDestPitch, inDeviceFormat, x, y); } }
