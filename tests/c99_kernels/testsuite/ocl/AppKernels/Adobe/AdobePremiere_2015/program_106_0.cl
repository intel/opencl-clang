...
#endif
GF_DEVICE_FUNCTION float MedianF( float inT0, float inT1, float inT2) { float min01 = min( inT0, inT1 ); float max01 = max( inT0, inT1 ); float med = max( min01, inT2 ); med = min( max01, med ); return med; } GF_DEVICE_FUNCTION float4 MedianF4( float4 inP0, float4 inP1, float4 inP2) { float4 result = { MedianF(inP0.x, inP1.x, inP2.x), MedianF(inP0.y, inP1.y, inP2.y), MedianF(inP0.z, inP1.z, inP2.z), MedianF(inP0.w, inP1.w, inP2.w), }; return result; } GF_DEVICE_FUNCTION float4 AverageF4( float4 inP0, float4 inP1) { float4 result = { (inP0.x + inP1.x) * 0.5f, (inP0.y + inP1.y) * 0.5f, (inP0.z + inP1.z) * 0.5f, (inP0.w + inP1.w) * 0.5f, }; return result; } GF_DEVICE_FUNCTION float PixelDifferenceSquaredF4( float4 inP0, float4 inP1) { float4 result = { (inP0.x - inP1.x), (inP0.y - inP1.y), (inP0.z - inP1.z), (0.0f), }; return result.z*result.z + result.y*result.y + result.x*result.x; } GF_DEVICE_FUNCTION float4 PremultiplyPixelF4( float4 inPixel) { float4 result = inPixel; result.x *= inPixel.w; result.y *= inPixel.w; result.z *= inPixel.w; return result; } GF_KERNEL DeinterlaceSharedKernel( GF_PTR(float4) ioBuf, int inPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, int inUpperField) { GF_SHARED(float4) blockArea[16+1][1+16+1]; int x = KernelX(); int y = KernelY(); int xMax = inWidth-1; int yMax = inHeight-1; y <<= 1; if (inUpperField) { y += 1; } int2 blocksize = BlockSize(); int2 blockPos = ThreadID(); int xin, yin, xin2, yin2; xin = clamp( x-1, 0, xMax ); yin = clamp( y-1, 0, yMax ); blockArea[blockPos.y][blockPos.x] = PremultiplyPixelF4( ReadPixel(ioBuf, inPitch, inDeviceFormat, xin, yin)); xin2 = min( x-1 + blocksize.x, xMax ); yin2 = min( y-1 + 2*blocksize.y, yMax ); if( blockPos.y == 0 ) { blockArea[blocksize.y][blockPos.x] = PremultiplyPixelF4( ReadPixel(ioBuf, inPitch, inDeviceFormat, xin, yin2)); } if( blockPos.x < 2 ) { blockArea[blockPos.y][blockPos.x + blocksize.x] = PremultiplyPixelF4( ReadPixel(ioBuf, inPitch, inDeviceFormat, xin2, yin)); if( blockPos.y == 0 ) { blockArea[blocksize.y][blockPos.x + blocksize.x] = PremultiplyPixelF4( ReadPixel(ioBuf, inPitch, inDeviceFormat, xin2, yin2)); } } SyncThreads(); int xb = blockPos.x; int yb = blockPos.y; if ( (x < inWidth) && (y < inHeight ) ) { if (0 < y && y+1 < inHeight && 0 < x && x+1 < inWidth) { float4 upLeft = blockArea[yb ][xb ]; float4 upCenter = blockArea[yb ][xb+1]; float4 upRight = blockArea[yb ][xb+2]; float4 downLeft = blockArea[yb+1][xb ]; float4 downCenter = blockArea[yb+1][xb+1]; float4 downRight = blockArea[yb+1][xb+2]; float dist0 = PixelDifferenceSquaredF4(upLeft , downRight); float dist1 = PixelDifferenceSquaredF4(upCenter, downCenter); float dist2 = PixelDifferenceSquaredF4(upRight , downLeft); float4 deintValue; if (dist0 < dist1 && dist0 < dist2) { deintValue = MedianF4(upCenter, downCenter, AverageF4(upLeft, downRight)); } else if (dist1 < dist2) { deintValue = AverageF4(upCenter, downCenter); } else { deintValue = MedianF4(upCenter, downCenter, AverageF4(upRight, downLeft)); } PixelRGB result = ConvertComponents_To_RGB( deintValue ); WriteRGBPixel( UnpremultiplyPixel(result), ioBuf, inPitch, inDeviceFormat, x, y ); } else if (0 < y && y+1 < inHeight) { float4 upCenter = blockArea[yb ][xb+1]; float4 downCenter = blockArea[yb+1][xb+1]; float4 deintValue = AverageF4( upCenter, downCenter ); PixelRGB result = ConvertComponents_To_RGB( deintValue ); WriteRGBPixel( UnpremultiplyPixel(result), ioBuf, inPitch, inDeviceFormat, x, y ); } else if (!inUpperField && y == 0) { PixelRGB result = ConvertComponents_To_RGB( ReadPixel(ioBuf, inPitch, inDeviceFormat, x, y+1)); WriteRGBPixel( result, ioBuf, inPitch, inDeviceFormat, x, y); } else if (inUpperField && y == inHeight-1) { PixelRGB result = ConvertComponents_To_RGB( ReadPixel(ioBuf, inPitch, inDeviceFormat, x, y-1)); WriteRGBPixel( result, ioBuf, inPitch, inDeviceFormat, x, y); } } }
