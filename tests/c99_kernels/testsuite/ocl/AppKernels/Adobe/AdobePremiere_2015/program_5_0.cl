...
#endif
GF_DEVICE_FUNCTION PixelRGB PixelLERP(PixelRGB inPixelA, PixelRGB inPixelB, float inInterp) { PixelRGB result = { LERP(inPixelA.b, inPixelB.b, inInterp), LERP(inPixelA.g, inPixelB.g, inInterp), LERP(inPixelA.r, inPixelB.r, inInterp), LERP(inPixelA.a, inPixelB.a, inInterp), }; return result; } GF_KERNEL WipeKernel( const GF_PTR(float4) inSrc0, int inSrcPitch0, const GF_PTR(float4) inSrc1, int inSrcPitch1, GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inSrcX, int inSrcY, int inDestX, int inDestY, int inDestWidth, int inDestHeight, float inBborderRed, float inBborderGreen, float inBorderBlue, float inAlpha0, float inAlpha1, float inAlphaDx, float inAlphaDy ) { int x = KernelX() + inDestX; int y = KernelY() + inDestY; if ( inDestX <= x && x < inDestX + inDestWidth && inDestY <= y && y < inDestY + inDestHeight) { int readX = inSrcX + x - inDestX; int readY = inSrcY + y - inDestY; float deltaAlpha = x * inAlphaDx + y * inAlphaDy; float alpha0 = inAlpha0 + deltaAlpha; float alpha1 = inAlpha1 + deltaAlpha; PixelRGB color; if (alpha1 >= 1.0f) { color = ReadRGBPixel(inSrc1, inSrcPitch1, inDeviceFormat, readX, readY); } else if (alpha1 <= 0.0f && alpha0 <= 0.0f) { color = ReadRGBPixel(inSrc0, inSrcPitch0, inDeviceFormat, readX, readY); } else { PixelRGB border = {inBborderRed, inBborderGreen, inBorderBlue, 1.0}; PixelRGB color0 = ReadRGBPixel(inSrc0, inSrcPitch0, inDeviceFormat, readX, readY); PixelRGB color1 = ReadRGBPixel(inSrc1, inSrcPitch1, inDeviceFormat, readX, readY); PixelRGB color0WithBorder = PixelLERP(color0, border, clamp(alpha0, 0.0f, 1.0f)); color = PixelLERP(color0WithBorder, color1, clamp(alpha1, 0.0f, 1.0f)); } WriteRGBPixel(color, outDest, inDestPitch, inDeviceFormat, x, y); } }
