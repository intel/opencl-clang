...
#endif
GF_KERNEL TransparencyGridKernel( GF_PTR(float4) inImage, int inPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, int inCellSize) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { int kCellSize = 8; float4 kEvenCellColor = make_float4(1.0f, 1.0f, 1.0f, 1.0f); float4 kOddCellColor = make_float4(0.8f, 0.8f, 0.8f, 1.0f); int even = ((x/inCellSize + y/inCellSize) % 2) == 0; WritePixel(even ? kEvenCellColor : kOddCellColor, inImage, inPitch, inDeviceFormat, x, y); } }
