...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE(float4, texture1_RECT) GF_KERNEL ShaderKernel_Convolve(COMMON_PARAMS) { BEGIN_NORMAL LOAD_PARAMS(4) PARAM (center, program_local[0]) PARAM (side , program_local[1]) PARAM (corner, program_local[2]) PARAM (invSum, program_local[3]) TEMP r0, r1, r2, r3, r4; ADD (&r2, s(fragment_texcoord0, xyxy), co_xyxy(-1, -1, 0, 0)); ADD (&r3, s(fragment_texcoord0, xyxy), co_xyxy( 1, 1, 0, 0)); ADD (&r4, s(fragment_texcoord0, xyxy), co_xyxy(-1, -1, 1, 1)); TEX (&r0, fragment_texcoord0, texture0_RECT); MUL (&r1, r0, center); TEX (&r0, r2, texture0_RECT); MAD (&r1, r0, corner, r1); TEX (&r0, s(r4, zyzy), texture0_RECT); MAD (&r1, r0, corner, r1); TEX (&r0, s(r4, xwxw), texture0_RECT); MAD (&r1, r0, corner, r1); TEX (&r0, s(r4, zwzw), texture0_RECT); MAD (&r1, r0, corner, r1); TEX (&r0, s(r2, xwxw), texture0_RECT); MAD (&r1, r0, side, r1); TEX (&r0, s(r3, xwxw), texture0_RECT); MAD (&r1, r0, side, r1); TEX (&r0, s(r2, zyzy), texture0_RECT); MAD (&r1, r0, side, r1); TEX (&r0, s(r3, zyzy), texture0_RECT); MAD (&r1, r0, side, r1); MUL (&result_color, r1, invSum); END }
