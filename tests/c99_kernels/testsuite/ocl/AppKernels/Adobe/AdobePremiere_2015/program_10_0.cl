...
#endif
GF_DEVICE_FUNCTION float CalculateThreeWayLuma( float inY, float inSecondaryWeight, float inMasterInputBlack, float inMasterInputWhite, float inMasterInputGray, float inMasterOutputBlack, float inMasterOutputWhite, int inEnableHalfMasterBlend) { float masterInputRange = inMasterInputWhite - inMasterInputBlack; float masterOutputRange = inMasterOutputWhite - inMasterOutputBlack; float threeWayY = inY; if (masterInputRange != 1.0f || masterOutputRange != 1.0f) { const float randPlusMinusOne = fdividef(Rand2D(), fdividef(kRandMax, 2.0f)) - 1.0f; const float k8BitBucketSize = 1.3f / 255.0f; threeWayY += randPlusMinusOne * (k8BitBucketSize * 0.5f); } float masterY = inMasterOutputBlack + masterOutputRange * Power( max(0.0f, fdividef(threeWayY - inMasterInputBlack, masterInputRange)), inMasterInputGray); if (inEnableHalfMasterBlend) { threeWayY = LERP(masterY, threeWayY, 0.5f); threeWayY = LERP(inY, threeWayY, inSecondaryWeight); } else { threeWayY = LERP(inY, masterY, inSecondaryWeight); } return threeWayY; } GF_KERNEL ThreeWayColorCorrector_MaskKernel( GF_PTR(float4) inImage, int inPitch, const GF_PTR(float) inSecondaryMask, int inSecondaryPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelYUV pixel = ConvertRGB_To_YUV(ReadRGBPixel(inImage, inPitch, inDeviceFormat, x, y)); float secondaryWeight = inSecondaryMask ? GF_READ2D(inSecondaryMask, inSecondaryPitch, x, y) : 1.0f; pixel.y = secondaryWeight; pixel.u = pixel.v = 0.0f; WriteRGBPixel(ConvertYUV_To_RGB(pixel), inImage, inPitch, inDeviceFormat, x, y); } } GF_KERNEL ThreeWayColorCorrector_TonalRangeKernel( GF_PTR(float4) inImage, int inPitch, const GF_PTR(float) inSecondaryMask, int inSecondaryPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, float inShadowsThreshold, float inShadowsSoftness, float inHighlightsThreshold, float inHighlightsSoftness) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelYUV pixel = ConvertRGB_To_YUV(ReadRGBPixel(inImage, inPitch, inDeviceFormat, x, y)); float shadowSoftness = min(1.0f, inShadowsThreshold + inShadowsSoftness); float highlightSoftness = max(0.0f, inHighlightsThreshold - inHighlightsSoftness); if (pixel.y <= inShadowsThreshold || (pixel.y <= shadowSoftness && inShadowsSoftness == 0)) { pixel.y = 0.0f; } else if (pixel.y >= inHighlightsThreshold || (pixel.y >= highlightSoftness && inHighlightsSoftness == 0)) { pixel.y = 1.0f; } else if (pixel.y > shadowSoftness && pixel.y < highlightSoftness) { pixel.y = 0.5f; } else if (pixel.y > inShadowsThreshold && pixel.y < shadowSoftness) { float shadowExcursion = shadowSoftness - inShadowsThreshold; float grayT = fdividef(pixel.y - inShadowsThreshold, shadowExcursion); pixel.y = grayT * 0.5f; } else if (pixel.y > highlightSoftness) { float highlightExcursion = inHighlightsThreshold - highlightSoftness; float grayT = fdividef(pixel.y - highlightSoftness, highlightExcursion); pixel.y = LERP(0.5f, 1.0f, grayT); } else { pixel.y = 0.5f; } pixel.u = pixel.v = 0.0f; WriteRGBPixel(ConvertYUV_To_RGB(pixel), inImage, inPitch, inDeviceFormat, x, y); } } GF_KERNEL ThreeWayColorCorrector_LumaKernel( GF_PTR(float4) inImage, int inPitch, const GF_PTR(float) inSecondaryMask, int inSecondaryPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, float inMasterInputBlack, float inMasterInputWhite, float inMasterInputGray, float inMasterOutputBlack, float inMasterOutputWhite, int inEnableHalfMasterBlend) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelYUV pixel = ConvertRGB_To_YUV(ReadRGBPixel(inImage, inPitch, inDeviceFormat, x, y)); float secondaryWeight = inSecondaryMask ? GF_READ2D(inSecondaryMask, inSecondaryPitch, x, y) : 1.0f; pixel.y = CalculateThreeWayLuma( pixel.y, secondaryWeight, inMasterInputBlack, inMasterInputWhite, inMasterInputGray, inMasterOutputBlack, inMasterOutputWhite, inEnableHalfMasterBlend); pixel.u = pixel.v = 0.0f; WriteRGBPixel(ConvertYUV_To_RGB(pixel), inImage, inPitch, inDeviceFormat, x, y); } } GF_KERNEL ThreeWayColorCorrector_CompositeKernel( GF_PTR(float4) inImage, int inPitch, const GF_PTR(float) inSecondaryMask, int inSecondaryPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, float inShadowsThreshold, float inShadowsSoftness, float inHighlightsThreshold, float inHighlightsSoftness, float inMasterSaturation, float inShadowSaturation, float inMidtoneSaturation, float inHighlightSaturation, float inShadowHueRadians, float inMidtoneHueRadians, float inHighlightHueRadians, float inMasterMaxBalanceX, float inMasterMaxBalanceY, float inMasterMagnitude, float inShadowMaxBalanceX, float inShadowMaxBalanceY, float inShadowMagnitude, float inMidtoneMaxBalanceX, float inMidtoneMaxBalanceY, float inMidtoneMagnitude, float inHighlightMaxBalanceX, float inHighlightMaxBalanceY, float inHighlightMagnitude, float inMasterInputBlack, float inMasterInputWhite, float inMasterInputGray, float inMasterOutputBlack, float inMasterOutputWhite, int inEnableHalfMasterBlend) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelYUV pixel = ConvertRGB_To_YUV(ReadRGBPixel(inImage, inPitch, inDeviceFormat, x, y)); float secondaryWeight = inSecondaryMask ? GF_READ2D(inSecondaryMask, inSecondaryPitch, x, y) : 1.0f; float originalU = pixel.u; float originalV = pixel.v; float shadowSoftness = min(1.0f, inShadowsThreshold + inShadowsSoftness); float highlightSoftness = max(0.0f, inHighlightsThreshold - inHighlightsSoftness); float shadowCosHue = cosf(inShadowHueRadians); float shadowSinHue = sinf(inShadowHueRadians); float midtoneCosHue = cosf(inMidtoneHueRadians); float midtoneSinHue = sinf(inMidtoneHueRadians); float highlightCosHue = cosf(inHighlightHueRadians); float highlightSinHue = sinf(inHighlightHueRadians); float shadowNewU = pixel.u * shadowCosHue - pixel.v * shadowSinHue; float shadowNewV = pixel.v * shadowCosHue + pixel.u * shadowSinHue; float midtoneNewU = pixel.u * midtoneCosHue - pixel.v * midtoneSinHue; float midtoneNewV = pixel.v * midtoneCosHue + pixel.u * midtoneSinHue; float highlightNewU = pixel.u * highlightCosHue - pixel.v * highlightSinHue; float highlightNewV = pixel.v * highlightCosHue + pixel.u * highlightSinHue; float masterNewU = LERP(pixel.u, inMasterMaxBalanceX, inMasterMagnitude); float masterNewV = LERP(pixel.v, inMasterMaxBalanceY, inMasterMagnitude); shadowNewU = LERP(shadowNewU, inShadowMaxBalanceX, inShadowMagnitude); shadowNewV = LERP(shadowNewV, inShadowMaxBalanceY, inShadowMagnitude); midtoneNewU = LERP(midtoneNewU, inMidtoneMaxBalanceX, inMidtoneMagnitude); midtoneNewV = LERP(midtoneNewV, inMidtoneMaxBalanceY, inMidtoneMagnitude); highlightNewU = LERP(highlightNewU, inHighlightMaxBalanceX, inHighlightMagnitude); highlightNewV = LERP(highlightNewV, inHighlightMaxBalanceY, inHighlightMagnitude); masterNewU *= inMasterSaturation; masterNewV *= inMasterSaturation; shadowNewU *= inShadowSaturation; shadowNewV *= inShadowSaturation; midtoneNewU *= inMidtoneSaturation; midtoneNewV *= inMidtoneSaturation; highlightNewU *= inHighlightSaturation; highlightNewV *= inHighlightSaturation; if (pixel.y <= inShadowsThreshold || (pixel.y <= shadowSoftness && inShadowsSoftness == 0)) { pixel.u = shadowNewU; pixel.v = shadowNewV; } else if ((pixel.y >= inHighlightsThreshold) || (pixel.y >= highlightSoftness && inHighlightsSoftness == 0)) { pixel.u = highlightNewU; pixel.v = highlightNewV; } else if (pixel.y > shadowSoftness && pixel.y < highlightSoftness) { pixel.u = midtoneNewU; pixel.v = midtoneNewV; } else if (pixel.y > inShadowsThreshold && pixel.y < shadowSoftness) { float shadowExcursion = shadowSoftness - inShadowsThreshold; float grayT = fdividef(pixel.y - inShadowsThreshold, shadowExcursion); pixel.u = LERP(shadowNewU, midtoneNewU, grayT); pixel.v = LERP(shadowNewV, midtoneNewV, grayT); } else if (pixel.y > highlightSoftness) { float highlightExcursion = inHighlightsThreshold - highlightSoftness; float grayT = fdividef(pixel.y - highlightSoftness, highlightExcursion); pixel.u = LERP(midtoneNewU, highlightNewU, grayT); pixel.v = LERP(midtoneNewV, highlightNewV, grayT); } else { pixel.u = midtoneNewU; pixel.v = midtoneNewV; } pixel.u = LERP(masterNewU, pixel.u, 0.5f); pixel.v = LERP(masterNewV, pixel.v, 0.5f); pixel.u = LERP(originalU, pixel.u, secondaryWeight); pixel.v = LERP(originalV, pixel.v, secondaryWeight); pixel.y = CalculateThreeWayLuma( pixel.y, secondaryWeight, inMasterInputBlack, inMasterInputWhite, inMasterInputGray, inMasterOutputBlack, inMasterOutputWhite, inEnableHalfMasterBlend); WriteRGBPixel(ConvertYUV_To_RGB(pixel), inImage, inPitch, inDeviceFormat, x, y); } } GF_KERNEL ThreeWayColorCorrector_MasterLevelsKernel( GF_PTR(float4) inImage, int inPitch, const GF_PTR(float) inSecondaryMask, int inSecondaryPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, float inMasterInputBlack, float inMasterInputWhite, float inMasterInputGray, float inMasterOutputBlack, float inMasterOutputWhite, int inEnableHalfMasterBlend) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelYUV pixel = ConvertRGB_To_YUV(ReadRGBPixel(inImage, inPitch, inDeviceFormat, x, y)); float secondaryWeight = inSecondaryMask ? GF_READ2D(inSecondaryMask, inSecondaryPitch, x, y) : 1.0f; pixel.y = CalculateThreeWayLuma( pixel.y, secondaryWeight, inMasterInputBlack, inMasterInputWhite, inMasterInputGray, inMasterOutputBlack, inMasterOutputWhite, inEnableHalfMasterBlend); WriteRGBPixel(ConvertYUV_To_RGB(pixel), inImage, inPitch, inDeviceFormat, x, y); } }
