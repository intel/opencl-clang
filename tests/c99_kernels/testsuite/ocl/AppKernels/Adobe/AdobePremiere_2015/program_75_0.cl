...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE(float4, texture1_RECT) GF_KERNEL ShaderKernel_fxDay2NiteV(COMMON_PARAMS, GF_OPENCL_TEXTURE(texture1_RECT)) { BEGIN_MULTITEXTURE LOAD_PARAMS(7) PARAM (w1 , program_local[0]) PARAM (w2 , program_local[1]) PARAM (w3 , program_local[2]) PARAM (w4 , program_local[3]) PARAM (invSum, program_local[4]) PARAM (colorTone, program_local[5]) PARAM (desat , program_local[6]) TEMP r0, r1, r2, r3, r4, r5, r6; ADD (&r2, s(fragment_texcoord0, yyyx), co_yyyx(1, -1, 2, 0)); ADD (&r3, s(fragment_texcoord0, yyyx), co_yyyx(-2, 3, -3, 0)); ADD (&r4, s(fragment_texcoord0, yyyx), co_yyyx(4, -4, 0, 0)); TEX (&r0, fragment_texcoord0, texture0_RECT); TEX (&r1, s(r2, wxwx), texture0_RECT); MAD (&r0, r1, w1, r0); TEX (&r1, s(r2, wywy), texture0_RECT); MAD (&r0, r1, w1, r0); TEX (&r1, s(r2, wzwz), texture0_RECT); MAD (&r0, r1, w2, r0); TEX (&r1, s(r3, wxwx), texture0_RECT); MAD (&r0, r1, w2, r0); TEX (&r1, s(r3, wywy), texture0_RECT); MAD (&r0, r1, w3, r0); TEX (&r1, s(r3, wzwz), texture0_RECT); MAD (&r0, r1, w3, r0); TEX (&r1, s(r4, wxwx), texture0_RECT); MAD (&r0, r1, w4, r0); TEX (&r1, s(r4, wywy), texture0_RECT); MAD (&r0, r1, w4, r0); MUL (&r0, r0, invSum); SUBs (r1.x, r0.x, r0.y); RCPs (r2.y, r1.x); MAXs (r2.x, r1.x, 0.001); POWs (r2.x, r2.x, 0.8); MULs_SAT (r2.y, r2.y, r2.x); LRPvsss (&r0, r2.y, r0.x, r0.y); ADD (&r2, s(fragment_texcoord1, xyxy), co_xyxy(-1, 0, 0, -1)); ADD (&r3, s(fragment_texcoord1, xyxy), co_xyxy(1, 1, 0, 0)); TEX (&r1, fragment_texcoord1, texture1_RECT); TEX (&r4, r2, texture1_RECT); TEX (&r5, s(r2, zwzw), texture1_RECT); TEX (&r6, r3, texture1_RECT); ADD (&r4, r4, r1); ADD (&r4, r4, r5); ADD (&r4, r4, r6); MUL (&r0, r0, colorTone); DP3 (&r2, r4, c3(0.07475, 0.14675, 0.0285)); ADD_SAT (&r2, r2, desat); MOVs (r2.w, 1); LRP (&result_color, r2, r1, r0); END }
