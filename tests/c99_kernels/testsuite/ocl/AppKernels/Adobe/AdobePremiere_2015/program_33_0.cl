...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE(float4, texture1_RECT) GF_KERNEL ShaderKernel_SecondaryPass4(COMMON_PARAMS, GF_OPENCL_TEXTURE(texture1_RECT)) { BEGIN_MULTITEXTURE LOAD_PARAMS(10) PARAM (offset , program_local[0]) PARAM (gamma , program_local[1]) PARAM (gain , program_local[2]) PARAM (weights , program_local[3]) PARAM (invSum , program_local[4]) PARAM (outsideA, program_local[5]) PARAM (outsideB, program_local[6]) PARAM (luminance , program_local[7]) PARAM (saturation , program_local[8]) PARAM (postSaturation, program_local[9]) TEMP r0, r1, r2, r3, r4, r5, r6, r7, r8; ADD (&r1, s(fragment_texcoord0, yyyx), co_yyyx(1, -1, 2, 0)); ADD (&r2, s(fragment_texcoord0, yyyx), co_yyyx(-2, 3, -3, 0)); ADD (&r3, s(fragment_texcoord0, yyyx), co_yyyx(4, -4, 0, 0)); TEX (&r0, fragment_texcoord0, texture0_RECT); TEX (&r7, s(r1, wxwx), texture0_RECT); TEX (&r6, s(r1, wzwz), texture0_RECT); TEX (&r5, s(r2, wywy), texture0_RECT); TEX (&r4, s(r3, wxwx), texture0_RECT); MOVs (r4.x, r7.w); MOVs (r4.y, r6.w); MOVs (r4.z, r5.w); MUL (&r4, r4, weights); TEX (&r5, s(r1, wywy), texture0_RECT); TEX (&r6, s(r2, wxwx), texture0_RECT); TEX (&r7, s(r2, wzwz), texture0_RECT); TEX (&r8, s(r3, wywy), texture0_RECT); MOVs (r8.x, r5.w); MOVs (r8.y, r6.w); MOVs (r8.z, r7.w); MAD (&r8, r8, weights, r4); DP4svs (r8.w, r8, 1); ADDs (r8.w, r8.w, r0.w); MULs (r8.w, r8.w, invSum.w); TEX (&r1, fragment_texcoord1, texture1_RECT); DP3 (&r3, r0, luminance); SUB (&r4, r0, r3); MAD (&r2, r4, saturation, r3); MAD (&r2, r2, gain, offset); CMPvvvs (&r7, r2, c4(-1,-1,-1, 1), 1); ABS (&r2, r2); POWs (r1.x, r2.x, gamma.x); POWs (r1.y, r2.y, gamma.y); POWs (r1.z, r2.z, gamma.z); MUL (&r2, r1, r7); DP3 (&r3, r2, luminance); SUB (&r4, r2, r3); MAD (&r2, r4, postSaturation, r3); MAD (&r3, r0, outsideA, outsideB); LRPvsvv_xyz (&result_color, r8.w, r2, r3); MOVs (result_color.w, r1.w); END }
