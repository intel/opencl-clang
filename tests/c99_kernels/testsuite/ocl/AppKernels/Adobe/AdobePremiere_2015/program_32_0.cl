...
#endif
typedef struct { float fLeftX; float fLeftZ; float fLeftExponent; float fLeftWeight; int iLeftInner; float fRightX; float fRightZ; float fRightExponent; float fRightWeight; int iRightInner; } SIntersection; GF_KERNEL ShadingMaskKernel(float fToX, float fFromX, unsigned int nWidth, unsigned int nHeight, GF_PTR(float) pfMaskBuffer, GF_PTR(unsigned int) pnScanlines) { int iX = KernelX(); int iY = KernelY(); if (iX < 0 || iX >= (int)nWidth || iY < 0 || iY >= (int)nHeight) return; float fResult = 0.0f; unsigned int nCount = pnScanlines[iY + 1] - pnScanlines[iY]; if (nCount > 0) { GF_PTR(SIntersection) psIntersections = (GF_PTR(SIntersection))(pnScanlines + (pnScanlines[iY] * 10) + nHeight + 1); float fPointX = fFromX + ((fToX - fFromX) / nWidth) * iX; unsigned int nFirstRight = 0; bool bLeftInInner = false; for (unsigned int n = 0; n < nCount; n++) { if (psIntersections[n].fRightX < fPointX) { if (psIntersections[n].iLeftInner | psIntersections[n].iRightInner) bLeftInInner = !bLeftInInner; nFirstRight = n + 1; } } if (nFirstRight != nCount) { bool bInInner = bLeftInInner; for (unsigned int n = nFirstRight; n < nCount; n++) { float fLeftX = psIntersections[n].fLeftX; if (fLeftX > fPointX) continue; if (psIntersections[n].iLeftInner) bInInner = !bInInner; float fLDist = fabsf(fPointX - fLeftX); float fRDist = fabsf(fPointX - psIntersections[n].fRightX); float fLDistPlusfRDist = fLDist + fRDist; float fRcpLDistPlusRDist = 1.0f / fLDistPlusfRDist; if (fLDistPlusfRDist <= 1e-3f) { fLDist = 1.0f; fRDist = 1.0f; fRcpLDistPlusRDist = 0.5f; } float f = (psIntersections[n].fLeftZ * fRDist + psIntersections[n].fRightZ * fLDist) * fRcpLDistPlusRDist; float fExponent = (psIntersections[n].fLeftExponent * fRDist + psIntersections[n].fRightExponent * fLDist) * fRcpLDistPlusRDist; float fWeight = (psIntersections[n].fLeftWeight * fRDist + psIntersections[n].fRightWeight * fLDist) * fRcpLDistPlusRDist; fExponent = fExponent * 2.0f + 1.0f; float f1 = Power(f, fExponent); float f2 = 1.0f - Power(1.0f - f, fExponent); float fRight = 2.0f * fWeight; float fLeft = fRight - 1.0f; fLeft = saturate(fLeft); fRight = saturate(fRight); f = fLeft + (fRight - fLeft) * f; f = f1 + (f2 - f1) * f; fResult = max(fResult, f); } if (bInInner) fResult = 1.0f; fResult = min(fResult, 1.0f); } } pfMaskBuffer[iY * nWidth + iX] = fResult; }
#define BLEND_BASE_PARAMS __global float4* outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, __read_only image2d_t texture0_RECT, __read_only image2d_t texture1_RECT

#define BLEND_ONLY_PARAMS __global float4* outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, __read_only image2d_t texture0_RECT, __read_only image2d_t texture1_RECT, float fOpacity

#define MASK_BLEND_PARAMS __global float4* outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, __read_only image2d_t texture0_RECT, __read_only image2d_t texture1_RECT, GF_PTR(float) pfMaskBuffer, float fOpacity

#define BEGIN_BLEND int x = KernelX(); \
 int y = KernelY(); \
 if (x < inWidth && y < inHeight) \
 { \
 float4 coords = MakeCoords(x, y); \
 sampler_t texSamplerB = CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST; \
 float4 texBG = read_imagef(texture0_RECT, texSamplerB, (float2)(coords.x, coords.y)); \
 float4 texFG = read_imagef(texture1_RECT, texSamplerB, (float2)(coords.x, coords.y));

#define END_BLEND float4 res; \
 res.x = texFG.x * fBlend + texBG.x * (1 - fBlend); \
 res.y = texFG.y * fBlend + texBG.y * (1 - fBlend); \
 res.z = texFG.z * fBlend + texBG.z * (1 - fBlend); \
 res.w = texFG.w; \
 WritePixel(res, outDest, inDestPitch, inDeviceFormat, x, y); \
 }

#define BIN_SIZE 32

#define QUEUE_ROUND_SIZE 256
GF_TEXTURE(float4, texture0_RECT, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST) GF_TEXTURE(float4, texture1_RECT, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST) GF_KERNEL Mask_Blend_Normal_ANormal(MASK_BLEND_PARAMS) { BEGIN_BLEND float fBlend = pfMaskBuffer[x + y * inWidth] * texFG.w * fOpacity; END_BLEND } GF_KERNEL Mask_Blend_Normal_AInverted(MASK_BLEND_PARAMS) { BEGIN_BLEND float fBlend = pfMaskBuffer[x + y * inWidth] * (1 - texFG.w) * fOpacity; END_BLEND } GF_KERNEL Mask_Blend_Normal(MASK_BLEND_PARAMS) { BEGIN_BLEND float fBlend = pfMaskBuffer[x + y * inWidth] * fOpacity; END_BLEND } GF_KERNEL Mask_Blend_Inverted_ANormal(MASK_BLEND_PARAMS) { BEGIN_BLEND float fBlend = (1 - pfMaskBuffer[x + y * inWidth]) * texFG.w * fOpacity; END_BLEND } GF_KERNEL Mask_Blend_Inverted_AInverted(MASK_BLEND_PARAMS) { BEGIN_BLEND float fBlend = (1 - pfMaskBuffer[x + y * inWidth]) * (1 - texFG.w) * fOpacity; END_BLEND } GF_KERNEL Mask_Blend_Inverted(MASK_BLEND_PARAMS) { BEGIN_BLEND float fBlend = (1 - pfMaskBuffer[x + y * inWidth]) * fOpacity; END_BLEND } GF_KERNEL Blend_Normal(BLEND_ONLY_PARAMS) { BEGIN_BLEND float fBlend = texFG.w * fOpacity; END_BLEND } GF_KERNEL Blend_Inverted(BLEND_ONLY_PARAMS) { BEGIN_BLEND float fBlend = (1 - texFG.w) * fOpacity; END_BLEND }GF_TEXTURE(float4, inMaskImage, CLK_NORMALIZED_COORDS_FALSE | CLK_ADDRESS_CLAMP_TO_EDGE | CLK_FILTER_NEAREST) GF_KERNEL CopyMaskToBufferKernel( GF_OPENCL_TEXTURE(inMaskImage), GF_PTR(float) outDest, int inWidth, int inHeight) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { float4 coords = MakeCoords(x, y); float4 texel = GF_READTEXTURE(inMaskImage, coords.x, coords.y); outDest[x + y * inWidth] = texel.x; } } typedef struct { float afData[18]; } STriangle; GF_KERNEL RasterizationKernel( GF_PTR(float) pfMaskBuffer, const unsigned int nLineStride, const uint4 rcROI, GF_PTR(const unsigned int) pnData, unsigned int nWidthInBins, unsigned int nTotalBins, unsigned int nQueueRounds ) { GF_PTR(const STriangle) psTriangles = (GF_PTR(const STriangle))(pnData + nTotalBins * nQueueRounds * QUEUE_ROUND_SIZE); const int iX = KernelX(); const int iY = KernelY(); if (iX < rcROI.x || iX >= rcROI.z || iY < rcROI.y || iY >= rcROI.w) return; const float fFragmentX = (float)iX + 0.5f; const float fFragmentY = (float)iY + 0.5f; const unsigned int nBinID = iX / BIN_SIZE + (iY / BIN_SIZE) * nWidthInBins; float fOutMaskValue = 0.0f; float fMaxMaskValue = 0.0f; for (unsigned int nRound = 0; nRound < nQueueRounds; nRound++) { GF_PTR(const unsigned int) pnQueue = pnData + (nTotalBins * nRound + nBinID) * QUEUE_ROUND_SIZE; for (unsigned int nQueueIndex = 0; nQueueIndex < QUEUE_ROUND_SIZE; nQueueIndex++) { const unsigned int nTriangle = pnQueue[nQueueIndex]; if (nTriangle == 0xFFFFFFFF) break; GF_PTR(const float) pf = &(psTriangles[nTriangle].afData[0]); float fBarycentricX = fFragmentX * pf[0] + fFragmentY * pf[1] + pf[2]; float fBarycentricY = fFragmentX * pf[3] + fFragmentY * pf[4] + pf[5]; float fBarycentricZ = fFragmentX * pf[6] + fFragmentY * pf[7] + pf[8]; if (fabs(fBarycentricX) < 1e-5f) fBarycentricX = 0.0f; if (fabs(fBarycentricY) < 1e-5f) fBarycentricY = 0.0f; if (fabs(fBarycentricZ) < 1e-5f) fBarycentricZ = 0.0f; if (fBarycentricX < 0.0f || fBarycentricY < 0.0f || fBarycentricZ < 0.0f) continue; if (fBarycentricX == 0.0f) { if (pf[0] < 0.0f) continue; else if (pf[0] == 0.0f && pf[1] >= 0.0f) continue; } if (fBarycentricY == 0.0f) { if (pf[3] < 0.0f) continue; else if (pf[3] == 0.0f && pf[4] >= 0.0f) continue; } if (fBarycentricZ == 0.0f) { if (pf[6] < 0.0f) continue; else if (pf[6] == 0.0f && pf[7] >= 0.0f) continue; } const float fSum = fBarycentricX + fBarycentricY + fBarycentricZ; fBarycentricX /= fSum; fBarycentricY /= fSum; fBarycentricZ /= fSum; const float fIntensity = fBarycentricX * pf[ 9] + fBarycentricY * pf[10] + fBarycentricZ * pf[11]; if (fIntensity > fMaxMaskValue) { const float fExponent = (fBarycentricX * pf[12] + fBarycentricY * pf[13] + fBarycentricZ * pf[14]) * 2.0f + 1.0f; float fWeight = fBarycentricX * pf[15] + fBarycentricY * pf[16] + fBarycentricZ * pf[17]; const float f1 = Power(fIntensity, fExponent); const float f2 = 1.0f - Power(1.0f - fIntensity, fExponent); float fRight = 2.0f * fWeight; float fLeft = fRight - 1.0f; fLeft = saturate(fLeft); fRight = saturate(fRight); fWeight = fLeft + (fRight - fLeft) * fIntensity; fOutMaskValue = f1 + (f2 - f1) * fWeight; fMaxMaskValue = fIntensity; } } } pfMaskBuffer[(rcROI.w - 1 - iY) * nLineStride + iX] = fOutMaskValue; }
