...
#endif
GF_KERNEL CopyAlphaKernel( const GF_PTR(float4) inAlphaSrc, int inAlphaSrcPitch, int inSrcWidth, int inSrcHeight, GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inDstWidth, int inDstHeight) { int x = KernelX(); int y = KernelY(); if (x < inSrcWidth && y < inSrcHeight && y < inDstHeight) { PixelRGB pixel = BGRA(0,0,0,0); PixelRGB maskPixel = ReadRGBPixel(inAlphaSrc, inAlphaSrcPitch, inDeviceFormat, x, y); int destX = x * 4; if (destX < inDstWidth) { pixel.a = maskPixel.b; WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, destX, y); } destX++; if (destX < inDstWidth) { pixel.a = maskPixel.g; WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, destX, y); } destX++; if (destX < inDstWidth) { pixel.a = maskPixel.r; WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, destX, y); } destX++; if (destX < inDstWidth) { pixel.a = maskPixel.a; WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, destX, y); } } } GF_KERNEL CopyDeepAlphaKernel( const GF_PTR(float4) inAlphaSrc, int inAlphaSrcPitch, int inSrcWidth, int inSrcHeight, GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inDstWidth, int inDstHeight) { int x = KernelX(); int y = KernelY(); if (x < inDstWidth && y < inDstHeight) { PixelRGB pixel = BGRA(0,0,0,0); if (x < inSrcWidth && y < inSrcHeight) { PixelRGB maskPixel = ReadRGBPixel(inAlphaSrc, inAlphaSrcPitch, inDeviceFormat, x, y); pixel.a = maskPixel.a; } WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, x, y); } }
