...
#endif
float GET_POLYPOINT_X(__constant float* pts, int i); float GET_POLYPOINT_Y(__constant float* pts, int i); float GET_POLYPOINT_Z(__constant float* pts, int i); float GET_POLYPOINT_X(__constant float* pts, int i) { return pts[i*3]; } float GET_POLYPOINT_Y(__constant float* pts, int i) { return pts[(i)*3+1]; } float GET_POLYPOINT_Z(__constant float* pts, int i) { return pts[(i)*3+2]; } GF_DEVICE_FUNCTION void WriteComponentIndex( float inComponent, GF_PTR(float) outImage, DevicePixelFormat inDeviceFormat, int inComponentIndex) { if (inDeviceFormat == kDevicePixelFormat_BGRA_4444_16f) { vstore_half_rtz(inComponent, inComponentIndex, (GF_PTR(half))outImage); } else { outImage[inComponentIndex] = inComponent; } } __kernel void amdcl_kernel_gm16_inplace( __global float* dstFrame, DevicePixelFormat inDeviceFormat, int width, int height, int inDestPitch, __constant float *polyPoints ) { int py = get_global_id(1); const int NUMPOINTS = 16; float x_intersect[16]; if (get_global_id(1) >= height) { return; }
#pragma unroll
 for(int n=0; n<NUMPOINTS; n++) { if( py>=GET_POLYPOINT_Y(polyPoints,n) == py<GET_POLYPOINT_Y(polyPoints,n+1)) { float x_walk = GET_POLYPOINT_Z(polyPoints,n) * (py - GET_POLYPOINT_Y(polyPoints,n)); x_intersect[n] = GET_POLYPOINT_X(polyPoints,n) + x_walk; } else x_intersect[n] = -1.f; } int px = get_global_id(0); int pidx = (py * inDestPitch + px) * 4 + 3; const int wgWidth = get_local_size(0); while( px < width ) { int inside = 0;
#pragma unroll
 for(int n=0; n<NUMPOINTS; n++) { if(px < x_intersect[n]) inside ^= 1; } if(!inside) { WriteComponentIndex(0.0f, dstFrame, inDeviceFormat, pidx); } px += wgWidth; pidx += wgWidth * 4; } } __kernel void amdcl_kernel_gm8_inplace( __global float* dstFrame, DevicePixelFormat inDeviceFormat, int width, int height, int inDestPitch, __constant float *polyPoints ) { int py = get_global_id(1); const int NUMPOINTS = 8; float x_intersect[8]; if (get_global_id(1) >= height) { return; }
#pragma unroll
 for(int n=0; n<NUMPOINTS; n++) { if( py>=GET_POLYPOINT_Y(polyPoints,n) == py<GET_POLYPOINT_Y(polyPoints,n+1)) { float x_walk = GET_POLYPOINT_Z(polyPoints,n) * (py - GET_POLYPOINT_Y(polyPoints,n)); x_intersect[n] = GET_POLYPOINT_X(polyPoints,n) + x_walk; } else x_intersect[n] = -1.f; } int px = get_global_id(0); int pidx = (py * inDestPitch + px) * 4 + 3; const int wgWidth = get_local_size(0); while( px < width ) { int inside = 0;
#pragma unroll
 for(int n=0; n<NUMPOINTS; n++) { if(px < x_intersect[n]) inside ^= 1; } if(!inside) { WriteComponentIndex(0.0f, dstFrame, inDeviceFormat, pidx); } px += wgWidth; pidx += wgWidth * 4; } } __kernel void amdcl_kernel_gm4_inplace( __global float* dstFrame, DevicePixelFormat inDeviceFormat, int width, int height, int inDestPitch, __constant float *polyPoints ) { int py = get_global_id(1); const int NUMPOINTS = 4; float x_intersect[4]; if (get_global_id(1) >= height) { return; }
#pragma unroll
 for(int n=0; n<NUMPOINTS; n++) { if( py>=GET_POLYPOINT_Y(polyPoints,n) == py<GET_POLYPOINT_Y(polyPoints,n+1)) { float x_walk = GET_POLYPOINT_Z(polyPoints,n) * (py - GET_POLYPOINT_Y(polyPoints,n)); x_intersect[n] = GET_POLYPOINT_X(polyPoints,n) + x_walk; } else x_intersect[n] = -1.f; } int px = get_global_id(0); int pidx = (py * inDestPitch + px) * 4 + 3; const int wgWidth = get_local_size(0); while( px < width ) { int inside = 0;
#pragma unroll
 for(int n=0; n<NUMPOINTS; n++) { if(px < x_intersect[n]) inside ^= 1; } if(!inside) { WriteComponentIndex(0.0f, dstFrame, inDeviceFormat, pidx); } px += wgWidth; pidx += wgWidth * 4; } }
