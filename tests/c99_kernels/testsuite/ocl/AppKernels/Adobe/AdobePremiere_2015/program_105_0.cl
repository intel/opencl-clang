...
#endif
GF_KERNEL FieldReverseKernel( const GF_PTR(float4) inSrcImage, int inSrcPitch, GF_PTR(float4) inDestImage, int inDestPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight) { int x = KernelX(); int destY = KernelY(); if (x < inWidth && destY < inHeight) { int srcY = max(destY - 1, 0); WriteRGBPixel( ReadRGBPixel(inSrcImage, inSrcPitch, inDeviceFormat, x, srcY), inDestImage, inDestPitch, inDeviceFormat, x, destY); } }
