...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE(float4, texture1_RECT) GF_KERNEL ShaderKernel_COMPOP_FADE(COMMON_PARAMS, GF_OPENCL_TEXTURE(texture1_RECT)) { BEGIN_MULTITEXTURE LOAD_PARAMS(1) PARAM (phase, program_local[0]) TEMP r0, r1; TEX (&r0, fragment_texcoord0, texture0_RECT); TEX (&r1, fragment_texcoord1, texture1_RECT); LRP (&result_color, phase, r1, r0); END }
