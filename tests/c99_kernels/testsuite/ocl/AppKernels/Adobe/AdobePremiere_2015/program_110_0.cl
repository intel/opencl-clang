...
#endif
GF_KERNEL AlphaGainKernel( const GF_PTR(float4) inSrc, int inSrcPitch, GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, float inAlphaGain) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB pixel = ReadRGBPixel(inSrc, inSrcPitch, inDeviceFormat, x, y); pixel.a *= inAlphaGain; WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, x, y); } } GF_KERNEL UnpremultiplyKernel( const GF_PTR(float4) inSrc, int inSrcPitch, GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inWidth, int inHeight, float4 inAlphaMatteColor) { int x = KernelX(); int y = KernelY(); if (x < inWidth && y < inHeight) { PixelRGB pixel = ReadRGBPixel(inSrc, inSrcPitch, inDeviceFormat, x, y); PixelRGB matte = ConvertComponents_To_RGB(inAlphaMatteColor); matte.a = 1.0f - saturate(pixel.a); matte = PremultiplyPixel(matte); pixel.b = saturate(pixel.b - matte.b); pixel.g = saturate(pixel.g - matte.g); pixel.r = saturate(pixel.r - matte.r); pixel = UnpremultiplyPixel(pixel); WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, x, y); } }
