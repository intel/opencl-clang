...
#endif
GF_KERNEL CreateShadowMask( const GF_PTR(float4) inSrc, int inSrcPitch, int inSrcWidth, int inSrcHeight, GF_PTR(float) inMask, int inMaskPitch, int inMaskWidth, int inMaskHeight, DevicePixelFormat inDeviceFormat, int inShadowOffsetX, int inShadowOffsetY) { int maskX = KernelX(); int maskY = KernelY(); if (maskX < inMaskWidth && maskY < inMaskHeight) { int srcX = maskX - inShadowOffsetX; int srcY = maskY - inShadowOffsetY; PixelRGB pixel; if (0 <= srcX && srcX < inSrcWidth && 0 <= srcY && srcY < inSrcHeight) { pixel = ReadRGBPixel(inSrc, inSrcPitch, inDeviceFormat, srcX, srcY); } else { pixel = BGRA(0,0,0,0); } GF_WRITE2D(pixel.a, inMask, inMaskPitch, maskX, maskY); } } GF_KERNEL CompositeShadowMask( const GF_PTR(float4) inSrc, int inSrcPitch, int inSrcWidth, int inSrcHeight, const GF_PTR(float) inMask, int inMaskPitch, GF_PTR(float4) outDest, int inDestPitch, int inDestWidth, int inDestHeight, int inSrcOffsetX, int inSrcOffsetY, DevicePixelFormat inDeviceFormat, float inColorB, float inColorG, float inColorR, float inOpacity, int inShadowOnly) { int destX = KernelX(); int destY = KernelY(); if (destX < inDestWidth && destY < inDestHeight) { float maskValue = GF_READ2D(inMask, inMaskPitch, destX, destY); PixelRGB shadowPixel = BGRA(inColorB, inColorG, inColorR, maskValue*inOpacity); PixelRGB pixel; if (inShadowOnly) { pixel = shadowPixel; } else { int srcX = destX - inSrcOffsetX; int srcY = destY - inSrcOffsetY; PixelRGB srcPixel; if (0 <= srcX && srcX < inSrcWidth && 0 <= srcY && srcY < inSrcHeight) { srcPixel = ReadRGBPixel(inSrc, inSrcPitch, inDeviceFormat, srcX, srcY); } else { srcPixel = BGRA(0,0,0,0); } srcPixel.a = saturate(srcPixel.a); shadowPixel.a = saturate(shadowPixel.a); float da = shadowPixel.a * (1.0f-srcPixel.a); float alpha = da + srcPixel.a; if (fIsLessThanOrEqual(alpha, 0.0f)) { pixel.b = 0.0f; pixel.g = 0.0f; pixel.r = 0.0f; pixel.a = 0.0f; } else { float ra = 1.0f / alpha; pixel.a = alpha; pixel.b = (srcPixel.b*srcPixel.a + shadowPixel.b*da) * ra; pixel.g = (srcPixel.g*srcPixel.a + shadowPixel.g*da) * ra; pixel.r = (srcPixel.r*srcPixel.a + shadowPixel.r*da) * ra; } } WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, destX, destY); } }
