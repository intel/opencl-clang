...
#define bTempTint paiBrVector[0]

#define bExposure paiBrVector[1]

#define bTone paiBrVector[2]

#define bSaturation paiBrVector[3]
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE(float4, texture2_2D) GF_KERNEL ShaderKernel_PrColorBasicCorrection(COMMON_PARAMS, GF_OPENCL_TEXTURE(texture2_2D), GF_PTR(int) paiBrVector) { BEGIN_NORMAL LOAD_PARAMS(3) PARAM (tempTint, program_local[0]) PARAM (Saturation, program_local[1]) PARAM (lumaCoeff, program_local[2]) TEMP r0, r1, r2, r3, r4; TEX (&r0, fragment_texcoord0, texture0_RECT); if (bTempTint) { MUL_xyz (&r0, r0, tempTint); } CMPvvss (&r3, r0, -1, 1); ABS (&r1, r0); MOV_SAT_xyz (&r0, r1); SUB (&r4, r1, r0); if (bExposure) { DP3 (&r1, r0, lumaCoeff); POWs (r1.x, r1.x, 0.454545454545); MAD (&r1, r1, c2(ONE_MINUS_TEXEL, 0), c2(HALF_TEXEL, 0.5)); TEX_2d (&r2, r1, texture2_2D); MADvsvs_xyz (&r0, r2.x, r0, r2.y); MAD (&r1, r4, r0, r0); MOV_SAT_xyz (&r0, r1); SUB (&r4, r1, r0); } if (bTone) { DP3 (&r1, r0, lumaCoeff); POWs (r1.x, r1.x, 0.454545454545); MAD (&r1, r1, c2(ONE_MINUS_TEXEL, 0), c2(HALF_TEXEL, 0.5)); TEX_2d (&r2, r1, texture2_2D); MADvsvs_xyz (&r0, r2.z, r0, r2.w); } MAD (&r4, r4, r0, r0); MUL_xyz (&r0, r4, r3); if (bSaturation) { DP3 (&r1, r0, lumaCoeff); SUB (&r2, r0, r1); MADvsvv_xyz (&r0, Saturation.x, r2, r1); } MOV (&result_color, r0); END }
