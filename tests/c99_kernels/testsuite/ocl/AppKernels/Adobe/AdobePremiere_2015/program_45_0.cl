...
#define bLMH paiBrVector[0]

#define bGammaLow paiBrVector[1]

#define bGammaMid paiBrVector[2]

#define bGammaHigh paiBrVector[3]

#define bBase paiBrVector[4]

#define bGamma paiBrVector[5]

#define bPostSat paiBrVector[6]

#define bPostSatLow paiBrVector[7]

#define bPostSatMid paiBrVector[8]

#define bPostSatHigh paiBrVector[9]
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE(float4, texture2_2D) GF_KERNEL ShaderKernel_IRIDASPrimary(COMMON_PARAMS, GF_OPENCL_TEXTURE(texture2_2D), GF_PTR(int) paiBrVector) { BEGIN_NORMAL LOAD_PARAMS(12) PARAM (luminance, program_local[0]); PARAM (saturationLMH, program_local[1]) PARAM (gainLow , program_local[2]) PARAM (gainMid , program_local[3]) PARAM (gainHigh , program_local[4]) PARAM (offsetLow , program_local[5]) PARAM (offsetMid , program_local[6]) PARAM (offsetHigh , program_local[7]) PARAM (gammaLow, program_local[8]) PARAM (gammaMid, program_local[9]) PARAM (gammaHigh, program_local[10]) PARAM (postSaturationLMH, program_local[11]) TEMP r0, r1, r2, r3, r4, r6, r7, r8, r9; TEX (&r0, fragment_texcoord0, texture0_RECT); DP3svv (r6.x, r0, luminance); SUBvvs (&r7, r0, r6.x); if (bLMH) { MADs (r4.x, r6.x, ONE_MINUS_TEXEL, HALF_TEXEL); MOVs (r4.y, 0.0625); TEX_2d (&r2, s(r4, xyyy), texture2_2D); MADvsvs (&r3, saturationLMH.x, r7, r6.x); MUL (&r3, r3, gainLow); if (bGammaLow) { ADD (&r3, r3, offsetLow); CMPvvvs (&r8, r3, c4(-1,-1,-1,-1), 1); ABS (&r3, r3); POWs (r3.x, r3.x, gammaLow.x); POWs (r3.y, r3.y, gammaLow.y); POWs (r3.z, r3.z, gammaLow.z); MUL (&r3, r3, r8); } else ADD (&r3, r3, offsetLow); if (bPostSatLow) { DP3 (&r8, r3, luminance); SUB (&r9, r3, r8); MADvsvv (&r3, postSaturationLMH.x, r9, r8); } MULvsv_xyz (&r0, r2.x, r3); MADvsvs (&r3, saturationLMH.y, r7, r6.x); MUL (&r3, r3, gainMid); if (bGammaMid) { ADD (&r3, r3, offsetMid); CMPvvvs (&r8, r3, c4(-1,-1,-1,-1), 1); ABS (&r3, r3); POWs (r3.x, r3.x, gammaMid.x); POWs (r3.y, r3.y, gammaMid.y); POWs (r3.z, r3.z, gammaMid.z); MUL (&r3, r3, r8); } else ADD (&r3, r3, offsetMid); if (bPostSatMid) { DP3 (&r8, r3, luminance); SUB (&r9, r3, r8); MADvsvv (&r3, postSaturationLMH.y, r9, r8); } MADvsvv_xyz (&r0, r2.y, r3, r0); MADvsvs (&r3, saturationLMH.z, r7, r6.x); MUL (&r3, r3, gainHigh); if (bGammaHigh) { ADD (&r3, r3, offsetHigh); CMPvvvs (&r8, r3, c4(-1,-1,-1,-1), 1); ABS (&r3, r3); POWs (r3.x, r3.x, gammaHigh.x); POWs (r3.y, r3.y, gammaHigh.y); POWs (r3.z, r3.z, gammaHigh.z); MUL (&r3, r3, r8); } else ADD (&r3, r3, offsetHigh); if (bPostSatHigh) { DP3 (&r8, r3, luminance); SUB (&r9, r3, r8); MADvsvv (&r3, postSaturationLMH.z, r9, r8); } MADvsvv_xyz (&r0, r2.z, r3, r0); } else if (bBase) { MADvsvs (&r3, saturationLMH.x, r7, r6.x); MUL (&r3, r3, gainLow); if (bGamma) { ADD (&r3, r3, offsetLow); CMPvvvs (&r8, r3, c4(-1,-1,-1, 1), 1); ABS (&r3, r3); POWs (r0.x, r3.x, gammaLow.x); POWs (r0.y, r3.y, gammaLow.y); POWs (r0.z, r3.z, gammaLow.z); MUL (&r0, r0, r8); } else ADD_xyz (&r0, r3, offsetLow); if (bPostSat) { DP3 (&r6, r0, luminance); SUB (&r7, r0, r6); MADvsvv_xyz (&r0, postSaturationLMH.x, r7, r6); } } MOV (&result_color, r0); END }
