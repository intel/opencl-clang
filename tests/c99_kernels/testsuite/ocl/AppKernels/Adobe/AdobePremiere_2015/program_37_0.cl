...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_CUDA_TEXTURE(float4, texture2_2D) GF_KERNEL ShaderKernel_PrimaryRGBCurves(COMMON_PARAMS, GF_OPENCL_TEXTURE(texture2_2D)) { BEGIN_NORMAL LOAD_PARAMS(1) PARAM (luminance, program_local[0]) TEMP r0, r1, r2; TEX_SAT (&r0, fragment_texcoord0, texture0_RECT); DP3svv (r2.x, r0, luminance); MADs (r2.x, r2.x, ONE_MINUS_TEXEL, HALF_TEXEL); MOVs (r2.y, 0.0625); TEX_2d (&r1, r2, texture2_2D); MAXs (r2.x, r2.x, 0.00000001); RCPs (r2.x, r2.x); MULs (r2.x, r2.x, r1.w); MULvsv_SAT (&r1, r2.x, r0); MADvvss (&r1, r1, ONE_MINUS_TEXEL, HALF_TEXEL); MOVs (r1.w, 0.0625); TEXs_2d_x (result_color.x, s(r1, xwxw), texture2_2D); TEXs_2d_y (result_color.y, s(r1, ywyw), texture2_2D); TEXs_2d_z (result_color.z, s(r1, zwzw), texture2_2D); MOVs (result_color.w, r0.w); END }
