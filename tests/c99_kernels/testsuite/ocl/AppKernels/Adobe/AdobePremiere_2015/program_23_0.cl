...
#endif
GF_KERNEL CropKernel( GF_PTR(float4) inSrc, int inSrcPitch, GF_PTR(float4) outDest, int inDestPitch, DevicePixelFormat inDeviceFormat, int inCenterTop, int inCenterLeft, int inCenterBottom, int inCenterRight, int inFeatherTop, int inFeatherLeft, int inFeatherBottom, int inFeatherRight, float inAlphaGainDecrementX, float inAlphaGainDecrementY) { int x = KernelX() + inFeatherLeft; int y = KernelY() + inFeatherTop; if (x >= inFeatherLeft && y >= inFeatherTop && x < inFeatherRight && y < inFeatherBottom) { PixelRGB pixel = ReadRGBPixel(inSrc, inSrcPitch, inDeviceFormat, x, y); int featherX = (x >= inCenterLeft) ? (x < inCenterRight) ? 0 : x - inCenterRight : inCenterLeft - x; int featherY = (y >= inCenterTop) ? (y < inCenterBottom) ? 0 : y - inCenterBottom : inCenterTop - y; if (featherX) { pixel.a *= 1.0 - (featherX * inAlphaGainDecrementX); } if (featherY) { pixel.a *= 1.0 - (featherY * inAlphaGainDecrementY); } WriteRGBPixel(pixel, outDest, inDestPitch, inDeviceFormat, x, y); } }
