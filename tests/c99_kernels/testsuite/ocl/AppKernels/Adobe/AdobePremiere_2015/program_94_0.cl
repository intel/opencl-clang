...
#endif
GF_CUDA_TEXTURE(float4, texture0_RECT) GF_KERNEL ShaderKernel_ColorMatch(COMMON_PARAMS) { BEGIN_NORMAL LOAD_PARAMS(5) PARAM (black , program_local[0]) PARAM (inScale , program_local[1]) PARAM (refBlack, program_local[2]) PARAM (outScale, program_local[3]) PARAM (exponent, program_local[4]) TEMP r0, r1, r2; TEX (&r0, fragment_texcoord0, texture0_RECT); SUB (&r1, r0, black); MUL (&r1, r1, inScale); CMPvvvs (&r2, r1, c4(-1,-1,-1, 1), 1); ABS (&r1, r1); POWs (r0.x, r1.x, exponent.x); POWs (r0.y, r1.y, exponent.y); POWs (r0.z, r1.z, exponent.z); MUL (&r0, r0, r2); MAD (&result_color, r0, outScale, refBlack); END }
