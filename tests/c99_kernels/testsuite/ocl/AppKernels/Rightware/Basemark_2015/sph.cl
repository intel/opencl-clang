...
/**
* Bitonic sort
* The array will always be sorted in ascending order based on the cell id (first element)
...
__kernel void bitonicSort(__global uint2* keys, uint stage, uint pass)
{
    ...
    //const uint leftID = (thread & (pairDistance - 1) ) | ((thread  & ~(pairDistance - 1) ) << 1);
    const uint low = thread & (pairDistance - 1);
    const uint leftID = ((thread - low) << 1) + low;

    const bool direction = ((thread >> stage) & 1) == 1 ? 0 : 1;
...
#define SORT(a,b) {bool swap = direction ^ (a.x < b.x); uint2 tmp1 = a; uint2 tmp2 = b; a = swap ? tmp1 : tmp2; b = swap ? tmp2 : tmp1; }

/* 'radix4' bitonic sort kernel based on same lessons as the increased FFT radix. One call is equivalent to 2 passes. pass and pass - 1 */
__kernel void bitonicSortR2(__global uint2* keys, uint stage, uint pass)
{
    const uint thread = get_global_id(0);
    ...
    //const uint leftID = (thread & (pairDistance - 1) ) | ((thread  & ~(pairDistance - 1) ) << 1);
    const uint low = thread & (pairDistance - 1);
    const uint leftID = ((thread - low) << 2) + low;

    const bool direction = ((thread*2 >> stage) & 1) == 1 ? 0 : 1;

    uint2 u[4];

    u[0] = keys[leftID];
    u[1] = keys[leftID + pairDistance];
    u[2] = keys[leftID + pairDistance*2];
    u[3] = keys[leftID + pairDistance*3];
    ...
    keys[leftID                   ] = u[0];
    keys[leftID + pairDistance    ] = u[1];
    keys[leftID + pairDistance * 2] = u[2];
    keys[leftID + pairDistance * 3] = u[3];
}
