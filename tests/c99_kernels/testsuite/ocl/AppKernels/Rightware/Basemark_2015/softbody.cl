...
/** If we allow usage of native functions or not */
...
__kernel void satisfyConstraints(__global float4* positions, __global int* neighbours, __global int* offsets, float tick, int startOffset)
{
    ...
    for(int z = 0; z < NEIGHBOUR_COUNT-2; ++z)
    {
        // Solve constraints in order 3,4,5,0,1,2. This reduces jitter on cloth edges.

        int j = 6 - (z < 3 ? (3-z) : (NEIGHBOUR_COUNT-2) - (z - 4) - 1);
        ...
        if(index >= 0)
        {
       /* printf("%d, %d, %d, %d, %d, %d\n", get_global_id(0), get_local_id(0), i, index,z,j);*/
            if(j>2)
            {
                rest = bmark_sqrt(rest*rest + rest*rest);
            }
            if(j>4)
            {
                rest = bmark_sqrt((2.f*rest)*(2.f*rest)+(1.f*rest)*(1.f*rest));
            }
            ...
            deltalength = bmark_sqrt(deltaDot);
    ...
    float offset = bmark_sin(tick * 0.03f) * distance * 1.20f;
    float offset2 = bmark_sin(tick * 0.026f) * distance * 1.15f;
