...
/* These variables are by default appended by benchmark to this file before compiling. */
#ifndef PARTICLES_PER_SIDE
#define PARTICLES_PER_SIDE 32
#endif
#ifndef REST_LENGTH
...
__kernel void satisfyConstraints(__global float4* positions, __global int* neighbours, __global int* offsets, float tick, int startOffset)
{
    ...
    //for(int j = NEIGHBOUR_COUNT - 1; j >= 0; --j)
    {
        ...
        if(index >= 0)
        {
            if(j>2)
            {
                rest = sqrt(rest*rest + rest*rest);
            }
            if(j>4)
            {
                rest = sqrt((2.f*rest)*(2.f*rest)+(1.f*rest)*(1.f*rest));
            }
            ...
            const float softness = 0.995f;
            deltalength = sqrt(deltaDot);
    ...
    float distance = (PARTICLES_PER_SIDE - 1) * REST_LENGTH * 1.f;
    float offset = sin(tick * 0.04f) * distance * 1.0f;
    float offset2 = sin(tick * 0.036f) * distance * 0.95f;
