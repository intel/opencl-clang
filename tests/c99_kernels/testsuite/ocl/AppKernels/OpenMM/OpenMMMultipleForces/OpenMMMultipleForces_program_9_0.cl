...
real energy = 0.0f;
if ((groups&1) != 0)
for (unsigned int index = get_global_id(0); index < 99; index += get_global_size(0)) {
...
if ((groups&1) != 0)
for (unsigned int index = get_global_id(0); index < 33; index += get_global_size(0)) {
...
float8 torsionParams = customArg4[index];
if (theta < 0.0f)
    theta += PI;
else
    theta -= PI;
