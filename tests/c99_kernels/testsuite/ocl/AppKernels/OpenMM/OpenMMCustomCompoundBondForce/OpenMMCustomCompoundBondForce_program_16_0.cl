...
#endif
__kernel void computeBondedForces(__global real4* restrict forceBuffers, __global real* restrict energyBuffer, __global const real4* restrict posq, int groups, __global const uint4* restrict atomIndices0, __global const uint4* restrict bufferIndices0, __global const uint4* restrict atomIndices1, __global const uint4* restrict bufferIndices1, __global float2* customArg1, __global float2* customArg2, __global float4* customArg3) {
real energy = 0.0f;
if ((groups&1) != 0)
