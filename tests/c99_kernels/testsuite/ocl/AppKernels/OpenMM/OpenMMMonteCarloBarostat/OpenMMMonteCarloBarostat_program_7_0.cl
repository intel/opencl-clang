 ...
 */
__kernel void sortShortList(__global real2* __restrict__ data, uint length, __local real2* dataBuffer) {
    ...
    for (unsigned int k = 2; k < 2*length; k *= 2) {
        for (unsigned int j = k/2; j > 0; j /= 2) {
            for (unsigned int i = get_local_id(0); i < length; i += get_local_size(0)) {
                int ixj = i^j;
                if (ixj > i && ixj < length) {
                    real2 value1 = dataBuffer[i];
                    real2 value2 = dataBuffer[ixj];
                    bool ascending = ((i&k) == 0);
                    for (unsigned int mask = k*2; mask < 2*length; mask *= 2)
                        ascending = ((i&mask) == 0 ? !ascending : ascending);
                    real lowKey  = (ascending ? getValue(value1) : getValue(value2));
                    real highKey = (ascending ? getValue(value2) : getValue(value1));
                    if (lowKey > highKey) {
                        dataBuffer[i] = value2;
 ...
 */
__kernel void computeRange(__global const real2* restrict data, uint length, __global real* restrict range, __local real* restrict buffer,
        uint numBuckets, __global uint* restrict bucketOffset) {
    real minimum = MAXFLOAT;
    real maximum = -MAXFLOAT;

    // Each thread calculates the range of a subset of values.

    for (uint index = get_local_id(0); index < length; index += get_local_size(0)) {
        real value = getValue(data[index]);
 ...
 */
__kernel void assignElementsToBuckets(__global const real2* restrict data, uint length, uint numBuckets, __global const real* restrict range,
 ...
 */
__kernel void copyDataToBuckets(__global const real2* restrict data, __global real2* restrict buckets, uint length, __global const uint* restrict bucketOffset, __global const uint* restrict bucketOfElement, __global const uint* restrict offsetInBucket) {
    for (uint index = get_global_id(0); index < length; index += get_global_size(0)) {
        real2 element = data[index];
 ...
 */
__kernel void sortBuckets(__global real2* restrict data, __global const real2* restrict buckets, uint numBuckets, __global const uint* restrict bucketOffset, __local real2* restrict buffer) {
    for (int index = get_group_id(0); index < numBuckets; index += get_num_groups(0)) {
        ...
        if (length <= get_local_size(0)) {
            ...
            if (get_local_id(0) < length)
                ...
                buffer[get_local_id(0)] = (real2) (MAXFLOAT, MAXFLOAT);
            barrier(CLK_LOCAL_MEM_FENCE);
            ...
            for (int k = 2; k <= get_local_size(0); k *= 2) {
                for (int j = k/2; j > 0; j /= 2) {
                    ...
                    if (ixj > get_local_id(0)) {
                        real2 value1 = buffer[get_local_id(0)];
                        real2 value2 = buffer[ixj];
            ...
            for (int k = 2; k < 2*length; k *= 2) {
                for (int j = k/2; j > 0; j /= 2) {
                    for (int i = get_local_id(0); i < length; i += get_local_size(0)) {
                        int ixj = i^j;
                        if (ixj > i && ixj < length) {
                            real2 value1 = data[startIndex+i];
                            real2 value2 = data[startIndex+ixj];
                            bool ascending = ((i&k) == 0);
