...
__kernel void computeNonbonded(
...
#endif
        , __global const float2* restrict global_sigmaEpsilon) {
    ...
    for (int pos = firstExclusionTile; pos < lastExclusionTile; pos++) {
        ...
        if (x == y) {
            ...
            localData[localAtomIndex].sigmaEpsilon_x = sigmaEpsilon1.x;
localData[localAtomIndex].sigmaEpsilon_y = sigmaEpsilon1.y;
    ...
    tempForce += prefactor*(invR - 2.0f*6.13102792e-02f*r2);
    tempEnergy += select((real) 0, prefactor*(invR + 6.13102792e-02f*r2 - 7.45241117e-01f), includeInteraction);
  #else
    const real prefactor = 138.935456f*posq1.w*posq2.w*invR;
    tempForce += prefactor;
...
localData[localAtomIndex].sigmaEpsilon_x = temp_sigmaEpsilon.x;
localData[localAtomIndex].sigmaEpsilon_y = temp_sigmaEpsilon.y;
