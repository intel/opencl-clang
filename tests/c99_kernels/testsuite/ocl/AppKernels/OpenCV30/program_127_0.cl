...
// buildOptions= -D INTEL_DEVICE -D AREA -D T=uchar -D WT=int -D WTV=int -D convertToWTV=convert_int -D convertToT=convert_uchar_sat_rte -D WT2V=float -D convertToWT2V=convert_float -D AREA_FAST -D XSCALE=2 -D YSCALE=2 -D SCALE=0.250000
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S  -D INTEL_DEVICE -D AREA -D T=uchar -D WT=int -D WTV=int -D convertToWTV=convert_int -D convertToT=convert_uchar_sat_rte -D WT2V=float -D convertToWT2V=convert_float -D AREA_FAST -D XSCALE=2 -D YSCALE=2 -D SCALE=0.250000" %cfg_path --cl-device=%cl_device 2>&1
