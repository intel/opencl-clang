...
// buildOptions= -D INTEL_DEVICE -D AREA -D T=float -D WT=float -D WTV=float -D convertToWTV=convert_float -D convertToT=convert_float
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S  -D INTEL_DEVICE -D AREA -D T=float -D WT=float -D WTV=float -D convertToWTV=convert_float -D convertToT=convert_float" %cfg_path --cl-device=%cl_device 2>&1
