...
// buildOptions= -D INTEL_DEVICE -D T=float -D convertToT=convert_float -D FT=float -D convertToFT=convert_float
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S  -D INTEL_DEVICE -D T=float -D convertToT=convert_float -D FT=float -D convertToFT=convert_float" %cfg_path --cl-device=%cl_device 2>&1
