...
// buildOptions= -D INTEL_DEVICE -D AREA -D T=float4 -D WT=float -D WTV=float4 -D convertToWTV=convert_float4 -D convertToT=convert_float4
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S  -D INTEL_DEVICE -D AREA -D T=float4 -D WT=float -D WTV=float4 -D convertToWTV=convert_float4 -D convertToT=convert_float4" %cfg_path --cl-device=%cl_device 2>&1
