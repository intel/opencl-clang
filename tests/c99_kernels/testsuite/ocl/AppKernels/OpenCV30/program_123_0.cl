...
// buildOptions= -D INTEL_DEVICE -D AREA -D T=float -D WT=float -D WTV=float -D convertToWTV=convert_float -D convertToT=convert_float -D WT2V=float -D convertToWT2V=convert_float -D AREA_FAST -D XSCALE=2 -D YSCALE=2 -D SCALE=0.250000
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S  -D INTEL_DEVICE -D AREA -D T=float -D WT=float -D WTV=float -D convertToWTV=convert_float -D convertToT=convert_float -D WT2V=float -D convertToWT2V=convert_float -D AREA_FAST -D XSCALE=2 -D YSCALE=2 -D SCALE=0.250000" %cfg_path --cl-device=%cl_device 2>&1
