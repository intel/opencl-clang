...
// buildOptions= -D INTEL_DEVICE -D LN -D T=uchar -D WT=int -D WTV=int -D convertToWTV=convert_int -D convertToT=convert_uchar_sat_rte
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S  -D INTEL_DEVICE -D LN -D T=uchar -D WT=int -D WTV=int -D convertToWTV=convert_int -D convertToT=convert_uchar_sat_rte" %cfg_path --cl-device=%cl_device 2>&1
