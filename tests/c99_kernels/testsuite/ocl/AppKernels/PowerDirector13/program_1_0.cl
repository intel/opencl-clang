...
__kernel void InvertRGB32toRGB32(__global const uchar4* src, __global uchar4* dst, int width, int height)
{
	int nSrcIndex = get_global_id(0);

	int x = nSrcIndex % width;
	int y = nSrcIndex / width;

	int nDstIndex = x + (height - 1 - y) * width;

	dst[nDstIndex] = src[nSrcIndex];
}

__kernel void YUV32toNV12_img(__global const uchar4* src, __write_only image2d_t dstY, __write_only image2d_t dstUV, int nWidth, int nHeight, int nHalfWidth, int nHalfHeight)
{
	const int x = get_global_id(0);
	const int y = get_global_id(1);
	int2 coord = (int2)(x, y);
	int nSrcIndex = y*nWidth + x;
	uchar4 tempSrc = src[nSrcIndex];
	float4 pixel = (float4)0;

	pixel = (float4)(tempSrc.z, 0.0, 0.0, 0.0);

	write_imagef(dstY, coord, pixel);

	if (x%2 == 0 && y%2 == 0)
	{
		int2 coordUV = (int2)(x/2, y/2);
		pixel = (float4)(tempSrc.y, tempSrc.x, 0.0, 0.0);
		write_imagef(dstUV, coordUV, pixel);
	}

}

__kernel void YUV32toNV12_img2(__global const uchar4* src, __write_only image2d_t dstY, __write_only image2d_t dstUV, int nWidth, int nHeight, int nHalfWidth, int nHalfHeight)
{
	const int x = get_global_id(0);
	const int y = get_global_id(1);
	int2 coord = (int2)(x, y);
	int nSrcIndex = y*nWidth + x;
	uchar4 tempSrc = src[nSrcIndex];
	float4 pixel = (float4)(tempSrc.z / 255.0, 0.0, 0.0, 0.0);
	write_imagef(dstY, coord, pixel);

	if (x%2 == 0 && y%2 == 0)
	{
		int2 coordUV = (int2)(x/2, y/2);
		pixel = (float4)(tempSrc.y / 255.0, tempSrc.x / 255.0, 0.0, 0.0);
		write_imagef(dstUV, coordUV, pixel);
	}
}


__kernel void YUV32toNV12_YPlane(__global const uchar4* src, __write_only image2d_t dstY, int nWidth)
{
	const int x = get_global_id(0);
	const int y = get_global_id(1);
...
__kernel void YUV32toNV12_UVPlane(__global const uchar4* src, __write_only image2d_t dstUV, int nWidth)
{
	const int x = get_global_id(0);
	const int y = get_global_id(1);
	int2	  coord = (int2)(x, y);

	int       nSrcIndex = y * 2 * nWidth + x * 2;
	uchar4	  tempSrc = src[nSrcIndex];
	float4    pixel = (float4)(tempSrc.y / 255.0, tempSrc.x / 255.0, 0.0, 0.0);
	write_imagef(dstUV, coord, pixel);
}

__kernel void YUV32toNV12(__global const uchar4* src, __global uchar* dstY, __global uchar2* dstUV, int nWidth, int nHeight, int nHalfWidth, int nHalfHeight)
{
	int nSrcIndex = get_global_id(0);

	uchar4 tempSrcTopLeft = src[nSrcIndex];

	dstY[nSrcIndex] = tempSrcTopLeft.z;

	int x = nSrcIndex % nWidth;
	int y = nSrcIndex / nWidth;

	if( x >= 0 && x < (nWidth-1) && y >=0 && y < (nHeight-1) )
	{
		int nTempIndex = nSrcIndex + 1;
		uchar4 tempSrcTopRight = src[nSrcIndex];

		nTempIndex = nSrcIndex + nWidth + 1;
		uchar4 tempSrcBottomRight = src[nSrcIndex];

		nTempIndex = nSrcIndex + nWidth;
		uchar4 tempSrcBottomLeft = src[nSrcIndex];

		int nDstUV_x = x / 2;
		int nDstUV_y = y / 2;
		int nDstUVIndex = nDstUV_y*nHalfWidth + nDstUV_x;

		uchar2 nAvgUV;
		nAvgUV.x = (tempSrcTopLeft.y + tempSrcTopRight.y + tempSrcBottomRight.y + tempSrcBottomLeft.y) / 4;
		nAvgUV.y = (tempSrcTopLeft.x + tempSrcTopRight.x + tempSrcBottomRight.x + tempSrcBottomLeft.x) / 4;
		dstUV[nDstUVIndex] = nAvgUV;
	}

}

__kernel void CheckYofNV12(__global const uchar* src, __global uchar4* dst, int nWidth, int nHeight)
{
	int nIndex = get_global_id(0);

	uchar tempSrc = src[nIndex];
	uchar4 tempDst;
...
__kernel void CheckUVofNV12(__global const uchar2* src, __global uchar4* dstU, __global uchar4* dstV, int nWidth, int nHeight)
{
	int nIndex = get_global_id(0);

	uchar2 tempSrc = src[nIndex];
	uchar4 tempDstU;
	uchar4 tempDstV;
	...
	dstU[nIndex] = tempDstU;
	dstV[nIndex] = tempDstV;
}
