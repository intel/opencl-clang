...
__global uchar4* dst, int nDstWidth, int nDstHeight, int nDstStride, int dwColorAdj, int dstStartIndex, unsigned char getAlphaType)
{
	...
	else
	{
		if( (j>=0) && (j<nDstHeight) && (i>=0) && (i<nDstWidth) )
		{
			...
			//dst[dstStartIndex].w = (getAlphaType == 1)? src[srcStartIndex].w : dst[dstStartIndex].w;
			//dst[dstStartIndex].w = (getAlphaType == 2)? 255 : dst[dstStartIndex].w;
			dst[dstStartIndex].w = src[srcStartIndex].w;
		}
	}
}
