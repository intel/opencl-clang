...
// buildOptions=-D OCL_CV_REDUCE_MIN -D dim=1 -D cn=2 -D ddepth=5 -D srcT=float -D dstT=float -D dstT0=float -D convertToWT=noconvert -D convertToDT=noconvert -D convertToDT0=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D OCL_CV_REDUCE_MIN -D dim=1 -D cn=2 -D ddepth=5 -D srcT=float -D dstT=float -D dstT0=float -D convertToWT=noconvert -D convertToDT=noconvert -D convertToDT0=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
