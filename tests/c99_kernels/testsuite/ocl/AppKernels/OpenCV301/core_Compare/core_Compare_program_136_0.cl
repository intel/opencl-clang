...
// buildOptions=-D UNARY_OP -D srcT1=float -D dstT=uchar -D workT=srcT1 -D cn=1 -D convertToDT=convert_uchar_sat_rte -D OP_CMP -D CMP_OPERATOR=!= -D srcT1_C1=float -D srcT2_C1=float -D dstT_C1=uchar -D workST=float -D rowsPerWI=4 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D UNARY_OP -D srcT1=float -D dstT=uchar -D workT=srcT1 -D cn=1 -D convertToDT=convert_uchar_sat_rte -D OP_CMP -D CMP_OPERATOR=!= -D srcT1_C1=float -D srcT2_C1=float -D dstT_C1=uchar -D workST=float -D rowsPerWI=4 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
