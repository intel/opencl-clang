...
// buildOptions=-D BINARY_OP -D OP_RECIP_SCALE -D srcT1=float -D srcT1_C1=float -D srcT2=float -D srcT2_C1=float -D dstT=float -D dstT_C1=float -D workT=float -D workST=float -D scaleT=float -D wdepth=5 -D convertToWT1=noconvert -D convertToWT2=noconvert -D convertToDT=noconvert -D cn=1 -D rowsPerWI=4 -D convertFromU=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D BINARY_OP -D OP_RECIP_SCALE -D srcT1=float -D srcT1_C1=float -D srcT2=float -D srcT2_C1=float -D dstT=float -D dstT_C1=float -D workT=float -D workST=float -D scaleT=float -D wdepth=5 -D convertToWT1=noconvert -D convertToWT2=noconvert -D convertToDT=noconvert -D cn=1 -D rowsPerWI=4 -D convertFromU=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
