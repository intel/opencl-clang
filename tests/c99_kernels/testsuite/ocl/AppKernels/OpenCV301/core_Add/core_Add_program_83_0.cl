...
// buildOptions=-D MASK_UNARY_OP -D OP_ADD -D srcT1=float3 -D srcT1_C1=float -D srcT2=float3 -D srcT2_C1=float -D dstT=float3 -D dstT_C1=float -D workT=float3 -D workST=float4 -D scaleT=float -D wdepth=5 -D convertToWT1=noconvert -D convertToWT2=noconvert -D convertToDT=noconvert -D cn=3 -D rowsPerWI=4 -D convertFromU=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D MASK_UNARY_OP -D OP_ADD -D srcT1=float3 -D srcT1_C1=float -D srcT2=float3 -D srcT2_C1=float -D dstT=float3 -D dstT_C1=float -D workT=float3 -D workST=float4 -D scaleT=float -D wdepth=5 -D convertToWT1=noconvert -D convertToWT2=noconvert -D convertToDT=noconvert -D cn=3 -D rowsPerWI=4 -D convertFromU=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
