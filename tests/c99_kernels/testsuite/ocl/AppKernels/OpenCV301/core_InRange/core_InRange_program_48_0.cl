...
// buildOptions=-D HAVE_SCALAR -D cn=4 -D srcT=float4 -D srcT1=float -D dstT=uchar -D kercn=4 -D depth=5 -D colsPerWI=1 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D HAVE_SCALAR -D cn=4 -D srcT=float4 -D srcT1=float -D dstT=uchar -D kercn=4 -D depth=5 -D colsPerWI=1 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
