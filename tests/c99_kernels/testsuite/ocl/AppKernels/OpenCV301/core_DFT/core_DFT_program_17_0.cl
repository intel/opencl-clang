...
// buildOptions=-D LOCAL_SIZE=72 -D kercn=8 -D FT=float -D CT=float2 -D RADIX_PROCESS=fft_radix8(smem,twiddles+0,ind,1,9);fft_radix3_B4(smem,twiddles+7,ind,8,24);fft_radix3_B4(smem,twiddles+23,ind,24,24); -D COMPLEX_INPUT -D REAL_OUTPUT -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D LOCAL_SIZE=72 -D kercn=8 -D FT=float -D CT=float2 -D RADIX_PROCESS=fft_radix8(smem,twiddles+0,ind,1,9);fft_radix3_B4(smem,twiddles+7,ind,8,24);fft_radix3_B4(smem,twiddles+23,ind,24,24); -D COMPLEX_INPUT -D REAL_OUTPUT -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
// XFAIL: *
