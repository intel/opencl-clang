...
// buildOptions=-D LOCAL_SIZE=512 -D kercn=8 -D FT=float -D CT=float2 -D RADIX_PROCESS=fft_radix8(smem,twiddles+0,ind,1,64);fft_radix8(smem,twiddles+7,ind,8,64);fft_radix8(smem,twiddles+63,ind,64,64); -D COMPLEX_INPUT -D COMPLEX_OUTPUT -D IS_1D -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D LOCAL_SIZE=512 -D kercn=8 -D FT=float -D CT=float2 -D RADIX_PROCESS=fft_radix8(smem,twiddles+0,ind,1,64);fft_radix8(smem,twiddles+7,ind,8,64);fft_radix8(smem,twiddles+63,ind,64,64); -D COMPLEX_INPUT -D COMPLEX_OUTPUT -D IS_1D -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
XFAIL: *
