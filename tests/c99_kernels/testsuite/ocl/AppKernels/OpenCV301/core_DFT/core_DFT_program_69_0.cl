...
// buildOptions=-D LOCAL_SIZE=1280 -D kercn=8 -D FT=float -D CT=float2 -D RADIX_PROCESS=fft_radix8(smem,twiddles+0,ind,1,160);fft_radix8(smem,twiddles+7,ind,8,160);fft_radix4_B2(smem,twiddles+63,ind,64,320);fft_radix5_B2(smem,twiddles+255,ind,256,256); -D COMPLEX_INPUT -D COMPLEX_OUTPUT -D IS_1D -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D LOCAL_SIZE=1280 -D kercn=8 -D FT=float -D CT=float2 -D RADIX_PROCESS=fft_radix8(smem,twiddles+0,ind,1,160);fft_radix8(smem,twiddles+7,ind,8,160);fft_radix4_B2(smem,twiddles+63,ind,64,320);fft_radix5_B2(smem,twiddles+255,ind,256,256); -D COMPLEX_INPUT -D COMPLEX_OUTPUT -D IS_1D -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
XFAIL: *
