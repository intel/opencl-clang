...
// buildOptions=-D LOCAL_SIZE=45 -D kercn=5 -D FT=float -D CT=float2 -D RADIX_PROCESS=fft_radix5(smem,twiddles+0,ind,1,9);fft_radix3_B3(smem,twiddles+4,ind,5,15);fft_radix3_B3(smem,twiddles+14,ind,15,15); -D DFT_SCALE -D COMPLEX_INPUT -D COMPLEX_OUTPUT -D IS_1D -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D LOCAL_SIZE=45 -D kercn=5 -D FT=float -D CT=float2 -D RADIX_PROCESS=fft_radix5(smem,twiddles+0,ind,1,9);fft_radix3_B3(smem,twiddles+4,ind,5,15);fft_radix3_B3(smem,twiddles+14,ind,15,15); -D DFT_SCALE -D COMPLEX_INPUT -D COMPLEX_OUTPUT -D IS_1D -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
// XFAIL: *
