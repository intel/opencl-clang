...
// buildOptions=-D cn=4 -D ANCHOR_X=0 -D ANCHOR_Y=2 -D KERNEL_SIZE_X=3 -D KERNEL_SIZE_Y=3 -D PX_LOAD_VEC_SIZE=4 -D PX_LOAD_NUM_PX=1 -D PX_PER_WI_X=2 -D PX_PER_WI_Y=1 -D PRIV_DATA_WIDTH=4 -D BORDER_REFLECT_101 -D NO_BORDER_ISOLATED -D PX_LOAD_X_ITERATIONS=4 -D PX_LOAD_Y_ITERATIONS=3 -D srcT=ushort4 -D srcT1=ushort -D dstT=ushort4 -D dstT1=ushort -D WT=float4 -D WT1=float -D convertToWT=convert_float4 -D convertToDstT=convert_ushort4_sat_rte  -D COEFF=DIG(-0.1158042103f)DIG(-0.05708277971f)DIG(0.1114236191f)DIG(-0.07989691943f)DIG(0.1459918022f)DIG(-0.09810204804f)DIG(-0.1620680690f)DIG(-0.07852491736f)DIG(0.1511056423f) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D cn=4 -D ANCHOR_X=0 -D ANCHOR_Y=2 -D KERNEL_SIZE_X=3 -D KERNEL_SIZE_Y=3 -D PX_LOAD_VEC_SIZE=4 -D PX_LOAD_NUM_PX=1 -D PX_PER_WI_X=2 -D PX_PER_WI_Y=1 -D PRIV_DATA_WIDTH=4 -D BORDER_REFLECT_101 -D NO_BORDER_ISOLATED -D PX_LOAD_X_ITERATIONS=4 -D PX_LOAD_Y_ITERATIONS=3 -D srcT=ushort4 -D srcT1=ushort -D dstT=ushort4 -D dstT1=ushort -D WT=float4 -D WT1=float -D convertToWT=convert_float4 -D convertToDstT=convert_ushort4_sat_rte  -D COEFF=DIG(-0.1158042103f)DIG(-0.05708277971f)DIG(0.1114236191f)DIG(-0.07989691943f)DIG(0.1459918022f)DIG(-0.09810204804f)DIG(-0.1620680690f)DIG(-0.07852491736f)DIG(0.1511056423f) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
