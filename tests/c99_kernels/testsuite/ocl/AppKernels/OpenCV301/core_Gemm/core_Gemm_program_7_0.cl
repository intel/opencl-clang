...
// buildOptions=-D T=float2 -D T1=float -D WT=float2 -D cn=2 -D kercn=2 -D LOCAL_SIZE=8 -D NO_MULT -D HAVE_C  -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=float2 -D T1=float -D WT=float2 -D cn=2 -D kercn=2 -D LOCAL_SIZE=8 -D NO_MULT -D HAVE_C  -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
