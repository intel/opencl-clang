...
// buildOptions=-D T=float -D T1=float -D WT=float -D cn=1 -D kercn=1 -D LOCAL_SIZE=16 -D NO_MULT -D HAVE_C  -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=float -D T1=float -D WT=float -D cn=1 -D kercn=1 -D LOCAL_SIZE=16 -D NO_MULT -D HAVE_C  -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
