...
// buildOptions=-D srcT=float3 -D srcT1=float -D dstT=float3 -D dstT1=float -D sqddepth=5 -D sqdstT=float3 -D sqdstT1=float -D convertToSDT=noconvert -D cn=3 -D convertToDT=noconvert -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_MASK -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=float3 -D srcT1=float -D dstT=float3 -D dstT1=float -D sqddepth=5 -D sqdstT=float3 -D sqdstT1=float -D convertToSDT=noconvert -D cn=3 -D convertToDT=noconvert -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_MASK -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
