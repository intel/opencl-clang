...
// buildOptions=-D srcT=float2 -D srcT1=float -D dstT=float2 -D dstT1=float -D sqddepth=5 -D sqdstT=float2 -D sqdstT1=float -D convertToSDT=noconvert -D cn=2 -D HAVE_MASK_CONT -D convertToDT=noconvert -D WGS=256 -D WGS2_ALIGNED=128 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=float2 -D srcT1=float -D dstT=float2 -D dstT1=float -D sqddepth=5 -D sqdstT=float2 -D sqdstT1=float -D convertToSDT=noconvert -D cn=2 -D HAVE_MASK_CONT -D convertToDT=noconvert -D WGS=256 -D WGS2_ALIGNED=128 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
