...
// buildOptions=-D srcT=char -D srcT1=char -D dstT=int -D dstT1=int -D sqddepth=5 -D sqdstT=float -D sqdstT1=float -D convertToSDT=convert_float -D cn=1 -D HAVE_SRC_CONT -D HAVE_MASK_CONT -D convertToDT=convert_int -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_MASK -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=char -D srcT1=char -D dstT=int -D dstT1=int -D sqddepth=5 -D sqdstT=float -D sqdstT1=float -D convertToSDT=convert_float -D cn=1 -D HAVE_SRC_CONT -D HAVE_MASK_CONT -D convertToDT=convert_int -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_MASK -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
