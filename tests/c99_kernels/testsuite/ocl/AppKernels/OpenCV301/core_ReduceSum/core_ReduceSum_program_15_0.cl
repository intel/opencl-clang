...
// buildOptions=-D OCL_CV_REDUCE_SUM -D dim=0 -D cn=4 -D ddepth=5 -D srcT=uchar -D dstT=float -D dstT0=float -D convertToWT=noconvert -D convertToDT=convert_float -D convertToDT0=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D OCL_CV_REDUCE_SUM -D dim=0 -D cn=4 -D ddepth=5 -D srcT=uchar -D dstT=float -D dstT0=float -D convertToWT=noconvert -D convertToDT=convert_float -D convertToDT0=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
