...
// buildOptions=-D srcT=uchar -D srcT1=uchar -D dstT=float -D dstTK=float -D dstT1=float -D ddepth=5 -D cn=1 -D convertToDT=convert_float -D OP_SUM_SQR -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_SRC_CONT -D kercn=1 -D convertFromU=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=uchar -D srcT1=uchar -D dstT=float -D dstTK=float -D dstT1=float -D ddepth=5 -D cn=1 -D convertToDT=convert_float -D OP_SUM_SQR -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_SRC_CONT -D kercn=1 -D convertFromU=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
