...
// buildOptions=-D srcT=int2 -D srcT1=int -D dstT=float2 -D dstTK=float2 -D dstT1=float -D ddepth=5 -D cn=2 -D convertToDT=convert_float2 -D OP_SUM_SQR -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_MASK -D kercn=1 -D convertFromU=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=int2 -D srcT1=int -D dstT=float2 -D dstTK=float2 -D dstT1=float -D ddepth=5 -D cn=2 -D convertToDT=convert_float2 -D OP_SUM_SQR -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_MASK -D kercn=1 -D convertFromU=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
