...
// buildOptions=-D srcT=float4 -D srcT1=float -D dstT=float4 -D dstTK=float4 -D dstT1=float -D ddepth=5 -D cn=4 -D convertToDT=noconvert -D OP_SUM_SQR -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_SRC_CONT -D kercn=1 -D HAVE_SRC2 -D OP_CALC2 -D HAVE_SRC2_CONT -D convertFromU=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=float4 -D srcT1=float -D dstT=float4 -D dstTK=float4 -D dstT1=float -D ddepth=5 -D cn=4 -D convertToDT=noconvert -D OP_SUM_SQR -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_SRC_CONT -D kercn=1 -D HAVE_SRC2 -D OP_CALC2 -D HAVE_SRC2_CONT -D convertFromU=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
