...
// buildOptions=-D INTER_LINEAR -D depth=5 -D T=float -D T1=float -D WT=float -D convertToWT=noconvert -D convertToDT=noconvert -D cn=1 -D INTER_RESIZE_COEF_BITS=11 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_LINEAR -D depth=5 -D T=float -D T1=float -D WT=float -D convertToWT=noconvert -D convertToDT=noconvert -D cn=1 -D INTER_RESIZE_COEF_BITS=11 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
