...
// buildOptions=-D ONLY_SUM_CONVERT -D srcT=short -D WT=float -D dstT=uchar -D coeffT=float -D wdepth=5 -D convertToWT=convert_float -D convertToDT=convert_uchar_sat_rte -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D ONLY_SUM_CONVERT -D srcT=short -D WT=float -D dstT=uchar -D coeffT=float -D wdepth=5 -D convertToWT=convert_float -D convertToDT=convert_uchar_sat_rte -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
