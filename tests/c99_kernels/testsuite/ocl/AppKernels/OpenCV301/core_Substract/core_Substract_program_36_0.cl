...
// buildOptions=-D MASK_BINARY_OP -D OP_SUB -D srcT1=float4 -D srcT1_C1=float -D srcT2=float4 -D srcT2_C1=float -D dstT=float4 -D dstT_C1=float -D workT=float4 -D workST=float4 -D scaleT=float -D wdepth=5 -D convertToWT1=noconvert -D convertToWT2=noconvert -D convertToDT=noconvert -D cn=4 -D rowsPerWI=4 -D convertFromU=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D MASK_BINARY_OP -D OP_SUB -D srcT1=float4 -D srcT1_C1=float -D srcT2=float4 -D srcT2_C1=float -D dstT=float4 -D dstT_C1=float -D workT=float4 -D workST=float4 -D scaleT=float -D wdepth=5 -D convertToWT1=noconvert -D convertToWT2=noconvert -D convertToDT=noconvert -D cn=4 -D rowsPerWI=4 -D convertFromU=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
