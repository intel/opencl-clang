...
// buildOptions=-D INTER_NEAREST -D BORDER_REPLICATE -D T=float -D rowsPerWI=4 -D T=float -D T1=float -D cn=1 -D ST=float -D depth=5 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_NEAREST -D BORDER_REPLICATE -D T=float -D rowsPerWI=4 -D T=float -D T1=float -D cn=1 -D ST=float -D depth=5 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
