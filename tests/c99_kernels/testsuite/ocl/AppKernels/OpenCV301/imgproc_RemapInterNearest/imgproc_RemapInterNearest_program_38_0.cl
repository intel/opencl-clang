...
// buildOptions=-D INTER_NEAREST -D BORDER_WRAP -D T=float3 -D rowsPerWI=4 -D T=float3 -D T1=float -D cn=3 -D ST=float4 -D depth=5 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_NEAREST -D BORDER_WRAP -D T=float3 -D rowsPerWI=4 -D T=float3 -D T1=float -D cn=3 -D ST=float4 -D depth=5 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
