...
// buildOptions=-D OP_MERGE -D cn=3 -D T=ulong -D DECLARE_SRC_PARAMS_N=DECLARE_SRC_PARAM(0)DECLARE_SRC_PARAM(1)DECLARE_SRC_PARAM(2) -D DECLARE_INDEX_N=DECLARE_INDEX(0)DECLARE_INDEX(1)DECLARE_INDEX(2) -D PROCESS_ELEMS_N=PROCESS_ELEM(0)PROCESS_ELEM(1)PROCESS_ELEM(2) -D scn0=1 -D scn1=2 -D scn2=2 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D OP_MERGE -D cn=3 -D T=ulong -D DECLARE_SRC_PARAMS_N=DECLARE_SRC_PARAM(0)DECLARE_SRC_PARAM(1)DECLARE_SRC_PARAM(2) -D DECLARE_INDEX_N=DECLARE_INDEX(0)DECLARE_INDEX(1)DECLARE_INDEX(2) -D PROCESS_ELEMS_N=PROCESS_ELEM(0)PROCESS_ELEM(1)PROCESS_ELEM(2) -D scn0=1 -D scn1=2 -D scn2=2 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
