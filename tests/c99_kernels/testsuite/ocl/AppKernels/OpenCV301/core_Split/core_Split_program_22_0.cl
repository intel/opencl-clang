...
// buildOptions=-D T=ulong -D OP_SPLIT -D cn=2 -D DECLARE_DST_PARAMS=DECLARE_DST_PARAM(0)DECLARE_DST_PARAM(1) -D PROCESS_ELEMS_N=PROCESS_ELEM(0)PROCESS_ELEM(1) -D DECLARE_INDEX_N=DECLARE_INDEX(0)DECLARE_INDEX(1) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=ulong -D OP_SPLIT -D cn=2 -D DECLARE_DST_PARAMS=DECLARE_DST_PARAM(0)DECLARE_DST_PARAM(1) -D PROCESS_ELEMS_N=PROCESS_ELEM(0)PROCESS_ELEM(1) -D DECLARE_INDEX_N=DECLARE_INDEX(0)DECLARE_INDEX(1) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
