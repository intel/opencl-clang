...
// buildOptions=-D T=ushort -D OP_SPLIT -D cn=4 -D DECLARE_DST_PARAMS=DECLARE_DST_PARAM(0)DECLARE_DST_PARAM(1)DECLARE_DST_PARAM(2)DECLARE_DST_PARAM(3) -D PROCESS_ELEMS_N=PROCESS_ELEM(0)PROCESS_ELEM(1)PROCESS_ELEM(2)PROCESS_ELEM(3) -D DECLARE_INDEX_N=DECLARE_INDEX(0)DECLARE_INDEX(1)DECLARE_INDEX(2)DECLARE_INDEX(3) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=ushort -D OP_SPLIT -D cn=4 -D DECLARE_DST_PARAMS=DECLARE_DST_PARAM(0)DECLARE_DST_PARAM(1)DECLARE_DST_PARAM(2)DECLARE_DST_PARAM(3) -D PROCESS_ELEMS_N=PROCESS_ELEM(0)PROCESS_ELEM(1)PROCESS_ELEM(2)PROCESS_ELEM(3) -D DECLARE_INDEX_N=DECLARE_INDEX(0)DECLARE_INDEX(1)DECLARE_INDEX(2)DECLARE_INDEX(3) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
