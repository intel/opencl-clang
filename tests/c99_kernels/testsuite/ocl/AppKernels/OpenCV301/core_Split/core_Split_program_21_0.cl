...
// buildOptions=-D T=ulong -D OP_SPLIT -D cn=1 -D DECLARE_DST_PARAMS=DECLARE_DST_PARAM(0) -D PROCESS_ELEMS_N=PROCESS_ELEM(0) -D DECLARE_INDEX_N=DECLARE_INDEX(0) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=ulong -D OP_SPLIT -D cn=1 -D DECLARE_DST_PARAMS=DECLARE_DST_PARAM(0) -D PROCESS_ELEMS_N=PROCESS_ELEM(0) -D DECLARE_INDEX_N=DECLARE_INDEX(0) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
