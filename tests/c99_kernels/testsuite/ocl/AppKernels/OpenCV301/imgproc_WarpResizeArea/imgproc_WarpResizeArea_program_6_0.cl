...
// buildOptions=-D INTER_AREA -D T=float -D T1=float -D WTV=float -D convertToWTV=noconvert -D cn=1 -D convertToT=noconvert -D WT2V=float -D convertToWT2V=noconvert -D INTER_AREA_FAST -D XSCALE=2 -D YSCALE=2 -D SCALE=0.250000f -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_AREA -D T=float -D T1=float -D WTV=float -D convertToWTV=noconvert -D cn=1 -D convertToT=noconvert -D WT2V=float -D convertToWT2V=noconvert -D INTER_AREA_FAST -D XSCALE=2 -D YSCALE=2 -D SCALE=0.250000f -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
