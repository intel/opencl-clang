...
// buildOptions=-D INTER_AREA -D T=float4 -D T1=float -D WTV=float4 -D convertToWTV=noconvert -D cn=4 -D convertToT=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_AREA -D T=float4 -D T1=float -D WTV=float4 -D convertToWTV=noconvert -D cn=4 -D convertToT=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
