...
// buildOptions=-D INTER_AREA -D T=uchar -D T1=uchar -D WTV=int -D convertToWTV=convert_int -D cn=1 -D convertToT=convert_uchar_sat_rte -D WT2V=float -D convertToWT2V=convert_float -D INTER_AREA_FAST -D XSCALE=2 -D YSCALE=2 -D SCALE=0.250000f -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_AREA -D T=uchar -D T1=uchar -D WTV=int -D convertToWTV=convert_int -D cn=1 -D convertToT=convert_uchar_sat_rte -D WT2V=float -D convertToWT2V=convert_float -D INTER_AREA_FAST -D XSCALE=2 -D YSCALE=2 -D SCALE=0.250000f -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
