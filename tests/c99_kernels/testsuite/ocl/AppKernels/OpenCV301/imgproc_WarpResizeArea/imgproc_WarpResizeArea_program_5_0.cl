...
// buildOptions=-D INTER_AREA -D T=float -D T1=float -D WTV=float -D convertToWTV=noconvert -D cn=1 -D convertToT=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_AREA -D T=float -D T1=float -D WTV=float -D convertToWTV=noconvert -D cn=1 -D convertToT=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
