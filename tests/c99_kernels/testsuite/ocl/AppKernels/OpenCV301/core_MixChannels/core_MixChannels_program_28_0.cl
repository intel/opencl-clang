...
// buildOptions=-D T=ushort -D DECLARE_INPUT_MAT_N=DECLARE_INPUT_MAT(0) -D DECLARE_OUTPUT_MAT_N=DECLARE_OUTPUT_MAT(0) -D PROCESS_ELEM_N=PROCESS_ELEM(0) -D DECLARE_INDEX_N=DECLARE_INDEX(0) -D scn0=3 -D dcn0=1 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=ushort -D DECLARE_INPUT_MAT_N=DECLARE_INPUT_MAT(0) -D DECLARE_OUTPUT_MAT_N=DECLARE_OUTPUT_MAT(0) -D PROCESS_ELEM_N=PROCESS_ELEM(0) -D DECLARE_INDEX_N=DECLARE_INDEX(0) -D scn0=3 -D dcn0=1 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
