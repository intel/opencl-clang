...
// buildOptions=-D T=ulong -D DECLARE_INPUT_MAT_N=DECLARE_INPUT_MAT(0)DECLARE_INPUT_MAT(1) -D DECLARE_OUTPUT_MAT_N=DECLARE_OUTPUT_MAT(0)DECLARE_OUTPUT_MAT(1) -D PROCESS_ELEM_N=PROCESS_ELEM(0)PROCESS_ELEM(1) -D DECLARE_INDEX_N=DECLARE_INDEX(0)DECLARE_INDEX(1) -D scn0=3 -D dcn0=2 -D scn1=3 -D dcn1=3 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=ulong -D DECLARE_INPUT_MAT_N=DECLARE_INPUT_MAT(0)DECLARE_INPUT_MAT(1) -D DECLARE_OUTPUT_MAT_N=DECLARE_OUTPUT_MAT(0)DECLARE_OUTPUT_MAT(1) -D PROCESS_ELEM_N=PROCESS_ELEM(0)PROCESS_ELEM(1) -D DECLARE_INDEX_N=DECLARE_INDEX(0)DECLARE_INDEX(1) -D scn0=3 -D dcn0=2 -D scn1=3 -D dcn1=3 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
