...
// buildOptions=-D INTER_LINEAR -D BORDER_WRAP -D T=ushort -D rowsPerWI=4 -D WT=float -D convertToT=convert_ushort_sat_rte -D convertToWT=convert_float -D convertToWT2=convert_float2 -D WT2=float2 -D T=ushort -D T1=ushort -D cn=1 -D ST=ushort -D depth=2 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_LINEAR -D BORDER_WRAP -D T=ushort -D rowsPerWI=4 -D WT=float -D convertToT=convert_ushort_sat_rte -D convertToWT=convert_float -D convertToWT2=convert_float2 -D WT2=float2 -D T=ushort -D T1=ushort -D cn=1 -D ST=ushort -D depth=2 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
