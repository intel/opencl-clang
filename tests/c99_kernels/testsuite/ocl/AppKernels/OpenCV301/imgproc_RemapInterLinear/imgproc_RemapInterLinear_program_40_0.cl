...
// buildOptions=-D INTER_LINEAR -D BORDER_REFLECT_101 -D T=float3 -D rowsPerWI=4 -D WT=float3 -D convertToT=noconvert -D convertToWT=noconvert -D convertToWT2=convert_float2 -D WT2=float2 -D T=float3 -D T1=float -D cn=3 -D ST=float4 -D depth=5 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_LINEAR -D BORDER_REFLECT_101 -D T=float3 -D rowsPerWI=4 -D WT=float3 -D convertToT=noconvert -D convertToWT=noconvert -D convertToWT2=convert_float2 -D WT2=float2 -D T=float3 -D T1=float -D cn=3 -D ST=float4 -D depth=5 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
