...
// buildOptions=-D INTER_LINEAR -D BORDER_CONSTANT -D T=uchar -D rowsPerWI=4 -D WT=float -D convertToT=convert_uchar_sat_rte -D convertToWT=convert_float -D convertToWT2=convert_float2 -D WT2=float2 -D T=uchar -D T1=uchar -D cn=1 -D ST=uchar -D depth=0 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_LINEAR -D BORDER_CONSTANT -D T=uchar -D rowsPerWI=4 -D WT=float -D convertToT=convert_uchar_sat_rte -D convertToWT=convert_float -D convertToWT2=convert_float2 -D WT2=float2 -D T=uchar -D T1=uchar -D cn=1 -D ST=uchar -D depth=0 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
