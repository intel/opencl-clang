...
// buildOptions=-D T=float2 -D FT=float2 -D convertToT=noconvert -D convertToFT=noconvert -D T1=float -D cn=2 -D LOCAL_SIZE=16 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=float2 -D FT=float2 -D convertToT=noconvert -D convertToFT=noconvert -D T1=float -D cn=2 -D LOCAL_SIZE=16 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
