...
// buildOptions=-D T=uchar -D FT=float -D convertToT=convert_uchar_sat_rte -D convertToFT=convert_float -D T1=uchar -D cn=1 -D LOCAL_SIZE=16 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=uchar -D FT=float -D convertToT=convert_uchar_sat_rte -D convertToFT=convert_float -D T1=uchar -D cn=1 -D LOCAL_SIZE=16 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
