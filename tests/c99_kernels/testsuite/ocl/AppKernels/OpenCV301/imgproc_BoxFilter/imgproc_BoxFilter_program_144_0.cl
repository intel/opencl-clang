...
// buildOptions=-D LOCAL_SIZE_X=32 -D BLOCK_SIZE_Y=5 -D ST=float2 -D DT=float2 -D WT=float2 -D convertToDT=noconvert -D convertToWT=noconvert -D ANCHOR_X=2 -D ANCHOR_Y=3 -D KERNEL_SIZE_X=4 -D KERNEL_SIZE_Y=4 -D BORDER_REFLECT_101 -D NORMALIZE -D ST1=float -D DT1=float -D cn=2 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D LOCAL_SIZE_X=32 -D BLOCK_SIZE_Y=5 -D ST=float2 -D DT=float2 -D WT=float2 -D convertToDT=noconvert -D convertToWT=noconvert -D ANCHOR_X=2 -D ANCHOR_Y=3 -D KERNEL_SIZE_X=4 -D KERNEL_SIZE_Y=4 -D BORDER_REFLECT_101 -D NORMALIZE -D ST1=float -D DT1=float -D cn=2 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
