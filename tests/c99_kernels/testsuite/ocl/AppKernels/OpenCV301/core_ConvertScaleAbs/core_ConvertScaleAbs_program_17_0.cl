...
// buildOptions=-D OP_CONVERT_SCALE_ABS -D UNARY_OP -D dstT=uchar2 -D srcT1=float2 -D workT=float2 -D wdepth=5 -D convertToWT1=noconvert -D convertToDT=convert_uchar2_sat_rte -D workT1=float -D rowsPerWI=4 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D OP_CONVERT_SCALE_ABS -D UNARY_OP -D dstT=uchar2 -D srcT1=float2 -D workT=float2 -D wdepth=5 -D convertToWT1=noconvert -D convertToDT=convert_uchar2_sat_rte -D workT1=float -D rowsPerWI=4 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
