...
// buildOptions=-D OP_CONVERT_SCALE_ABS -D UNARY_OP -D dstT=uchar4 -D srcT1=float4 -D workT=float4 -D wdepth=5 -D convertToWT1=noconvert -D convertToDT=convert_uchar4_sat_rte -D workT1=float -D rowsPerWI=4 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D OP_CONVERT_SCALE_ABS -D UNARY_OP -D dstT=uchar4 -D srcT1=float4 -D workT=float4 -D wdepth=5 -D convertToWT1=noconvert -D convertToDT=convert_uchar4_sat_rte -D workT1=float -D rowsPerWI=4 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
