...
// buildOptions=-D OP_CONVERT_SCALE_ABS -D UNARY_OP -D dstT=uchar -D srcT1=uchar -D workT=float -D wdepth=5 -D convertToWT1=convert_float -D convertToDT=convert_uchar_sat_rte -D workT1=float -D rowsPerWI=4 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D OP_CONVERT_SCALE_ABS -D UNARY_OP -D dstT=uchar -D srcT1=uchar -D workT=float -D wdepth=5 -D convertToWT1=convert_float -D convertToDT=convert_uchar_sat_rte -D workT1=float -D rowsPerWI=4 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
