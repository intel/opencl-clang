...
// buildOptions=-D srcT=uchar -D WT=float -D dstT=ushort -D convertToWT=convert_float -D convertToDT=convert_ushort_sat_rte -D NO_SCALE -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=uchar -D WT=float -D dstT=ushort -D convertToWT=convert_float -D convertToDT=convert_ushort_sat_rte -D NO_SCALE -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
