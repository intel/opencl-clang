...
// buildOptions=-D srcT=int -D WT=float -D dstT=uchar -D convertToWT=convert_float -D convertToDT=convert_uchar_sat_rte -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=int -D WT=float -D dstT=uchar -D convertToWT=convert_float -D convertToDT=convert_uchar_sat_rte -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
