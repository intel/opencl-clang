...
// buildOptions=-D srcT=float -D WT=float -D dstT=short -D convertToWT=noconvert -D convertToDT=convert_short_sat_rte -D NO_SCALE -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=float -D WT=float -D dstT=short -D convertToWT=noconvert -D convertToDT=convert_short_sat_rte -D NO_SCALE -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
