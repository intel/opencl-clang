...
// buildOptions=-D srcT=float -D WT=float -D dstT=int -D convertToWT=noconvert -D convertToDT=convert_int_rte -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=float -D WT=float -D dstT=int -D convertToWT=noconvert -D convertToDT=convert_int_rte -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
