...
// buildOptions=-D srcT=ushort -D WT=float -D dstT=float -D convertToWT=convert_float -D convertToDT=noconvert -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=ushort -D WT=float -D dstT=float -D convertToWT=convert_float -D convertToDT=noconvert -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
