...
// buildOptions=-D cn=1 -D ANCHOR_X=2 -D ANCHOR_Y=2 -D KERNEL_SIZE_X=5 -D KERNEL_SIZE_Y=5 -D PX_LOAD_VEC_SIZE=1 -D PX_LOAD_NUM_PX=1 -D DEPTH_5 -D PX_PER_WI_X=1 -D PX_PER_WI_Y=1 -D PRIV_DATA_WIDTH=5 -D BORDER_CONSTANT -D NO_BORDER_ISOLATED -D PX_LOAD_X_ITERATIONS=5 -D PX_LOAD_Y_ITERATIONS=5 -D srcT=float -D srcT1=float -D dstT=srcT -D dstT1=srcT1 -D WT=srcT -D WT1=srcT1 -D convertToWT=noconvert -D convertToDstT=noconvert -D PX_LOAD_FLOAT_VEC_CONV=convert_float -D PROCESS_ELEM_=PROCESS(0,0)PROCESS(0,1)PROCESS(0,2)PROCESS(0,3)PROCESS(0,4)PROCESS(1,0)PROCESS(1,1)PROCESS(1,2)PROCESS(1,3)PROCESS(1,4)PROCESS(2,0)PROCESS(2,1)PROCESS(2,2)PROCESS(2,3)PROCESS(2,4)PROCESS(3,0)PROCESS(3,1)PROCESS(3,2)PROCESS(3,3)PROCESS(3,4)PROCESS(4,0)PROCESS(4,1)PROCESS(4,2)PROCESS(4,3)PROCESS(4,4) -D OP_DILATE -D INTEL_DEVICE
