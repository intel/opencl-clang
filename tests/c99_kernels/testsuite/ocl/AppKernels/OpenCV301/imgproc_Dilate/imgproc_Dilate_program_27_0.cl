...
// buildOptions=-D RADIUSX=1 -D RADIUSY=1 -D LSIZE0=16 -D LSIZE1=16 -D OP_DILATE -D PROCESS_ELEMS=PROCESS(0,0)PROCESS(0,1)PROCESS(0,2)PROCESS(1,0)PROCESS(1,1)PROCESS(1,2)PROCESS(2,0)PROCESS(2,1) -D T=float -D DEPTH_5 -D cn=1 -D T1=float -D convertToWT=noconvert -D convertToT=noconvert -D ST=float -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D RADIUSX=1 -D RADIUSY=1 -D LSIZE0=16 -D LSIZE1=16 -D OP_DILATE -D PROCESS_ELEMS=PROCESS(0,0)PROCESS(0,1)PROCESS(0,2)PROCESS(1,0)PROCESS(1,1)PROCESS(1,2)PROCESS(2,0)PROCESS(2,1) -D T=float -D DEPTH_5 -D cn=1 -D T1=float -D convertToWT=noconvert -D convertToT=noconvert -D ST=float -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
