...
// buildOptions=-D srcT=uchar4 -D srcT1=uchar -D dstT=float -D dstTK=float4 -D ddepth=5 -D convertToDT=convert_float4 -D OP_DOT -D WGS=256 -D WGS2_ALIGNED=128 -D kercn=4 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=uchar4 -D srcT1=uchar -D dstT=float -D dstTK=float4 -D ddepth=5 -D convertToDT=convert_float4 -D OP_DOT -D WGS=256 -D WGS2_ALIGNED=128 -D kercn=4 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
