...
// buildOptions=-D srcT=ushort2 -D srcT1=ushort -D dstT=float -D dstTK=float2 -D ddepth=5 -D convertToDT=convert_float2 -D OP_DOT -D WGS=256 -D WGS2_ALIGNED=128 -D kercn=2 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=ushort2 -D srcT1=ushort -D dstT=float -D dstTK=float2 -D ddepth=5 -D convertToDT=convert_float2 -D OP_DOT -D WGS=256 -D WGS2_ALIGNED=128 -D kercn=2 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
