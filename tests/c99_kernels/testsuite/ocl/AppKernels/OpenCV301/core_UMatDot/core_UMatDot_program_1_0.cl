...
// buildOptions=-D srcT=uchar -D srcT1=uchar -D dstT=float -D dstTK=float -D ddepth=5 -D convertToDT=convert_float -D OP_DOT -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_SRC_CONT -D HAVE_SRC2_CONT -D kercn=1 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D srcT=uchar -D srcT1=uchar -D dstT=float -D dstTK=float -D ddepth=5 -D convertToDT=convert_float -D OP_DOT -D WGS=256 -D WGS2_ALIGNED=128 -D HAVE_SRC_CONT -D HAVE_SRC2_CONT -D kercn=1 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
