...
// buildOptions=-D RADIUSX=2 -D LSIZE0=16 -D LSIZE1=16 -D CN=4 -D BORDER_REPLICATE -D EXTRA_EXTRAPOLATION -D NO_BORDER_ISOLATED -D srcT=ushort4 -D dstT=float4 -D convertToDstT=convert_float4 -D srcT1=ushort -D dstT1=float -D COEFF=DIG(0.01964691281f)DIG(0.2257081866f)DIG(0.5092898011f)DIG(0.2257081866f)DIG(0.01964691281f) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D RADIUSX=2 -D LSIZE0=16 -D LSIZE1=16 -D CN=4 -D BORDER_REPLICATE -D EXTRA_EXTRAPOLATION -D NO_BORDER_ISOLATED -D srcT=ushort4 -D dstT=float4 -D convertToDstT=convert_float4 -D srcT1=ushort -D dstT1=float -D COEFF=DIG(0.01964691281f)DIG(0.2257081866f)DIG(0.5092898011f)DIG(0.2257081866f)DIG(0.01964691281f) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
