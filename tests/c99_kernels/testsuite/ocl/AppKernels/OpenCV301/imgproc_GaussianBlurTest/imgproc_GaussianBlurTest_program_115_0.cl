...
// buildOptions=-D RADIUSY=2 -D LSIZE0=16 -D LSIZE1=16 -D CN=3 -D srcT=float3 -D dstT=float3 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D SHIFT_BITS=16 -D COEFF=DIG(0.0004319940927f)DIG(0.1177661493f)DIG(0.7636036873f)DIG(0.1177661493f)DIG(0.0004319940927f) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D RADIUSY=2 -D LSIZE0=16 -D LSIZE1=16 -D CN=3 -D srcT=float3 -D dstT=float3 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D SHIFT_BITS=16 -D COEFF=DIG(0.0004319940927f)DIG(0.1177661493f)DIG(0.7636036873f)DIG(0.1177661493f)DIG(0.0004319940927f) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
