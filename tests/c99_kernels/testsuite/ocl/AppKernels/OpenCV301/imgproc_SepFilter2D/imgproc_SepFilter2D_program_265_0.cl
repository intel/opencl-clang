...
// buildOptions=-D RADIUSX=2 -D LSIZE0=16 -D LSIZE1=16 -D CN=4 -D BORDER_REPLICATE -D EXTRA_EXTRAPOLATION -D NO_BORDER_ISOLATED -D srcT=float4 -D dstT=float4 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D COEFF=DIG(-0.1963987350f)DIG(-0.2256549448f)DIG(0.3866854012f)DIG(-0.09433235228f)DIG(0.09692859650f) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D RADIUSX=2 -D LSIZE0=16 -D LSIZE1=16 -D CN=4 -D BORDER_REPLICATE -D EXTRA_EXTRAPOLATION -D NO_BORDER_ISOLATED -D srcT=float4 -D dstT=float4 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D COEFF=DIG(-0.1963987350f)DIG(-0.2256549448f)DIG(0.3866854012f)DIG(-0.09433235228f)DIG(0.09692859650f) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
