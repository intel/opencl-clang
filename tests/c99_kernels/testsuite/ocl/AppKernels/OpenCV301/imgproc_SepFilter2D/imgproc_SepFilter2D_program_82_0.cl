...
// buildOptions=-D RADIUSY=4 -D LSIZE0=16 -D LSIZE1=16 -D CN=3 -D srcT=float3 -D dstT=uchar3 -D convertToDstT=convert_uchar3_sat_rte -D srcT1=float -D dstT1=uchar -D SHIFT_BITS=16 -D COEFF=DIG(-0.06147346646f)DIG(-0.07180448622f)DIG(0.1147519723f)DIG(-0.1352790445f)DIG(-0.1516204625f)DIG(-0.1217835024f)DIG(-0.1543113738f)DIG(-0.1373673230f)DIG(-0.05160841346f) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D RADIUSY=4 -D LSIZE0=16 -D LSIZE1=16 -D CN=3 -D srcT=float3 -D dstT=uchar3 -D convertToDstT=convert_uchar3_sat_rte -D srcT1=float -D dstT1=uchar -D SHIFT_BITS=16 -D COEFF=DIG(-0.06147346646f)DIG(-0.07180448622f)DIG(0.1147519723f)DIG(-0.1352790445f)DIG(-0.1516204625f)DIG(-0.1217835024f)DIG(-0.1543113738f)DIG(-0.1373673230f)DIG(-0.05160841346f) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
