...
// buildOptions=-D RADIUSY=1 -D LSIZE0=16 -D LSIZE1=16 -D CN=2 -D srcT=float2 -D dstT=float2 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D SHIFT_BITS=16 -D COEFF=DIG(-0.3661555350f)DIG(0.4813595414f)DIG(0.1524849236f) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D RADIUSY=1 -D LSIZE0=16 -D LSIZE1=16 -D CN=2 -D srcT=float2 -D dstT=float2 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D SHIFT_BITS=16 -D COEFF=DIG(-0.3661555350f)DIG(0.4813595414f)DIG(0.1524849236f) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
