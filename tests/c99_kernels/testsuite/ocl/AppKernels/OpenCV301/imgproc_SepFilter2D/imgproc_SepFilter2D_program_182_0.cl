...
// buildOptions=-D RADIUSY=2 -D LSIZE0=16 -D LSIZE1=16 -D CN=2 -D srcT=float2 -D dstT=float2 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D SHIFT_BITS=16 -D COEFF=DIG(-0.04577890411f)DIG(0.05427941680f)DIG(-0.4562918246f)DIG(-0.1920820475f)DIG(0.2515677810f) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D RADIUSY=2 -D LSIZE0=16 -D LSIZE1=16 -D CN=2 -D srcT=float2 -D dstT=float2 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D SHIFT_BITS=16 -D COEFF=DIG(-0.04577890411f)DIG(0.05427941680f)DIG(-0.4562918246f)DIG(-0.1920820475f)DIG(0.2515677810f) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
