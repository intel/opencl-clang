...
// buildOptions=-D RADIUSX=3 -D LSIZE0=16 -D LSIZE1=16 -D CN=3 -D BORDER_REPLICATE -D EXTRA_EXTRAPOLATION -D BORDER_ISOLATED -D srcT=float3 -D dstT=float3 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D COEFF=DIG(-0.02230604738f)DIG(0.2124560773f)DIG(-0.2290304899f)DIG(0.08035211265f)DIG(-0.1796571612f)DIG(0.2580593526f)DIG(0.01813877001f) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D RADIUSX=3 -D LSIZE0=16 -D LSIZE1=16 -D CN=3 -D BORDER_REPLICATE -D EXTRA_EXTRAPOLATION -D BORDER_ISOLATED -D srcT=float3 -D dstT=float3 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D COEFF=DIG(-0.02230604738f)DIG(0.2124560773f)DIG(-0.2290304899f)DIG(0.08035211265f)DIG(-0.1796571612f)DIG(0.2580593526f)DIG(0.01813877001f) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
