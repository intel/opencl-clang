...
// buildOptions=-D BLK_X=16 -D BLK_Y=8 -D RADIUSX=3 -D RADIUSY=4 -D KERNEL_MATRIX_X=DIG(-0.02230604738f)DIG(0.2124560773f)DIG(-0.2290304899f)DIG(0.08035211265f)DIG(-0.1796571612f)DIG(0.2580593526f)DIG(0.01813877001f) -D KERNEL_MATRIX_Y=DIG(-0.06147346646f)DIG(-0.07180448622f)DIG(0.1147519723f)DIG(-0.1352790445f)DIG(-0.1516204625f)DIG(-0.1217835024f)DIG(-0.1543113738f)DIG(-0.1373673230f)DIG(-0.05160841346f) -D srcT=uchar4 -D convertToWT=convert_float4 -D WT=float4 -D dstT=uchar4 -D convertToDstT=convert_uchar4_sat_rte -D BORDER_REFLECT -D srcT1=uchar -D dstT1=uchar -D WT1=float -D CN=4 -D SHIFT_BITS=16 -D INTEL_DEVICE
