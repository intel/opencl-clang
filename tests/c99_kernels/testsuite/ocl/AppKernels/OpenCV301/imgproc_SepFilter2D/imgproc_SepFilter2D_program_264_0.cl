...
// buildOptions=-D RADIUSY=1 -D LSIZE0=16 -D LSIZE1=16 -D CN=4 -D srcT=float4 -D dstT=float4 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D SHIFT_BITS=16 -D COEFF=DIG(0.7018499970f)DIG(-0.1931490451f)DIG(0.1050009429f) -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D RADIUSY=1 -D LSIZE0=16 -D LSIZE1=16 -D CN=4 -D srcT=float4 -D dstT=float4 -D convertToDstT=noconvert -D srcT1=float -D dstT1=float -D SHIFT_BITS=16 -D COEFF=DIG(0.7018499970f)DIG(-0.1931490451f)DIG(0.1050009429f) -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
