...
__kernel void DoubleFishEye_NoInverse(__read_only image2d_t input, __write_only image2d_t output, int width, int height, float curvature, float dMaxRSqaure, float dSR, int CenterX, int CenterY, int CenterX1, int CenterY1,
                                      int CenterX2, int CenterY2, unsigned char getAlphaType)
{
	...
	{
		float fRadius = sqrt(dR1);
		dR1 = ( exp( fRadius * dSR ) - 1 ) * curvature;
	...
	{
		float fRadius = sqrt(dR2);
		dR2 = ( exp( fRadius * dSR ) - 1 ) * curvature;
...
__kernel void DoubleFishEye_Inverse(__read_only image2d_t input, __write_only image2d_t output, int width, int height, float curvature, float dMaxRSqaure, float dS, int CenterX, int CenterY, int CenterX1, int CenterY1,
                                      int CenterX2, int CenterY2, unsigned char getAlphaType)
{
	// convert to polar
	...
	{
		float fRadius = sqrt(dR1);
		dR1 = dS * log(1 + curvature * fRadius);
	...
	{
		float fRadius = sqrt(dR2);
		dR2 = dS * log(1 + curvature * fRadius);
