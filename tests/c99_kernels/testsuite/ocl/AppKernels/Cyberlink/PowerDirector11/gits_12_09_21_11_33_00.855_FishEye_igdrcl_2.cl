...
__kernel void FishEye_NoInverse(__read_only image2d_t input, __write_only image2d_t output, int width, int height, float curvature, float dMaxRSqaure, float dSR, int CenterX, int CenterY, unsigned char getAlphaType)
{
	// convert to polar
	...
	if (dR <= dMaxRSqaure )  // if we're inside R (i.e. inside the circle)
	{
		...
		else
		{
			...
			dR = ( exp( sqrt(dR) * dSR ) - 1 ) * curvature;

			// convert back to cartesian
			float oldX = dR * cos(dA) + (float)CenterX;
			float oldY = dR * sin(dA) + (float)CenterY;
...
__kernel void FishEye_Inverse(__read_only image2d_t input, __write_only image2d_t output, int width, int height, float curvature, float dMaxRSqaure, float dS, int CenterX, int CenterY, unsigned char getAlphaType)
{
	// convert to polar
