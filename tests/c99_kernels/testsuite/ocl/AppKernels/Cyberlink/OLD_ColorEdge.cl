...
__kernel void MaximumPixel(__read_only image2d_t src, __write_only image2d_t dst, __global uchar4* MaximumTable, int width, int height, int block_size, int TableSize)
{
	...
	if(w == 0 && h == 0)
	{
		...
		{
			int nTempMax = MaximumTable[i].x;
			if(nTempMax > nMaximumValue)
				nMaximumValue = nTempMax;
		}

		tempValue.x = nMaximumValue;
		coord = (int2)(0, 0);
...
__kernel void Normalize(__read_only image2d_t src, __read_only image2d_t MaximumValue, __write_only image2d_t dst, int width, int height)
{
	int2 coord = (int2)(get_global_id(0), get_global_id(1));
	int w = get_global_id(0);
	int h = get_global_id(1);
	...
	coord = (int2)(0, 0);
	tempValue = read_imageui(MaximumValue, imageSampler_Right, coord);


	int Ratio = 255 * 65536 / (int)tempValue.x;
