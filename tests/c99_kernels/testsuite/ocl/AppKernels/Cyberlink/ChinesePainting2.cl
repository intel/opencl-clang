...
__kernel void ChinesePainting2_Kernel(__read_only image2d_t src, __read_only image2d_t src2, __read_only image2d_t Original, __write_only image2d_t dst, int Threshold)
{
	...
	float relativeDiff = (float)tempSrc.x / (float)tempSrc2.x;

	float tempResult = 0.0f;

	if (relativeDiff < 1.0f)
	{
		float mult = (ramp - min(ramp, (1.0f - relativeDiff))) / ramp;
		tempResult = (float)tempSrc.x * mult;
	}
	else
	{
		tempResult = (float)tempSrc.x;
	}
