...
//__kernel void AverageGrayPixel(__read_only image2d_t src, __write_only image2d_t dst, __global int* MaximumTable, int width, int height, int block_size, int TableSize,
//					__global float* Mean, int ShiftLength)
__kernel void AverageGrayPixel(__read_only image2d_t src,  __global int* MaximumTable, int width, int height, int block_size, int tableWidth )
{
	int2 coord = (int2)(get_global_id(0), get_global_id(1));
