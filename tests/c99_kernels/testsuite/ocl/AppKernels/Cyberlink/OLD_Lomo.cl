...
__kernel void AverageRGBPixel(__read_only image2d_t src, __global uint* MaximumTable, int width, int height, int block_size, int TableSize, int ShiftLength)
{
	int2 coord = (int2)(get_global_id(0), get_global_id(1));
	...
	for(int x = nStartX; x <= nEndX; x++)
	{
		for(int y = nStartY; y <= nEndY; y++)
		{
			...
			//nTotal += (uint)tempValue.x;
			//nTotal += (uint)tempValue.y;
			//nTotal += (uint)tempValue.z;

		}
	}

	nBlockTotal = nTotal.x + nTotal.y + nTotal.z;

	MaximumTable[w + h * ShiftLength] = nBlockTotal;


}

__kernel void AverageRGBFinal( __global int* MaximumTable, int width, int height, int TableSize, __global float* Mean )
{
	...
	float fMean = (float)nMaximumValue / (float)(width * height * 3);
	Mean[0] = fMean;
