...
__kernel void MoonLight_Kernel(__read_only image2d_t src, __write_only image2d_t dst, int width, int height, int degree, int Reciprocal)
{
	int2 coord;
	...
	int	firstDegree = min( degree, min( h, w ) );
	int	iLayerCount = 1;

	new_w = max( 0, w-1);
	new_h = max( 0, h-1);

	coord = (int2)(new_w, new_h);
	SumARGB = convert_int4( read_imageui(src, imageSampler, coord) );

	for(  ; iLayerCount < firstDegree; iLayerCount++ )
	{
		new_h--;	new_w--;
		coord = (int2)(new_w, new_h);
	...
	for(  ; iLayerCount < degree; iLayerCount++ )
	{
		new_h = max( 0, new_h - 1 );
		new_w = max( 0, new_w - 1 );
	...
	SumARGB.x = (SumARGB.x * Reciprocal) >> 14;
	SumARGB.y = (SumARGB.y * Reciprocal) >> 14;
	SumARGB.z = (SumARGB.z * Reciprocal) >> 14;
