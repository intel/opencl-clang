...
__kernel void generatePrimaryRaysDebug(__global const CLCamera* restrict camera, __global float* restrict rayweights, __global CLRay* restrict rays, __global CLRayDifferential* restrict raydiffs)
{
	// workitem/worksize info
	const size_t idX = get_global_id(0);
	const size_t idY = get_global_id(1);
...
__kernel void convertToUCharDebug(__global const float4* restrict pixelsFloat4, __global uchar4* restrict pixelsUchar4)
{
	// workitem/worksize info
	const size_t idX = get_global_id(0);
	const size_t idY = get_global_id(1);
