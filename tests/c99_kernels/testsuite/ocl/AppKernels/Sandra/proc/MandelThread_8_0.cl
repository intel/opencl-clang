						...
						const float fYinc0, const float fYinc1)
{
	...
	if ((iX < imageW) && (iY < imageH)) {
		...
		const float4 fzpos0 = (float4)(iX, 0, iY, 0);
		const float4 fzinit0 = dsadd4(dsmul4(fzpos0, fzinc), fzmin);
		iCount.s0 = MandelPixelDblEmul(fzinit0);

		const float4 fzpos1 = (float4)(iX+1, 0, iY, 0);
		const float4 fzinit1 = dsadd4(dsmul4(fzpos1, fzinc), fzmin);
		iCount.s1 = MandelPixelDblEmul(fzinit1);

		const float4 fzpos2 = (float4)(iX+2, 0, iY, 0);
		const float4 fzinit2 = dsadd4(dsmul4(fzpos2, fzinc), fzmin);
		iCount.s2 = MandelPixelDblEmul(fzinit2);

		const float4 fzpos3 = (float4)(iX+3, 0, iY, 0);
		const float4 fzinit3 = dsadd4(dsmul4(fzpos3, fzinc), fzmin);
		iCount.s3 = MandelPixelDblEmul(fzinit3);

#if DO_CHECK
        uchar4 iCountGold;

		const float4 gfzpos0 = (float4)(iX, 0, iY, 0);
		const float4 gfzinit0 = dsadd4(dsmul4(gfzpos0, fzinc), fzmin);
		iCountGold.s0 = GoldMandelPixelDblEmul(gfzinit0);

		const float4 gfzpos1 = (float4)(iX+1, 0, iY, 0);
		const float4 gfzinit1 = dsadd4(dsmul4(gfzpos1, fzinc), fzmin);
		iCountGold.s1 = GoldMandelPixelDblEmul(gfzinit1);

		const float4 gfzpos2 = (float4)(iX+2, 0, iY, 0);
		const float4 gfzinit2 = dsadd4(dsmul4(gfzpos1, fzinc), fzmin);
		iCountGold.s2 = GoldMandelPixelDblEmul(gfzinit3);

		const float4 gfzpos3 = (float4)(iX+3, 0, iY, 0);
		const float4 gfzinit3 = dsadd4(dsmul4(gfzpos1, fzinc), fzmin);
