...
#define MAND_EXIT_VAL       (float4)4.0f
...
inline
void MandelAddQuadPixel(__global unsigned char* restrict pMemory, const uchar4 iPixel,
...
uchar4 MandelQuadPixelFloat(const float4 fzinitX, const float4 fzinitY)
{
    ...
    do {
        ...
        fvalY = fma_f(fvalX+fvalX, fvalY, fzinitY);

        fvalX = fma_f(ftmp, ftmp, fzinitX);
        fvalX = fma_f(-fvalY, fvalY, fvalX);
        fvalY = fma_f(ftmp+ftmp, fvalY, fzinitY);
        ...
        fvalY = 2.0f*fvalX*fvalY + fzinitY;
        fvalX = ftmp*ftmp + fzinitX - fvalY*fvalY ;
        fvalY = 2.0f*ftmp*fvalY + fzinitY;
		...
		#ifdef USE_MAD
		stay = (fma_f(fvalY, fvalY, fvalX*fvalX) <= MAND_EXIT_VAL);
#else
        ...
        ccount -= stay*ITER_PER_LOOP;  	} while ((stay.s0|stay.s1|stay.s2|stay.s3) && (acount));
      ...
      do {
        ...
        ccount-=stay;
#ifdef USE_MAD
		stay = (fma_f(fvalY, fvalY, ftmp*ftmp) <= MAND_EXIT_VAL);
...
inline int MandelPixelFloat(const float2 fzinit)
{
    ...
    do {
        ...
        fval.y = 2.0f*fval.x*fval.y + fzinit.y;
...
__attribute__((vec_type_hint(char4)))
