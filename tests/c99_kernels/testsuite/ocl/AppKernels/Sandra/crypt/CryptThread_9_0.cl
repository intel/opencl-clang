...
inline void sha256_step_global( global UINT *data, UINT *state, int offset )
{
   DEF_W_VARS;
   UINT temp1, temp2;
...
inline void sha256_step_local( const UINT *data, UINT *state, int offset )
{
   DEF_W_VARS;
   UINT temp1, temp2;
   UINT A, B, C, D, E, F, G, H;
...
__kernel void CryptThread(__global uint *buffer, __global uint *state, const uint blockLen)
{
	const uint init[8] = {
		0x6a09e667,
		0xbb67ae85,
...
#else
	const uint step = WAVE_FRONT;
	const uint startPosInDWORDs = (len*((id / WAVE_FRONT) * WAVE_FRONT))/4 + (id % WAVE_FRONT);
#endif
