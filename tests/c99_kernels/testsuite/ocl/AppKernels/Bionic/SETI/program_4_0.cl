...
__kernel void fft0(__global float2 *in, __global float2 *out, int dir, int S, __global float2 * cossinLUT1, __global float2 * cossinLUT2)
{
 ...
 barrier(CLK_LOCAL_MEM_FENCE);
    ...
    ii = lId & 15;
    jj = lId >> 4;
    lMemStore = sMem + mad24( jj, 18, ii );
    offset = mad24( groupId, 32, jj);
    offset = mad24( offset, 16, ii );
        in += offset;
        out += offset;
if((groupId == get_num_groups(0)-1) && s) {
    ...
    ang = dir * ( 2.0f * M_PI * 4.0f / 16.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[4] = complexMul(a[4], w);
    ang = dir * ( 2.0f * M_PI * 5.0f / 16.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[5] = complexMul(a[5], w);
    ang = dir * ( 2.0f * M_PI * 6.0f / 16.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[6] = complexMul(a[6], w);
    ang = dir * ( 2.0f * M_PI * 7.0f / 16.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[7] = complexMul(a[7], w);
    lMemStore = sMem + mad24(jj, 24, ii);
    j = ii;
    i = 0;
    i = mad24(jj, 24, i);
    lMemLoad = sMem + mad24(j, 3, i);
    lMemStore[0] = a[0].x;
    lMemStore[3] = a[1].x;
    lMemStore[6] = a[2].x;
    lMemStore[9] = a[3].x;
