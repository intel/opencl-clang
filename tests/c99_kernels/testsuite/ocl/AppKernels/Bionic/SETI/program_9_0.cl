...
__kernel void fft0(__global float2 *in, __global float2 *out, int dir, int S, __global float2 * cossinLUT1, __global float2 * cossinLUT2)
{
 ...
 barrier(CLK_LOCAL_MEM_FENCE);
    ...
    fftKernel8(a+0, dir);
    angf = (float) ii;
    ang = dir * ( 2.0f * M_PI * 1.0f / 512.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[1] = complexMul(a[1], w);
    ang = dir * ( 2.0f * M_PI * 2.0f / 512.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[2] = complexMul(a[2], w);
    ang = dir * ( 2.0f * M_PI * 3.0f / 512.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[3] = complexMul(a[3], w);
    ang = dir * ( 2.0f * M_PI * 4.0f / 512.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[4] = complexMul(a[4], w);
    ang = dir * ( 2.0f * M_PI * 5.0f / 512.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[5] = complexMul(a[5], w);
    ang = dir * ( 2.0f * M_PI * 6.0f / 512.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[6] = complexMul(a[6], w);
    ang = dir * ( 2.0f * M_PI * 7.0f / 512.0f ) * angf;
    w = (float2)(native_cos(ang), native_sin(ang));
    a[7] = complexMul(a[7], w);
    lMemStore = sMem + ii;
    j = ii & 7;
    i = ii >> 3;
    lMemLoad = sMem + mad24(j, 66, i);
    lMemStore[0] = a[0].x;
    lMemStore[66] = a[1].x;
    lMemStore[132] = a[2].x;
    lMemStore[198] = a[3].x;
