...
l = ((bNum << 3) + i) >> 0;
k = j << 0;
{ int ang_index = (1 * ( l * (k + 0))) & 32767;
 int k = (ang_index +  64) >> 7;
 float2 csx0 =cossin_T_LUT[k];
 float2 csx0Transp= (float2)(csx0.y,-csx0.x);
 int    r=ang_index - k * ( 128 );
  float mh=-1.9174760382e-004*(float)r;
...
l = ((bNum << 3) + i) >> 6;
k = j << 0;
{ int ang_index = (1 * ( l * (k + 0))) & 511;
 int k = (ang_index +  1) >> 1;
 float2 csx0 =cossin_T_LUT[k];
 float2 csx0Transp= (float2)(csx0.y,-csx0.x);
 int    r=ang_index - k * ( 2 );
  float mh=-1.2271846645e-002*(float)r;
  float mhsqr2= mh*mh*(-0.5f);
  float hqub6= mhsqr2*mh*(1.0f/3.0f);
  float2 cs;
   cs= hqub6 * csx0Transp;
