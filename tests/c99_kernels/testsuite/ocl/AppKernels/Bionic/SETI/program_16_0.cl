...
l = ((bNum << 3) + i) >> 0;
k = j << 0;
{ int ang_index = (1 * ( l * (k + 0))) & 65535;
 int k = (ang_index +  128) >> 8;
 float2 csx0 =cossin_T_LUT[k];
 float2 csx0Transp= (float2)(csx0.y,-csx0.x);
 int    r=ang_index - k * ( 256 );
  float mh=-9.5873801911e-005*(float)r;
...
{ int ang_index = (1 * ( l * (k + 0))) & 1023;
 ...
 float2 csx0Transp= (float2)(csx0.y,-csx0.x);
 int    r=ang_index - k * ( 4 );
  float mh=-6.1359233223e-003*(float)r;
  float mhsqr2= mh*mh*(-0.5f);
  float hqub6= mhsqr2*mh*(1.0f/3.0f);
  float2 cs;
   cs= hqub6 * csx0Transp;
