...
l = ((bNum << 3) + i) >> 0;
k = j << 0;
{ int ang_index = (1 * ( l * (k + 0))) & 524287;
 int k = (ang_index +  1024) >> 11;
 float2 csx0 =cossin_T_LUT[k];
 float2 csx0Transp= (float2)(csx0.y,-csx0.x);
 int    r=ang_index - k * ( 2048 );
  float mh=-1.1984225239e-005*(float)r;
...
l = ((bNum << 3) + i) >> 6;
k = j << 0;
{ int ang_index = (1 * ( l * (k + 0))) & 8191;
 int k = (ang_index +  16) >> 5;
 float2 csx0 =cossin_T_LUT[k];
 float2 csx0Transp= (float2)(csx0.y,-csx0.x);
 int    r=ang_index - k * ( 32 );
  float mh=-7.6699041529e-004*(float)r;
  float mhsqr2= mh*mh*(-0.5f);
  float hqub6= mhsqr2*mh*(1.0f/3.0f);
  float2 cs;
   cs= hqub6 * csx0Transp;
...
__kernel __attribute__((reqd_work_group_size(64, 1, 1)))
 void fft2(__global float2 *in, __global float2 *out, int dir, int S, __global float2 * cossinLUT1, __global float2 * cossinLUT2)
{
    __local float sMem[512];
    int i, j, r, indexIn, indexOut, index, tid, bNum, xNum, k, l;
    int s, ii, jj, offset;
...
__kernel __attribute__((reqd_work_group_size(64, 1, 1)))
 void fft3(__global float2 *in, __global float2 *out, int dir, int S, __global float2 * cossinLUT1, __global float2 * cossinLUT2)
{
    int i, j, r, indexIn, indexOut, index, tid, bNum, xNum, k, l;
    int s, ii, jj, offset;
    float2 w;
...
bNum = groupId & 4095;
xNum = groupId >> 12;
indexIn = mul24(bNum, 64);
tid = indexIn;
i = tid >> 18;
j = tid & 262143;
indexOut = mad24(i, 524288, j);
indexIn += (xNum << 19);
indexOut += (xNum << 19);
tid = lId;
i = tid & 63;
j = tid >> 6;
indexIn += mad24(j, 262144, i);
in += indexIn;
a[0] = in[0];
a[1] = in[262144];
fftKernel2(a, dir);
indexOut += mad24(j, 524288, i);
out += indexOut;
out[0] = a[0];
out[262144] = a[1];
}
