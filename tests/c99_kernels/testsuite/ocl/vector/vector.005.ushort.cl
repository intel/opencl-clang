...
int main_function_ocl(int argc, ushort **argv, __global int *ocl_test_results) {
    ushort4 vf = (ushort4)(1, 2, 3, 4);
    ushort2 low = vf.lo; // returns vf.xy
    ushort2 high = vf.hi; // returns vf.zw
    ushort2 even = vf.even; // returns vf.xz
    ushort2 odd = vf.odd; // returns vf.yw
