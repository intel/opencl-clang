...
int main_function_ocl(int argc, char **argv, __global int *ocl_test_results) {
    ...
    if (!((pos.x == 5.0f) && (pos.y == 2.0f) && (pos.z == 3.0f) && (pos.w == 6.0f)))
        return -1;

    pos.wx = (float2)(7.0f, 8.0f);// pos = (8.0f, 2.0f, 3.0f, 7.0f)
    if (!((pos.x == 8.0f) && (pos.y == 2.0f) && (pos.z == 3.0f) && (pos.w == 7.0f)))
        return -1;

    pos.xyz = (float3)(3.0f, 5.0f, 9.0f); // pos = (3.0f, 5.0f, 9.0f, 7.0f)
    if (!((pos.x == 3.0f) && (pos.y == 5.0f) && (pos.z == 9.0f) && (pos.w == 7.0f)))
        return -1;
