...
float Texture_Index0_EvaluateFloat(__global Texture *texture, __global HitPoint *hitPoint
		TEXTURES_PARAM_DECL) {
	return ImageMapTexture_ConstEvaluateFloat(hitPoint, 1.1, 0, &texture->imageMapTex.mapping IMAGEMAPS_PARAM);
}
float3 Texture_Index0_EvaluateSpectrum(__global Texture *texture, __global HitPoint *hitPoint
		TEXTURES_PARAM_DECL) {
	return ImageMapTexture_ConstEvaluateSpectrum(hitPoint, 1.1, 0, &texture->imageMapTex.mapping IMAGEMAPS_PARAM);
}
float Texture_Index1_EvaluateFloat(__global Texture *texture, __global HitPoint *hitPoint
		TEXTURES_PARAM_DECL) {
	return ConstFloat3Texture_ConstEvaluateFloat(hitPoint, (float3)(0.8, 0.8, 0.8));
}
float3 Texture_Index1_EvaluateSpectrum(__global Texture *texture, __global HitPoint *hitPoint
		TEXTURES_PARAM_DECL) {
	return ConstFloat3Texture_ConstEvaluateSpectrum(hitPoint, (float3)(0.8, 0.8, 0.8));
}
float Texture_Index2_EvaluateFloat(__global Texture *texture, __global HitPoint *hitPoint
		TEXTURES_PARAM_DECL) {
	return ConstFloat3Texture_ConstEvaluateFloat(hitPoint, (float3)(0.25, 0.25, 0.25));
}
float3 Texture_Index2_EvaluateSpectrum(__global Texture *texture, __global HitPoint *hitPoint
		TEXTURES_PARAM_DECL) {
	return ConstFloat3Texture_ConstEvaluateSpectrum(hitPoint, (float3)(0.25, 0.25, 0.25));
}
float Texture_Index3_EvaluateFloat(__global Texture *texture, __global HitPoint *hitPoint
		TEXTURES_PARAM_DECL) {
	return ConstFloat3Texture_ConstEvaluateFloat(hitPoint, (float3)(0.75, 0.75, 0.75));
}
float3 Texture_Index3_EvaluateSpectrum(__global Texture *texture, __global HitPoint *hitPoint
		TEXTURES_PARAM_DECL) {
...
#if defined(PARAM_HAS_PASSTHROUGH)
float3 MatteTranslucentMaterial_GetPassThroughTransparency(__global Material *material,
		__global HitPoint *hitPoint, const float3 localFixedDir, const float passThroughEvent
		TEXTURES_PARAM_DECL) {
	return BLACK;
}
...
float3 MirrorMaterial_Sample(__global Material *material,
		...
		const BSDFEvent requestedEvent
		TEXTURES_PARAM_DECL) {
	const float3 krVal = Texture_GetSpectrumValue(material->mirror.krTexIndex, hitPoint
			TEXTURES_PARAM);

	return MirrorMaterial_ConstSample(hitPoint, fixedDir, sampledDir,
			u0, u1,
#if defined(PARAM_HAS_PASSTHROUGH)
			passThroughEvent,
#endif
...
BSDFEvent Material_Index0_GetEventTypes(__global Material *material MATERIALS_PARAM_DECL) {
	return MatteMaterial_GetEventTypes();
}
bool Material_Index0_IsDelta(__global Material *material MATERIALS_PARAM_DECL) {
	return MatteMaterial_IsDelta();
}
float3 Material_Index0_Evaluate(__global Material *material, __global HitPoint *hitPoint, const float3 lightDir, const float3 eyeDir, BSDFEvent *event, float *directPdfW MATERIALS_PARAM_DECL) {
...
float3 Material_Index0_Sample(__global Material *material, __global HitPoint *hitPoint, const float3 fixedDir, float3 *sampledDir, const float u0, const float u1,
#if defined(PARAM_HAS_PASSTHROUGH)
	const float passThroughEvent,
#endif
...
#if defined(PARAM_HAS_PASSTHROUGH)
float3 Material_Index0_GetPassThroughTransparency(__global Material *material, __global HitPoint *hitPoint, const float3 localFixedDir, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return MatteMaterial_GetPassThroughTransparency(material, hitPoint, localFixedDir, passThroughEvent TEXTURES_PARAM);
}
#endif
float3 Material_Index0_GetEmittedRadiance(__global Material *material, __global HitPoint *hitPoint, const float oneOverPrimitiveArea MATERIALS_PARAM_DECL) {
	return Material_GetEmittedRadianceNoMix(material, hitPoint TEXTURES_PARAM);
}
#if defined(PARAM_HAS_BUMPMAPS)
void Material_Index0_Bump(__global Material *material, __global HitPoint *hitPoint, const float3 dpdu, const float3 dpdv, const float3 dndu, const float3 dndv, const float weight MATERIALS_PARAM_DECL) {
	 Material_BumpNoMix(material, hitPoint, dpdu, dpdv, dndu, dndv, weight TEXTURES_PARAM);
}
#endif
#if defined(PARAM_HAS_VOLUMES)
uint Material_Index0_GetInteriorVolume(__global Material *material, __global HitPoint *hitPoint, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return Material_GetInteriorVolumeNoMix(material);
}
uint Material_Index0_GetExteriorVolume(__global Material *material, __global HitPoint *hitPoint, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return Material_GetExteriorVolumeNoMix(material);
}
#endif
BSDFEvent Material_Index1_GetEventTypes(__global Material *material MATERIALS_PARAM_DECL) {
	return MirrorMaterial_GetEventTypes();
}
bool Material_Index1_IsDelta(__global Material *material MATERIALS_PARAM_DECL) {
	return MirrorMaterial_IsDelta();
}
float3 Material_Index1_Evaluate(__global Material *material, __global HitPoint *hitPoint, const float3 lightDir, const float3 eyeDir, BSDFEvent *event, float *directPdfW MATERIALS_PARAM_DECL) {
...
float3 Material_Index1_Sample(__global Material *material, __global HitPoint *hitPoint, const float3 fixedDir, float3 *sampledDir, const float u0, const float u1,
#if defined(PARAM_HAS_PASSTHROUGH)
	const float passThroughEvent,
#endif
	float *pdfW, float *cosSampledDir, BSDFEvent *event, const BSDFEvent requestedEvent MATERIALS_PARAM_DECL) {
...
#if defined(PARAM_HAS_PASSTHROUGH)
float3 Material_Index1_GetPassThroughTransparency(__global Material *material, __global HitPoint *hitPoint, const float3 localFixedDir, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return MirrorMaterial_GetPassThroughTransparency(material, hitPoint, localFixedDir, passThroughEvent TEXTURES_PARAM);
}
#endif
float3 Material_Index1_GetEmittedRadiance(__global Material *material, __global HitPoint *hitPoint, const float oneOverPrimitiveArea MATERIALS_PARAM_DECL) {
	return Material_GetEmittedRadianceNoMix(material, hitPoint TEXTURES_PARAM);
}
#if defined(PARAM_HAS_BUMPMAPS)
void Material_Index1_Bump(__global Material *material, __global HitPoint *hitPoint, const float3 dpdu, const float3 dpdv, const float3 dndu, const float3 dndv, const float weight MATERIALS_PARAM_DECL) {
	 Material_BumpNoMix(material, hitPoint, dpdu, dpdv, dndu, dndv, weight TEXTURES_PARAM);
}
#endif
#if defined(PARAM_HAS_VOLUMES)
uint Material_Index1_GetInteriorVolume(__global Material *material, __global HitPoint *hitPoint, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return Material_GetInteriorVolumeNoMix(material);
}
uint Material_Index1_GetExteriorVolume(__global Material *material, __global HitPoint *hitPoint, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return Material_GetExteriorVolumeNoMix(material);
}
#endif
BSDFEvent Material_Index2_GetEventTypes(__global Material *material MATERIALS_PARAM_DECL) {
	return MatteMaterial_GetEventTypes();
}
bool Material_Index2_IsDelta(__global Material *material MATERIALS_PARAM_DECL) {
	return MatteMaterial_IsDelta();
}
float3 Material_Index2_Evaluate(__global Material *material, __global HitPoint *hitPoint, const float3 lightDir, const float3 eyeDir, BSDFEvent *event, float *directPdfW MATERIALS_PARAM_DECL) {
...
float3 Material_Index2_Sample(__global Material *material, __global HitPoint *hitPoint, const float3 fixedDir, float3 *sampledDir, const float u0, const float u1,
#if defined(PARAM_HAS_PASSTHROUGH)
	const float passThroughEvent,
#endif
	float *pdfW, float *cosSampledDir, BSDFEvent *event, const BSDFEvent requestedEvent MATERIALS_PARAM_DECL) {
...
#if defined(PARAM_HAS_PASSTHROUGH)
float3 Material_Index2_GetPassThroughTransparency(__global Material *material, __global HitPoint *hitPoint, const float3 localFixedDir, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return MatteMaterial_GetPassThroughTransparency(material, hitPoint, localFixedDir, passThroughEvent TEXTURES_PARAM);
}
#endif
float3 Material_Index2_GetEmittedRadiance(__global Material *material, __global HitPoint *hitPoint, const float oneOverPrimitiveArea MATERIALS_PARAM_DECL) {
	return Material_GetEmittedRadianceNoMix(material, hitPoint TEXTURES_PARAM);
}
#if defined(PARAM_HAS_BUMPMAPS)
void Material_Index2_Bump(__global Material *material, __global HitPoint *hitPoint, const float3 dpdu, const float3 dpdv, const float3 dndu, const float3 dndv, const float weight MATERIALS_PARAM_DECL) {
	 Material_BumpNoMix(material, hitPoint, dpdu, dpdv, dndu, dndv, weight TEXTURES_PARAM);
}
#endif
#if defined(PARAM_HAS_VOLUMES)
uint Material_Index2_GetInteriorVolume(__global Material *material, __global HitPoint *hitPoint, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return Material_GetInteriorVolumeNoMix(material);
}
uint Material_Index2_GetExteriorVolume(__global Material *material, __global HitPoint *hitPoint, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return Material_GetExteriorVolumeNoMix(material);
}
#endif
BSDFEvent Material_Index3_GetEventTypes(__global Material *material MATERIALS_PARAM_DECL) {
	return MatteMaterial_GetEventTypes();
}
bool Material_Index3_IsDelta(__global Material *material MATERIALS_PARAM_DECL) {
	return MatteMaterial_IsDelta();
}
float3 Material_Index3_Evaluate(__global Material *material, __global HitPoint *hitPoint, const float3 lightDir, const float3 eyeDir, BSDFEvent *event, float *directPdfW MATERIALS_PARAM_DECL) {
...
float3 Material_Index3_Sample(__global Material *material, __global HitPoint *hitPoint, const float3 fixedDir, float3 *sampledDir, const float u0, const float u1,
#if defined(PARAM_HAS_PASSTHROUGH)
	const float passThroughEvent,
#endif
	float *pdfW, float *cosSampledDir, BSDFEvent *event, const BSDFEvent requestedEvent MATERIALS_PARAM_DECL) {
...
#if defined(PARAM_HAS_PASSTHROUGH)
float3 Material_Index3_GetPassThroughTransparency(__global Material *material, __global HitPoint *hitPoint, const float3 localFixedDir, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return MatteMaterial_GetPassThroughTransparency(material, hitPoint, localFixedDir, passThroughEvent TEXTURES_PARAM);
}
#endif
float3 Material_Index3_GetEmittedRadiance(__global Material *material, __global HitPoint *hitPoint, const float oneOverPrimitiveArea MATERIALS_PARAM_DECL) {
	return Material_GetEmittedRadianceNoMix(material, hitPoint TEXTURES_PARAM);
}
#if defined(PARAM_HAS_BUMPMAPS)
void Material_Index3_Bump(__global Material *material, __global HitPoint *hitPoint, const float3 dpdu, const float3 dpdv, const float3 dndu, const float3 dndv, const float weight MATERIALS_PARAM_DECL) {
	 Material_BumpNoMix(material, hitPoint, dpdu, dpdv, dndu, dndv, weight TEXTURES_PARAM);
}
#endif
#if defined(PARAM_HAS_VOLUMES)
uint Material_Index3_GetInteriorVolume(__global Material *material, __global HitPoint *hitPoint, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return Material_GetInteriorVolumeNoMix(material);
}
uint Material_Index3_GetExteriorVolume(__global Material *material, __global HitPoint *hitPoint, const float passThroughEvent MATERIALS_PARAM_DECL) {
	return Material_GetExteriorVolumeNoMix(material);
}
#endif
...
#if defined(PARAM_HAS_BUMPMAPS)
void Material_BumpWithMix(const uint index, __global HitPoint *hitPoint, const float3 dpdu, const float3 dpdv, const float3 dndu, const float3 dndv, const float weight MATERIALS_PARAM_DECL) {
	__global Material *mat = &mats[index];
	switch (index) {
		case 0:
			Material_Index0_Bump(mat, hitPoint, dpdu, dpdv, dndu, dndv, weight MATERIALS_PARAM);
			break;
		case 1:
			Material_Index1_Bump(mat, hitPoint, dpdu, dpdv, dndu, dndv, weight MATERIALS_PARAM);
			break;
		case 2:
			Material_Index2_Bump(mat, hitPoint, dpdu, dpdv, dndu, dndv, weight MATERIALS_PARAM);
			break;
		case 3:
			Material_Index3_Bump(mat, hitPoint, dpdu, dpdv, dndu, dndv, weight MATERIALS_PARAM);
			break;
	}
}
#endif
