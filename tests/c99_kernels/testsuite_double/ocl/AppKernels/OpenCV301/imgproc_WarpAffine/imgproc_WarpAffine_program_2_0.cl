...
// buildOptions=-D INTER_LINEAR -D T=uchar -D T1=uchar -D ST=float -D WT=float -D depth=0 -D convertToWT=convert_float -D convertToT=convert_uchar_sat_rte -D cn=1 -D rowsPerWI=4 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_LINEAR -D T=uchar -D T1=uchar -D ST=float -D WT=float -D depth=0 -D convertToWT=convert_float -D convertToT=convert_uchar_sat_rte -D cn=1 -D rowsPerWI=4 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
