...
// buildOptions=-D INTER_LINEAR -D T=uchar4 -D T1=uchar -D ST=float4 -D WT=float4 -D depth=0 -D convertToWT=convert_float4 -D convertToT=convert_uchar4_sat_rte -D cn=4 -D rowsPerWI=4 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_LINEAR -D T=uchar4 -D T1=uchar -D ST=float4 -D WT=float4 -D depth=0 -D convertToWT=convert_float4 -D convertToT=convert_uchar4_sat_rte -D cn=4 -D rowsPerWI=4 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
