...
// buildOptions=-D INTER_CUBIC -D T=uchar3 -D T1=uchar -D ST=float4 -D WT=float3 -D depth=0 -D convertToWT=convert_float3 -D convertToT=convert_uchar3_sat_rte -D cn=3 -D rowsPerWI=1 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_CUBIC -D T=uchar3 -D T1=uchar -D ST=float4 -D WT=float3 -D depth=0 -D convertToWT=convert_float3 -D convertToT=convert_uchar3_sat_rte -D cn=3 -D rowsPerWI=1 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
