...
// buildOptions=-D INTER_CUBIC -D T=float -D T1=float -D ST=float -D WT=float -D depth=5 -D convertToWT=noconvert -D convertToT=noconvert -D cn=1 -D rowsPerWI=1 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_CUBIC -D T=float -D T1=float -D ST=float -D WT=float -D depth=5 -D convertToWT=noconvert -D convertToT=noconvert -D cn=1 -D rowsPerWI=1 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
