...
// buildOptions=-D INTER_NEAREST -D T=float -D T1=float -D ST=float -D cn=1 -D rowsPerWI=1 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D INTER_NEAREST -D T=float -D T1=float -D ST=float -D cn=1 -D rowsPerWI=1 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
