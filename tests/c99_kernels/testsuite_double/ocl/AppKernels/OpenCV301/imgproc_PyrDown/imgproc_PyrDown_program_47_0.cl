...
// buildOptions=-D T=float4 -D FT=float4 -D convertToT=noconvert -D convertToFT=noconvert -D T1=float -D cn=4 -D kercn=1 -D fdepth=5 -D BORDER_REFLECT -D LOCAL_SIZE=256 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=float4 -D FT=float4 -D convertToT=noconvert -D convertToFT=noconvert -D T1=float -D cn=4 -D kercn=1 -D fdepth=5 -D BORDER_REFLECT -D LOCAL_SIZE=256 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
