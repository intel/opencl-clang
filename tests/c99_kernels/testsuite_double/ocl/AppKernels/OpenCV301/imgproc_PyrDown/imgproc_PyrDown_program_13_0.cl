...
// buildOptions=-D T=ushort -D FT=float -D convertToT=convert_ushort_sat_rte -D convertToFT=convert_float -D T1=ushort -D cn=1 -D kercn=1 -D fdepth=5 -D BORDER_REPLICATE -D LOCAL_SIZE=256 -D INTEL_DEVICE
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D T=ushort -D FT=float -D convertToT=convert_ushort_sat_rte -D convertToFT=convert_float -D T1=ushort -D cn=1 -D kercn=1 -D fdepth=5 -D BORDER_REPLICATE -D LOCAL_SIZE=256 -D INTEL_DEVICE" %cfg_path --cl-device=%cl_device 2>&1
