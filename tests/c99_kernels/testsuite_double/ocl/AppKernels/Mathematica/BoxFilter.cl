...
#ifdef USETEXTURE
    // Row summation filter kernel with rescaling, using Image (texture)
    __kernel void BoxRowsTex( __read_only image2d_t SourceRgbaTex, __global unsigned mint* uiDest, sampler_t RowSampler,
                              unsigned mint uiWidth, unsigned mint uiHeight, mint iRadius, float fScale)
    {
...
//*****************************************************************
    ...
    // Row summation filter kernel with rescaling, using LMEM
    __kernel void BoxRowsLmem( __global const uchar4* uc4Source, __global unsigned mint* uiDest,
                               __local uchar4* uc4LocalData,
                               unsigned mint uiWidth, unsigned mint uiHeight, mint iRadius, mint iRadiusAligned,
                               float fScale, unsigned mint uiNumOutputPix)
    {
...
//*****************************************************************
__kernel void BoxColumns(__global unsigned mint* uiInputImage, __global unsigned mint* uiOutputImage,
                         unsigned mint uiWidth, unsigned mint uiHeight, mint iRadius, float fScale)
{
