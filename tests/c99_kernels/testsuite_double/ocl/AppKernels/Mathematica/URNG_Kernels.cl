...
float ran1(mint idum, __local mint *iv)
{
    int j;
    int k;
    int iy = 0;
...
__kernel void noise_uniform(__global float4* inputImage, __global float4* outputImage, mint factor)
{
	int pos = get_global_id(0) + get_global_id(1) * get_global_size(0);

	/* compute average value of a pixel from its compoments */
	int avg = 255 * (inputImage[pos].x + inputImage[pos].y + inputImage[pos].z + inputImage[pos].w) / 4.0;
	...
	outputImage[pos] = inputImage[pos] + (float4)(dev);
}
