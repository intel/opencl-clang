...
////////////////////////////////////////////////////////////////////////////////
    ...
    inline unsigned int scan1Inclusive(unsigned int idata, __local unsigned int *l_Data, unsigned int size){
    ...
    inline unsigned int scan1Exclusive(unsigned int idata, __local unsigned int *l_Data, unsigned int size){
    ...
    inline unsigned int warpScanInclusive(unsigned int idata, __local unsigned int *l_Data, unsigned int size){
        uint pos = 2 * get_local_id(0) - (get_local_id(0) & (size - 1));
        l_Data[pos] = 0;
        pos += size;
        l_Data[pos] = idata;
    ...
    inline unsigned int warpScanExclusive(unsigned int idata, __local unsigned int *l_Data, unsigned int size){
...
inline uint4 scan4Inclusive(uint4 data4, __local unsigned int *l_Data, unsigned int size){
...
inline uint4 scan4Exclusive(uint4 data4, __local unsigned int *l_Data, unsigned int size){
...
void scanExclusiveLocal2(
    __global unsigned int *d_Buf,
    __global unsigned int *d_Dst,
    __global unsigned int *d_Src,
    __local unsigned int *l_Data,
    unsigned int N,
...
// buildOptions=-D WORKGROUP_SIZE=100
// RUN: %occ-cli %s --cl-options="-I%cwd -I%S -D WORKGROUP_SIZE=100" %cfg_path --cl-device=%cl_device 2>&1
