...
__kernel void test_fn(const __global char2 *src, __global char2 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (char2)(char)0;
  ...
  event = async_work_group_strided_copy(
      (__global char2 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const char2 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
