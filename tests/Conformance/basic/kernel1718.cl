...
__kernel void test_fn(const __global short3 *src, __global short3 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (short3)(short)0;
  ...
  event = async_work_group_strided_copy(
      (__global short3 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const short3 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
