...
__kernel void test_fn(const __global uint3 *src, __global uint3 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (uint3)(uint)0;
  ...
  event = async_work_group_strided_copy(
      (__global uint3 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const uint3 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
