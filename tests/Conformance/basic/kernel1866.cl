...
__kernel void test_fn(const __global float4 *src, __global float4 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (float4)(float)0;
  ...
  event = async_work_group_strided_copy(
      (__global float4 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const float4 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
