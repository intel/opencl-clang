...
__kernel void test_fn(const __global ushort8 *src, __global ushort8 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (ushort8)(ushort)0;
  ...
  event = async_work_group_strided_copy(
      (__global ushort8 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const ushort8 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
