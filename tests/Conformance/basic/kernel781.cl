...
__kernel void test_fn(__local char *sSharedStorage, __global char *srcValues,
                      __global uint *offsets, __global char *destBuffer,
                      uint alignmentOffset) {
  int tid = get_global_id(0);
  sSharedStorage[3 * offsets[tid]] = (char)0;
  ...
  int i;
  __local char *sp = (sSharedStorage + 3 * offsets[tid]) + alignmentOffset;
  __global char *dp = (destBuffer + 3 * offsets[tid]) + alignmentOffset;
  for (i = 0; i < 3; i++)
    dp[i] = sp[i];
}
