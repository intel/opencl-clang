...
__kernel void test_fn(const __global uchar *src, __global uchar *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (uchar)(uchar)0;
  ...
  event = async_work_group_strided_copy(
      (__global uchar *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const uchar *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
