...
__kernel void test_fn(const __global ulong2 *src, __global ulong2 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (ulong2)(ulong)0;
  ...
  event = async_work_group_strided_copy(
      (__global ulong2 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const ulong2 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
