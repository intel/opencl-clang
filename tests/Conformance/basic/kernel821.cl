...
__kernel void test_fn(__local float *sSharedStorage, __global float *srcValues,
                      __global uint *offsets, __global float *destBuffer,
                      uint alignmentOffset) {
  int tid = get_global_id(0);
  sSharedStorage[3 * offsets[tid]] = (float)0;
  ...
  int i;
  __local float *sp = (sSharedStorage + 3 * offsets[tid]) + alignmentOffset;
  __global float *dp = (destBuffer + 3 * offsets[tid]) + alignmentOffset;
  for (i = 0; i < 3; i++)
    dp[i] = sp[i];
}
