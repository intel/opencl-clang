...
__kernel void test_fn(const __global long3 *src, __global long3 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (long3)(long)0;
  ...
  event = async_work_group_strided_copy(
      (__global long3 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const long3 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
