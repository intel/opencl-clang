...
__kernel void test_fn(__local uchar *sSharedStorage, __global uchar *srcValues,
                      __global uint *offsets, __global uchar *destBuffer,
                      uint alignmentOffset) {
  int tid = get_global_id(0);
  sSharedStorage[3 * offsets[tid]] = (uchar)0;
  ...
  int i;
  __local uchar *sp = (sSharedStorage + 3 * offsets[tid]) + alignmentOffset;
  __global uchar *dp = (destBuffer + 3 * offsets[tid]) + alignmentOffset;
  for (i = 0; i < 3; i++)
    dp[i] = sp[i];
}
