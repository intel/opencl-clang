...
__kernel void test_fn(__local float4 *sSharedStorage,
                      __global float4 *srcValues, __global uint *offsets,
                      __global float4 *destBuffer, uint alignmentOffset) {
  int tid = get_global_id(0);
  sSharedStorage[offsets[tid]] = (float4)(float)0;
