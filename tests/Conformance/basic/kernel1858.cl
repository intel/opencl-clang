...
__kernel void test_fn(const __global float2 *src, __global float2 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (float2)(float)0;
  ...
  event = async_work_group_strided_copy(
      (__global float2 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const float2 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
