...
__kernel void test_fn(const __global float3 *src, __global float3 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (float3)(float)0;
  ...
  event = async_work_group_strided_copy(
      (__global float3 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const float3 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
