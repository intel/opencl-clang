...
__kernel void test_fn(const __global float16 *src, __global float16 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (float16)(float)0;
  ...
  event = async_work_group_strided_copy(
      (__global float16 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const float16 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
