...
__kernel void test_fn(const __global ulong3 *src, __global ulong3 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (ulong3)(ulong)0;
  ...
  event = async_work_group_strided_copy(
      (__global ulong3 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const ulong3 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
