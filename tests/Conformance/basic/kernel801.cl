...
__kernel void test_fn(__local int *sSharedStorage, __global int *srcValues,
                      __global uint *offsets, __global int *destBuffer,
                      uint alignmentOffset) {
  int tid = get_global_id(0);
  sSharedStorage[3 * offsets[tid]] = (int)0;
  ...
  int i;
  __local int *sp = (sSharedStorage + 3 * offsets[tid]) + alignmentOffset;
  __global int *dp = (destBuffer + 3 * offsets[tid]) + alignmentOffset;
  for (i = 0; i < 3; i++)
    dp[i] = sp[i];
}
