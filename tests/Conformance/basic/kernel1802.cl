...
__kernel void test_fn(const __global uint16 *src, __global uint16 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (uint16)(uint)0;
  ...
  event = async_work_group_strided_copy(
      (__global uint16 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const uint16 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
