...
__kernel void test_fn(const __global ulong4 *src, __global ulong4 *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (ulong4)(ulong)0;
  ...
  event = async_work_group_strided_copy(
      (__global ulong4 *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const ulong4 *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
