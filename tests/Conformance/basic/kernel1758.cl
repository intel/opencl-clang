...
__kernel void test_fn(const __global int *src, __global int *dst,
  ...
  for (i = 0; i < copiesPerWorkItem; i++)
    localBuffer[get_local_id(0) * copiesPerWorkItem + i] = (int)(int)0;
  ...
  event = async_work_group_strided_copy(
      (__global int *)(dst + copiesPerWorkgroup * stride * get_group_id(0)),
      (__local const int *)localBuffer, (size_t)copiesPerWorkgroup,
      (size_t)stride, 0);
