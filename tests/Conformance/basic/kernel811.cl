...
__kernel void test_fn(__local long *sSharedStorage, __global long *srcValues,
                      __global uint *offsets, __global long *destBuffer,
                      uint alignmentOffset) {
  int tid = get_global_id(0);
  sSharedStorage[3 * offsets[tid]] = (long)0;
  ...
  int i;
  __local long *sp = (sSharedStorage + 3 * offsets[tid]) + alignmentOffset;
  __global long *dp = (destBuffer + 3 * offsets[tid]) + alignmentOffset;
  for (i = 0; i < 3; i++)
    dp[i] = sp[i];
}
